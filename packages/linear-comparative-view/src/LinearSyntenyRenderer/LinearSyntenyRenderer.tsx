...
export default class LinearSyntenyRenderer extends ComparativeRendererType {
  ...
  async makeImageData(props: LinearSyntenyRenderProps) {
    const { highResolutionScaling = 1, views, layoutMatches } = props
      ...
      <g stroke="#333" fill="none">
        {layoutMatches.map(chunk => {
          ...
          for (let i = 0; i < chunk.length - 1; i += 1) {
            ...
            if (
              !v1.displayedRegions.find(region => region.refName === ref1) ||
              !v2.displayedRegions.find(region => region.refName === ref2)
            ) {
              //  eslint-disable-next-line no-continue
              continue
            }
