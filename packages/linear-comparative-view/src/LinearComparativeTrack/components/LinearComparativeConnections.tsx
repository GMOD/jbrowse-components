...
export default observer(
  ({
    ...
    model: LinearComparativeTrackModel
    view: LinearComparativeViewModel
    tracks: any[]
    height: number
    syntenyGroup: string
    layoutMatches: {
    ...
    const { views, showIntraviewLinks } = view
    const middle = getConf(model, 'middle') as boolean
    const hideTiny = getConf(model, 'hideTiny') as boolean
    if (!layoutMatches) {
      return null
    }
      ...
      >
        {layoutMatches.map(chunk => {
          ...
          for (let i = 0; i < chunk.length - 1; i += 1) {
            ...
            if (!view.refNames[level1].includes(ref1)) {
              //  eslint-disable-next-line no-continue
              continue
            }
            if (!view.refNames[level2].includes(ref2)) {
              //   eslint-disable-next-line no-continue
              continue
            }
