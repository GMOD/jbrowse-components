...
import { LayoutTreeSidebarMixin } from './LayoutTreeSidebarMixin.ts'
...
import type { LayoutTreeSidebarHost } from './LayoutTreeSidebarMixin.ts'
...
function makeModel() {
  return types
    .compose('TestTreeSidebar', LayoutTreeSidebarMixin<Src>(), types.model({}))
    .create({})
}
...
// A display whose row order is declared elsewhere passes no `rows` sentence
// and owes a `rowDomain` getter of its own. Reading the mixin's one is the
// failure this refuses to let pass as an empty order, which would silently drop
// the declared order and take adapter order instead.
...
// unused @ts-expect-error fails `pnpm typecheck`. It asks `LayoutTreeSidebarHost`
...
const treeSidebarPin: HostChecksSlotNames<LayoutTreeSidebarHost> = true

test('the host type checks the slot names the mixin reads through it', () => {
  expect(treeSidebarPin).toBe(true)
  const host = {} as LayoutTreeSidebarHost
  const read = () => {
    // @ts-expect-error
