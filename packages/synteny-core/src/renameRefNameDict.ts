 ...
 * is the rule; `getAdapterToCanonicalRefNameMap` builds the `map`, per axis.
 *
 ...
 * canonical name (a file spelling one contig `chr1` on some rows and `1` on
 * others, against an assembly aliasing both). Duplicates break every reader that
 * resolves a name to an id ONCE and then compares integers — `pickFollowFeature`
 * and `followWindowMapping` both do, via `dict.indexOf` — since `indexOf` finds
 * the first duplicate and every feature carrying the second silently stops
 * matching.
 *
...
export function renameRefNameDict({
  ...
  const remap = dict.map(name => interned.idFor(map[name] ?? name))
