...
`track` is offered once two tracks overlay, and `reference` only across a stack
of two or more levels, since below that it degenerates to query or target.
...
The colour object that paints by `field`, written over `current`: `''` keeps the
field and its order under `scale: 'none'` for a switch back, and a field keeps
its order only when it is the one already named.
