...
structural mode radios, the measurements one hop in, and the per-track swatches
once more than one track is overlaid.
...
time (after `colorableTracks` and the legend) and repeating the same setter
lambdas.
