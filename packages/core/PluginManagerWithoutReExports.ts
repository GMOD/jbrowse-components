...
type Base = PluginManagerWithoutReExports
...
export default class PluginManagerWithoutReExports {
  plugins: Plugin[] = []
  ...
  addElementType(
    groupName: PluggableElementTypeGroup,
    creationCallback: (pluginManager: Base) => PluggableElementType,
  ) {
  ...
  addRendererType(cb: (pm: Base) => RendererType) {
    return this.addElementType('renderer', cb)
  }

  addAdapterType(cb: (pm: Base) => AdapterType) {
    return this.addElementType('adapter', cb)
  }

  addTextSearchAdapterType(cb: (pm: Base) => TextSearchAdapterType) {
    return this.addElementType('text search adapter', cb)
  }

  addTrackType(cb: (pm: Base) => TrackType) {
    return this.addElementType('track', () => {
      const track = cb(this)
      for (const display of this.getDisplayElements()) {
  ...
  addDisplayType(cb: (pluginManager: Base) => DisplayType) {
    return this.addElementType('display', cb)
  }

  addViewType(cb: (pluginManager: Base) => ViewType) {
    return this.addElementType('view', () => {
  ...
  addWidgetType(cb: (pm: Base) => WidgetType) {
    return this.addElementType('widget', cb)
  }

  addConnectionType(cb: (pm: Base) => ConnectionType) {
    return this.addElementType('connection', cb)
  }

  addRpcMethod(cb: (pm: Base) => RpcMethodType) {
    return this.addElementType('rpc method', cb)
  }

  addInternetAccountType(cb: (pm: Base) => InternetAccountType) {
    return this.addElementType('internet account', cb)
  }

  addAddTrackWorkflowType(cb: (pm: Base) => AddTrackWorkflowType) {
