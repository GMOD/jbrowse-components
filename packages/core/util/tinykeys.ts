 ...
 * Vendored from https://github.com/jamiebuilds/tinykeys
 * MIT License - Copyright (c) Jamie Kyle
 */
...
export function parseKeybinding(str: string): KeyBindingPress[] {
  return str
    ...
    .map(press => {
      ...
      const normalizedMods = mods.map(mod => (mod === '$mod' ? MOD : mod))
      return [normalizedMods, key]
    })
}
