...
describe('function string parsing', () => {
  ...
  it('can use the loaded core helper functions to access feature info', () => {
    ...
    expect(
      stringToJexlExpression(`jexl:getFeatureData(feature, 'score')`).evalSync({
        feature,
      }),
    ).toEqual(10)
    expect(
      stringToJexlExpression(
        `jexl:getFeatureData(feature, 'uniqueId')`,
      ).evalSync({ feature }),
    ).toBe('jexlFeature')
    expect(
      stringToJexlExpression(
        `jexl:getFeatureData(feature, 'end') - getFeatureData(feature, 'start') == 8`,
      ).evalSync({ feature }),
    ).toBe(true)
  })
})
