...
A row facet: split the features on `field`'s value, run every layer's own steps
over each section alone, and stack the sections, each starting on the row after
...
the whole domain, so a key lists each one.
