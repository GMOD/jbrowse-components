...
- **Re-export shims** (`RenderLifecycleMixin.ts`, `renderBlock.ts`,
  `hal/index.ts`, …) — thin `export * from '@jbrowse/render-core/X'` files so
  the ~160 existing `@jbrowse/core/gpu/*` imports keep resolving. New code
  should import from `@jbrowse/render-core`. Don't add logic to a shim; if you
  need to change behavior, edit the source in render-core.
- `passes/` — shared "simple shape" GPU passes (arrow/chevron/line/rect) + their
  ...
  generated outputs the codegen writes here.
