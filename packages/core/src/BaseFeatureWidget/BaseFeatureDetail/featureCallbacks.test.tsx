...
// A `formatDetails.feature` callback's object is merged onto the feature the
// panel renders: new keys add fields, existing keys override the raw value, and
// a key set to null/undefined hides the field, since every detail component
// filters with `!= null`. Set session-wide here, which is the tier a widget
// without a track still reads.
function setup(feature: SimpleFeatureSerialized, callback?: unknown) {
  ...
  const model = Session.create(
    {
      configuration: {
        formatDetails: callback ? { feature: callback } : {},
      },
...
test('callback adds an extra field', async () => {
  const { findByText } = setup(base, { customField: 'hello world' })
...
test('callback overrides an existing field value', async () => {
  const { findByText, queryByText } = setup(base, {
    score: '37 (high confidence)',
  })
...
test('callback hides a field by setting it null', async () => {
  const { findByText, queryByText } = setup(base, { score: null })
...
test('callback hides a field by setting it undefined', async () => {
  const { findByText, queryByText } = setup(base, 'jexl:{score:undefined}')
  expect(await findByText('ctgA:3..102 (+)')).toBeTruthy()
  expect(queryByText('score')).toBeNull()
})
...
test('callback can hide the derived Length row', async () => {
  const { findByText, queryByText } = setup(base, { length: null })
  expect(await findByText('ctgA:3..102 (+)')).toBeTruthy()
...
test('a formatted value that is just a URL renders as a link', async () => {
  const { findByText } = setup(base, {
    homepage: 'https://example.com/gene',
  })
