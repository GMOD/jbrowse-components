 ...
 * support.
 *
 * A `signal` errors the subscriber with an abort error the moment it aborts,
 * rather than waiting for `func` to finish work whose result is already
 * discarded. The fetch paths filter abort errors out (`isAbortException`) so a
 * cancel does not surface as a track error. It cannot interrupt `func` itself
 * — a body doing long synchronous work still needs its own `checkAbortSignal`
 * ticks — but it does stop delivery downstream.
 *
