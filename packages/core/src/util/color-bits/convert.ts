...
const OKLAB_TO_LMS_MATRIX: [number, number, number][] = [
  [0.9999999984505198, 0.3963377921737679, 0.2158037580607588],
  [1.000000008881761, -0.10556134232365635, -0.06385417477170591],
  [1.00000005467241, -0.0894841820949658, -1.2914855378640917],
]

const LMS_TO_OKLAB_MATRIX: [number, number, number][] = [
...
const XYZ_TO_LMS_MATRIX: [number, number, number][] = [
  [0.8190224432164319, 0.3619062562801221, -0.12887378261216414],
  [0.0329836671980271, 0.9292868468965546, 0.03614466816999844],
  [0.048177199566046255, 0.26423952494422764, 0.6335478258136937],
]

const LMS_TO_XYZ_MATRIX: [number, number, number][] = [
...
const PRO_PHOTO_TO_XYZD50_MATRIX: [number, number, number][] = [
...
const XYZD50_TO_PRO_PHOTO_MATRIX: [number, number, number][] = [
...
const XYZD65_TO_XYZD50_MATRIX: [number, number, number][] = [
...
const XYZD50_TO_XYZD65_MATRIX: [number, number, number][] = [
...
const XYZD65_TO_SRGB_MATRIX: [number, number, number][] = [
...
export function oklabToXyzd65(
  ...
  lmsIntermediate[1] =
    lmsIntermediate[1] * lmsIntermediate[1] * lmsIntermediate[1]
  lmsIntermediate[2] =
    lmsIntermediate[2] * lmsIntermediate[2] * lmsIntermediate[2]
  return multiply(LMS_TO_XYZ_MATRIX, lmsIntermediate)
}

export function xyzd65ToOklab(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
...
export function displayP3ToXyzd50(
  ...
  const [mappedR, mappedG, mappedB] = applyTransferFns(
    NAMED_TRANSFER_FN.sRGB,
    r,
    g,
    b,
  )
  return multiply(NAMED_GAMUTS.displayP3!, [mappedR, mappedG, mappedB])
}

export function xyzd50ToDisplayP3(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  const rgbOutput = multiply(NAMED_GAMUTS.displayP3_INVERSE!, [x, y, z])
...
export function proPhotoToXyzd50(
  r: number,
  g: number,
  b: number,
): [number, number, number] {
  const [mappedR, mappedG, mappedB] = applyTransferFns(
    NAMED_TRANSFER_FN.proPhotoRGB,
    r,
    g,
    b,
  )
  return multiply(PRO_PHOTO_TO_XYZD50_MATRIX, [mappedR, mappedG, mappedB])
}

export function xyzd50ToProPhoto(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  const rgbOutput = multiply(XYZD50_TO_PRO_PHOTO_MATRIX, [x, y, z])
  return applyTransferFns(
...
export function adobeRGBToXyzd50(
  r: number,
  g: number,
  b: number,
): [number, number, number] {
  const [mappedR, mappedG, mappedB] = applyTransferFns(
    NAMED_TRANSFER_FN.k2Dot2,
    r,
    g,
    b,
  )
  return multiply(NAMED_GAMUTS.adobeRGB!, [mappedR, mappedG, mappedB])
}

export function xyzd50ToAdobeRGB(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  const rgbOutput = multiply(NAMED_GAMUTS.adobeRGB_INVERSE!, [x, y, z])
  return applyTransferFns(
...
export function rec2020ToXyzd50(
  r: number,
  g: number,
  b: number,
): [number, number, number] {
  ...
  return multiply(NAMED_GAMUTS.rec2020!, [mappedR, mappedG, mappedB])
}

export function xyzd50ToRec2020(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  const rgbOutput = multiply(NAMED_GAMUTS.rec2020_INVERSE!, [x, y, z])
  return applyTransferFns(
...
export function xyzd50ToD65(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  return multiply(XYZD50_TO_XYZD65_MATRIX, [x, y, z])
}

export function xyzd65ToD50(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  return multiply(XYZD65_TO_XYZD50_MATRIX, [x, y, z])
}

export function xyzd65TosRGBLinear(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  return multiply(XYZD65_TO_SRGB_MATRIX, [x, y, z])
}

export function xyzd50TosRGBLinear(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  return multiply(NAMED_GAMUTS.sRGB_INVERSE!, [x, y, z])
}

export function srgbLinearToXyzd50(
  r: number,
  g: number,
  b: number,
): [number, number, number] {
  return multiply(NAMED_GAMUTS.sRGB!, [r, g, b])
}

export function srgbToXyzd50(
  r: number,
  g: number,
  b: number,
): [number, number, number] {
  ...
  return multiply(NAMED_GAMUTS.sRGB!, [mappedR, mappedG, mappedB])
}

export function xyzd50ToSrgb(
  x: number,
  y: number,
  z: number,
): [number, number, number] {
  const rgbOutput = multiply(NAMED_GAMUTS.sRGB_INVERSE!, [x, y, z])
  return applyTransferFns(
    NAMED_TRANSFER_FN.sRGB_INVERSE,
    rgbOutput[0],
    rgbOutput[1],
