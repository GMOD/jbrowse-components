...
import { SNPCoverageTrackModel } from '../model'
...
const YScaleBar = observer(
  ({ model }: { model: Instance<SNPCoverageTrackModel> }) => {
...
function SNPCoverageTrackComponent(props: {
  model: Instance<SNPCoverageTrackModel>
}) {
  const { model } = props
  const { ready } = model
...
SNPCoverageTrackComponent.propTypes = {
  model: MobxPropTypes.observableObject.isRequired,
}

export default observer(SNPCoverageTrackComponent)
