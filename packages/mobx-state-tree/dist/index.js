...
var ObjectNode = class extends BaseNode {
  ...
  constructor(complexType, parent, subpath, environment, initialValue) {
    ...
    this._snapshotComputed = (0, import_mobx4.computed)(() => freeze(this.getSnapshot()));
  ...
  fireHook(name) {
    ...
    if (typeof fn === "function") {
      if (import_mobx4._allowStateChangesInsideComputed) {
        (0, import_mobx4._allowStateChangesInsideComputed)(() => {
  ...
  _addSnapshotReaction() {
    if (!this._hasSnapshotReaction) {
      const snapshotDisposer = (0, import_mobx4.reaction)(
        () => this.snapshot,
...
ObjectNode.prototype.createObservableInstance = (0, import_mobx4.action)(
...
function runMiddleWares(node, baseCall, originalFn) {
  ...
  if (middlewares.isEmpty)
    return (0, import_mobx7.action)(originalFn).apply(null, baseCall.args);
  let result = null;
...
var IdentifierCache = class _IdentifierCache {
  ...
  cache = import_mobx8.observable.map();
  ...
  lastCacheModificationPerId = import_mobx8.observable.map();
  constructor() {
  }
  ...
  addNodeToCache(node, lastCacheUpdate = true) {
    if (node.identifierAttribute) {
      ...
      if (!this.cache.has(identifier2)) {
        this.cache.set(
          identifier2,
          import_mobx8.observable.array([], mobxShallow)
        );
      }
  ...
  mergeCache(node) {
    (0, import_mobx8.values)(node.identifierCache.cache).forEach(
  ...
  splitCache(splitNode) {
    ...
    (0, import_mobx8.entries)(this.cache).forEach(([id, nodes]) => {
      let modified = false;
...
var mobxShallow = (0, import_mobx9._getGlobalState)().useProxies ? { deep: false } : { deep: false, proxy: false };
...
var MSTMap = class extends import_mobx10.ObservableMap {
  constructor(initialData, name) {
    super(initialData, import_mobx10.observable.ref.enhancer, name);
  }
  get(key) {
...
var ArrayType = class _ArrayType extends ComplexType2 {
  ...
  createNewInstance(childNodes) {
    ...
    return import_mobx11.observable.array(
...
var ModelType = class _ModelType extends ComplexType2 {
  ...
  instantiateVolatileState(self, state) {
    ...
    Object.getOwnPropertyNames(state).forEach((name) => {
      ...
      (0, import_mobx12.set)(self, name, state[name]);
    });
  }
  extend(fn) {
  ...
  instantiateViews(self, views) {
    ...
    Object.getOwnPropertyNames(views).forEach((name) => {
      ...
      if ("get" in descriptor) {
        (0, import_mobx12.defineProperty)(self, name, descriptor);
        (0, import_mobx12.makeObservable)(self, { [name]: import_mobx12.computed });
  ...
  getChildNode(node, key) {
    ...
    const adm = (0, import_mobx12._getAdministration)(node.storedValue, key);
  ...
  getSnapshot(node, applyPostProcess = true) {
    ...
    this.forAllProps((name, type) => {
      const atom = (0, import_mobx12.getAtom)(node.storedValue, name);
      atom.reportObserved();
...
var Lazy = class extends SimpleType2 {
  ...
  flags = 1048576 /* Lazy */;
  loadedType = null;
  pendingNodeList = import_mobx13.observable.array();
  describe() {
