...
describe('migrateSessionSnapshot', () => {
  ...
  test("lifts a v4.3.0 view's dismissed highlights onto the session flag", () => {
    const snap = {
      name: 'test',
      views: [{ type: 'LinearGenomeView', id: 'v1', highlightsVisible: false }],
    }
    const result = migrateSessionSnapshot(snap)
    expect(result.highlightsVisible).toBe(false)
    expect(result.views).toEqual([{ type: 'LinearGenomeView', id: 'v1' }])
  })

  test("lifts grid-bookmark's per-view bookmarkHighlightsVisible too", () => {
    const snap = {
      views: [
        {
          type: 'LinearGenomeView',
  ...
  test('one dismissed view among several turns the session flag off', () => {
    const snap = {
      views: [
        { type: 'LinearGenomeView', id: 'v1' },
        { type: 'LinearGenomeView', id: 'v2', highlightsVisible: false },
      ],
    }
    expect(migrateSessionSnapshot(snap).highlightsVisible).toBe(false)
  })

  test('lifts a dismissal off a synteny sub-view', () => {
    const snap = {
      views: [
        {
          type: 'LinearSyntenyView',
    ...
    const result = migrateSessionSnapshot(snap)
    expect(result.highlightsVisible).toBe(false)
    expect((result.views as any)[0].views[1]).toEqual({
      type: 'LinearGenomeView',
      id: 'v2',
    })
  })

  test('a session that already carries highlightsVisible wins', () => {
    const snap = {
      highlightsVisible: true,
      views: [{ type: 'LinearGenomeView', id: 'v1', highlightsVisible: false }],
    }
    const result = migrateSessionSnapshot(snap)
    expect(result.highlightsVisible).toBe(true)
  })

  test('a view that never dismissed the band leaves the flag alone', () => {
    const snap = {
      views: [{ type: 'LinearGenomeView', id: 'v1', highlightsVisible: true }],
    }
    const result = migrateSessionSnapshot(snap)
    expect('highlightsVisible' in result).toBe(false)
    expect(result.views).toEqual([{ type: 'LinearGenomeView', id: 'v1' }])
  })

  test('the highlight lift is idempotent', () => {
    const snap = {
      views: [{ type: 'LinearGenomeView', id: 'v1', highlightsVisible: false }],
    }
    const once = migrateSessionSnapshot(snap)
    expect(migrateSessionSnapshot(once)).toBe(once)
  })
})
