...
families hold their `discoveredRows` through it.
...
folding content staleness into `displayPhase` was declined.
`computeActivityPhase` reads every other term off the model. Routing it through
a display-local getter means re-remembering the cancel term — one edit from the
dead-Retry bug.
