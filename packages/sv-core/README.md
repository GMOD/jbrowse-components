...
`windowBp` either side of each breakpoint. Each panel is turned so the sequence
its end keeps reads left to right into the join: an end keeping the sequence to
its right is reversed on the left panel, one keeping its left is reversed on the
right. Two ends of one contig closer than a window collapse to the single span
between them. `undefined` for a record with one end.
...
Whether the panel showing this end has to be turned for the join to read left to
...
spreadsheet row menu's loc string, and the single-level breakpoint split view's
displayed regions.
