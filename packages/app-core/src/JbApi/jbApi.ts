...
function describeSchema(type: mst.IAnyType): SlotDescription | undefined {
  ...
  if (mst.isArrayType(bare)) {
    const items = describeSchema(
      (bare as unknown as { getChildType(): mst.IAnyType }).getChildType(),
    )
    return items ? { type: 'array', items } : undefined
  }
  if (mst.isUnionType(bare)) {
    const members = [bare.getSubTypes()]
      .flat()
      .flatMap(sub =>
        sub && typeof sub === 'object' ? [describeSchema(sub)] : [],
      )
      .filter(d => d !== undefined)
    return members.length > 0
      ? {
          type: 'union',
          oneOf: Object.fromEntries(members.map(m => [m.type, m])),
        }
      : undefined
  }
  return undefined
...
// Vocabulary introspection: every config slot a config node's schema defines,
// read off the schema rather than the live node, so code never has to guess
// which settings keys exist — an unknown key is not an error, only an entry in
// applyDisplaySettings' `unapplied` list. A nested object (`facet`, `color`)
// lists its own slots and the slot its string shorthand lifts into, and a list
// of them (`marks`, `transform`) its entry's.
function describeSlots(
  confOrType: AnyConfigurationModel | mst.IAnyType,
): Record<string, SlotDescription> {
  const definition = getConfigurationSchemaDefinition(confOrType) ?? {}
  return Object.fromEntries(
    Object.entries(definition).flatMap(
      ([name, def]): [string, SlotDescription][] => {
        ...
        const described = mst.isType(def) ? describeSchema(def) : undefined
        return described ? [[name, described]] : []
      },
    ),
  )
}
