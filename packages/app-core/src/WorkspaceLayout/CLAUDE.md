...
The workspace is **one MST tree**. There is no window manager library, no
imperative api, and nothing to reconcile: the layout is session state, React
renders it, and a gesture is an action.

[ADR-068](../../../../agent-docs/architecture-decision-records/adr-068-workspace-layout-is-an-mst-tree.md)
is the decision. It holds what this replaced, what the replacement cost, why the
chrome is still dockview's, and the four integrity bugs found after it worked —
read it before reasoning about an old comment or commit that names dockview, and
before re-deriving anything below that cites it.
...
`nextId` uses `createElementId()` (nanoid), never a module-level counter — panel
and tab ids are `types.identifier` and must be unique **within the tree**, which
includes the ids in a snapshot restored from a previous page load. The obvious
test passes against the broken version; `integrity.test.ts` is the one that
doesn't, and ADR-068 says why.
...
lifecycle. Every operation is `tree in -> tree out`, and every operation has to
leave the tree canonical: four rules, bottom-up — drop empty branches, replace a
single-child branch with its child, flatten a branch into a same-direction
parent, renormalise sizes to sum to 1. ADR-068 covers why that purity is what
makes the 2000-step randomised sequence in `integrity.test.ts` possible; what
follows is the assertion in it that is not obvious.
...
The one place sizes are computed is `splitter.ts`, which moves the boundary
_within the combined space of the pair either side of it_ so every other pane
holds still — pure, and tested without a DOM, the same split as `dropZone.ts`.
...
`stripDropAt` answers by midpoint, which is what makes it total — every x
belongs to exactly one gap, so there is no dead band where a drop degrades into
appending.
