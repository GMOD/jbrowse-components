export default ({ jbrequire }) => {
  ...
  // work with autorun
  function fetchBlockReactionData(self) {
  ...
  async function fetchBlockReactionEffect(self, props, allowRefetch = true) {
    const {
      trackError,
      rendererType,
      renderProps,
    ...
    try {
      ...
      if (isAbortException(error) && !aborter.signal.aborted) {
        ...
        if (allowRefetch) {
          ...
          fetchBlockReactionEffect(self, props, false)
          return
        }
    ...
    .actions(self => ({
      afterAttach() {
        ...
        const renderDisposer = reaction(
          () => fetchBlockReactionData(self),
          data => fetchBlockReactionEffect(self, data),
          {
