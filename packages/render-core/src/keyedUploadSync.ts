 ...
 * owning one keyed geometry slot — the dotplot view's canvas (keyed by track
 * index) and the synteny level's canvas (keyed by `displayKey`).
 *
 ...
 *     upload: b => { syncUpload(b, self.geometryByTrackIndex) },
 *     render: …,
 *   })
 * }
 * ```
