...
JBROWSE_DESKTOP_TAG=$(git tag --sort=-creatordate -l "@gmod/jbrowse-desktop*"|head -n1)
DATE=$DATE NOTES=$NOTES perl -p -i -e 's/\$\{([^}]+)\}/defined $ENV{$1} ? $ENV{$1} : $&/eg' < build/blog_template.txt > $BLOGPOST_FILENAME
