...
// ---------------------------------------------------------------------------
const newest = readdirSync(BLOG_DIR)
...
const raw = readFileSync(path.join(BLOG_DIR, newest), 'utf8')
const fmMatch = /^---\n([\s\S]*?)\n---\n([\s\S]*)$/.exec(raw)
if (!fmMatch) {
  throw new Error(`no frontmatter in ${newest}`)
}
const [, frontmatter, body] = fmMatch
const titleMatch = /^title:\s*(.+)$/m.exec(frontmatter)
const title = titleMatch ? titleMatch[1].trim() : newest

// Filename is YYYY-MM-DD-<slug>.md; tag is the vX.Y.Z inside the slug.
const idMatch = /^(\d{4})-(\d{2})-(\d{2})-(.+)\.md$/.exec(newest)
if (!idMatch) {
