...
export function emit(inputs: CodegenInputs) {
  ...
  const lines: string[] = []
  lines.push(`// AUTO-GENERATED by scripts/shader-codegen from ${baseName}.slang.`)
  lines.push('// Do not edit. Run `pnpm gen:shaders` to regenerate.')
  lines.push('')
  ...
  if (cb) {
    ...
    lines.push(`export const UNIFORMS_SIZE_F32 = ${totalBytes / 4}`)
    lines.push('')

    lines.push('// Byte offsets (into an ArrayBuffer / DataView).')
    lines.push('export const UNIFORM_OFFSET_BYTES = {')
    ...
    lines.push('// Indices into a Float32Array / Uint32Array view.')
    lines.push('export const UNIFORM_OFFSET_F32 = {')
    for (const f of u.fields) {
      if (f.binding?.kind === 'uniform') {
  ...
  if (vs) {
    ...
    lines.push(`export const INSTANCE_STRIDE_BYTES = ${stride}`)
    lines.push(`export const INSTANCE_STRIDE_F32 = ${stride / 4}`)
    lines.push('')

    lines.push('export const FIELD_OFFSET_BYTES = {')
    ...
    lines.push('export const FIELD_OFFSET_F32 = {')
    for (const a of attrs) {
    ...
    lines.push(`export const GL_ATTRIBUTES: readonly GlAttributeLayout[] = [`)
    for (const a of attrs) {
    ...
    lines.push(`  const base = instanceIndex * ${stride}`)
    lines.push(`  const dv = new DataView(buf)`)
    for (const a of attrs) {
  ...
  if (textures && textures.length > 0) {
    lines.push('// Combined `Sampler2D` bindings. Texture unit indices start at 0.')
    lines.push('export const TEXTURES: readonly TextureBinding[] = [')
