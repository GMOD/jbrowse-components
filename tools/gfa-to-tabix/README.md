...
Converts GFA pangenome graphs into tabix-indexed files for JBrowse's `GfaTabixAdapter`.
...
Reads a `vg deconstruct` VCF and generates per-snarl CS strings for the
synteny view's per-base mismatch/indel detail.
...
4-byte little-endian u32 per ordinal, contiguous from ordinal 0. To look up
the length of ordinal `k`, read bytes `[k*4, k*4+4]`.
