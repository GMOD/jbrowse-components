...
export type { ExtractRequest, ExtractResult, TruthBackend } from './backends/types.ts'
export { canonicalize, canonicalizeParsed, summarizeDiff } from './canonicalize.ts'
...
export async function extractTruthSubgraph(
  ...
  switch (req.backend) {
    ...
    default:
      throw new Error(`Unknown backend: ${(req as { backend: string }).backend}`)
  }
}
