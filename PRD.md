...
- After fatal error: `Uncaught Error: no containing view found` in
  `getContainingView`
- ~~Infinite loop after error in multi-wiggle~~ **FIXED** — Added try/catch + `isAlive` guard to `VisibleScoreRange` autorun in multi-wiggle `afterAttach`; prevents `getContainingView` crash from causing infinite autorun re-trigger
...
- Color by reference CpG not working (should show blue over orange for
  hypomethylated)
