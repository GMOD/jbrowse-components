  ...
  return types
    ...
    .actions(self => ({
      async connect() {
        ...
        const hubData: UnifiedHubData | undefined = self.fetching.hubData
        if (!hubData) {
          throw new Error(
            'hubData not found, cannot connect. was it actually fetched?',
          )
        }
        const genomesFiles = hubData.genomes
