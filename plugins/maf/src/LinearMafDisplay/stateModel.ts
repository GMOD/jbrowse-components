...
export default function stateModelFactory(
  ...
  return (
    types
      ...
      // banner becomes a pure function of the cached estimate scaled to the current
      // viewport (self-releases on zoom-in, no flicker on pan). The mixin's
      // pre-flight (getByteEstimateConfig) still captures the estimate and
      // short-circuits the download server-side; afterAttach clears the estimate on
      // chromosome nav. Byte-only — no density axis.
      .views(self => ({
        /**
         * #getter
         */
