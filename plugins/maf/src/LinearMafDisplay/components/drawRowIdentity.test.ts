...
test('matches vs mismatches against the reference', () => {
  ...
  accumulateRowIdentity(match, cls, bytes('ACGT'), bytes('ACGA'), 0, identityMapper, 4)
...
test('sample gaps are excluded from the denominator', () => {
  const match = new Float32Array(4)
  const cls = new Float32Array(4)
  accumulateRowIdentity(match, cls, bytes('ACGT'), bytes('A-GT'), 0, identityMapper, 4)
...
test('reference insertion columns (ref dash) consume no ref position', () => {
  ...
  // are A(0) C(1) G(2).
  accumulateRowIdentity(match, cls, bytes('A-CG'), bytes('ATCG'), 0, identityMapper, 3)
...
test('reference N columns are unclassifiable', () => {
  ...
  accumulateRowIdentity(match, cls, bytes('NA'), bytes('CA'), 0, identityMapper, 2)
...
test('comparison is case-insensitive (soft-masking ignored)', () => {
  const match = new Float32Array(2)
  const cls = new Float32Array(2)
  accumulateRowIdentity(match, cls, bytes('AC'), bytes('ac'), 0, identityMapper, 2)
...
test('zoomed out: several bases average into one pixel', () => {
  ...
  accumulateRowIdentity(match, cls, bytes('ACGT'), bytes('ATGT'), 0, bp => bp / 4, 1)
  expect(cls[0]).toBe(4)
