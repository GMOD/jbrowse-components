...
test('zoomed out: many bases average into one pixel', () => {
  ...
  accumulateConservation(sum, count, new Float32Array([1, 0, 1, 0]), 0, bp => bp / 4, 1)
...
test('zoomed in: one base fills every pixel of its span', () => {
  ...
  accumulateConservation(sum, count, new Float32Array([1]), 0, bp => bp * 10, 10)
  for (let x = 0; x < 10; x++) {
...
test('NaN positions are skipped', () => {
  ...
  accumulateConservation(sum, count, new Float32Array([Number.NaN, 1]), 0, bp => bp, 2)
...
test('pixels outside [0, width) are clamped away', () => {
  ...
  accumulateConservation(sum, count, new Float32Array(10).fill(1), 0, bp => bp - 5, 3)
