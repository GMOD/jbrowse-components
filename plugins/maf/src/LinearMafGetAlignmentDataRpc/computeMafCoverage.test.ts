...
// per-row identity = matchesPerRow[i] / classifiablePerRow[i]
test('per-row identity counts each species against the reference, ref excluded', () => {
  const blocks: MafBlock[] = [
    block(0, 'ACGT', [row(0, 'ACGT'), row(1, 'ACGT'), row(2, 'ATGT')]),
  ]
  const r = computeMafCoverage(blocks, 0, 4, 0, 3)
  // reference row (0) is excluded → its counters stay 0.
  expect(r.classifiablePerRow[0]).toBe(0)
  expect(r.matchesPerRow[0]).toBe(0)
  // row 1 matches the reference at all 4 columns.
  expect(r.matchesPerRow[1]).toBe(4)
  expect(r.classifiablePerRow[1]).toBe(4)
  // row 2 differs at one column (T vs C).
  expect(r.matchesPerRow[2]).toBe(3)
  expect(r.classifiablePerRow[2]).toBe(4)
})

test('per-row identity excludes sample gaps and counts sample N as a mismatch', () => {
  const blocks: MafBlock[] = [
    block(0, 'ACGT', [row(0, 'ACGT'), row(1, 'A-NT')]),
  ]
  const r = computeMafCoverage(blocks, 0, 4, 0, 2)
  // row 1: pos0 A matches, pos1 gap (excluded), pos2 N mismatch, pos3 T matches
  // → 2 matches of 3 classifiable.
  expect(r.matchesPerRow[1]).toBe(2)
  expect(r.classifiablePerRow[1]).toBe(3)
})

test('per-row arrays are sized to nRows (rows absent in this region stay 0)', () => {
  const blocks: MafBlock[] = [block(0, 'AC', [row(0, 'AC'), row(1, 'AC')])]
  // 4 display rows, but only rows 0-1 present here.
  const r = computeMafCoverage(blocks, 0, 2, 0, 4)
  expect(r.matchesPerRow).toHaveLength(4)
  expect(r.classifiablePerRow[2]).toBe(0)
  expect(r.classifiablePerRow[3]).toBe(0)
})
