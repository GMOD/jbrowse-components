...
// Flat list (no submenus) sorted so each family is contiguous. A short family
// prefix on every row, rendered in a fixed-width muted column, makes the groups
// scannable and the active mode identifiable at a glance.
const COLOR_MODES = [
  {
    group: 'Basic',
    label: 'Default',
...
const ColorBySelector = observer(function ColorBySelector({
  ...
  return (
    <CascadingMenuButton
      tooltip={
        active ? `Color by: ${active.group} — ${active.label}` : 'Color by'
      }
      menuItems={[
        ...modes.map(({ group, label, value, helpText }) => ({
          label: (
            <span
              style={{
                display: 'inline-flex',
            ...
            >
              <span style={{ display: 'inline-flex', gap: 8 }}>
                <span style={{ opacity: 0.55, minWidth: 100 }}>{group}</span>
                <span>{label}</span>
              </span>
