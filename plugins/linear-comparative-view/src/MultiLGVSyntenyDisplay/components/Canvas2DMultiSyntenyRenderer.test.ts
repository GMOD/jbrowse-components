import { DEFAULT_CIGAR_OP_DRAW_COLORS } from '@jbrowse/alignments-core'
import { SvgCanvas } from '@jbrowse/core/util/SvgCanvas'

import { drawSyntenyToCtx } from './Canvas2DMultiSyntenyRenderer.ts'
import { buildRegionData, feat } from './syntenyTestHelpers.ts'
...
describe('drawSyntenyToCtx multi-region coverage', () => {
  test('renders coverage for multiple regions', () => {
    const region1 = buildRegionData({ refName: 'chr1', start: 0, end: 1000 }, [
      feat({ origRefName: 'chr1', start: 100, end: 500 }),
    ])
    const region2 = buildRegionData({ refName: 'chr2', start: 0, end: 1000 }, [
  ...
  test('renders SNP colored segments on coverage', () => {
    const region = buildRegionData({ refName: 'chr1', start: 0, end: 200 }, [
