...
export async function renderToSvg(
  ...
  for (let i = 1; i < views.length; i++) {
    RenderList.push(
      <g key={views[i].id} transform={`translate(0 ${currOffset})`}>
        <defs>
          <clipPath id={`synclip-${i}`}>
            <rect x={0} y={0} width={width} height={levels[i - 1].height} />
          </clipPath>
        </defs>
        <g
        ...
        >
          {renderings[i - 1].map((r, i) => (
            <ReactRendering key={i} rendering={r} />
          ))}
        </g>
        <g transform={`translate(0 ${levels[i - 1].height})`}>
          <SVGLinearGenomeView
            rulerHeight={rulerHeight}
            shift={shift}
            trackLabelOffset={trackLabelOffset}
    ...
    currOffset += heights[i] + levels[i - 1].height + fontSize + rulerHeight
  }
