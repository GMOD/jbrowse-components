...
function LinearSyntenyRendering({
  ...
  useEffect(
    () => {
      ...
      const featPos = Object.values(featPositions)
      ...
      for (let i = 0; i < featPos.length; i++) {
        ...
        const y1 = 0 //interstitialYPos(l1 < l2, height)
        const y2 = height //interstitialYPos(l2 < l1, height)

        const mid = (y2 - y1) / 2

        // drawing a line if the results are thin results in much less
        // pixellation than filling in a thin polygon
        if (x21 < view.width + 800 && x21 > -800) {
          ctx1.moveTo(x11, y1)
