// Hand-written GLSL ES 3.0 shaders for WebGL2 fallback. Mirrors wgslShaders.ts.
// Line rendering: https://mattdesl.svbtle.com/drawing-lines-is-hard

import { EDGE_SEGMENTS, FILL_SEGMENTS } from './wgslShaders.ts'

const UNIFORMS = /* glsl */ `layout(std140) uniform Uniforms {
  vec2 resolution;
  float height;
  float adjOff0;
  float adjOff1;
  float scale0;
  float scale1;
  float maxOffScreenPx;
  float minAlignmentLength;
  float alpha;
  float hoveredFeatureId;
  float clickedFeatureId;
  float yTop;
  float _pad0;
  float _pad1;
  float _pad2;
} u;`

const SEGMENT_CONSTS = /* glsl */ `
const uint FILL_SEGS = ${FILL_SEGMENTS}u;
const uint EDGE_SEGS = ${EDGE_SEGMENTS}u;`

// Shared helpers: screen-space corner positions + off-canvas cull.
const HELPERS = /* glsl */ `
struct Corners { float x1; float x2; float x3; float x4; };

Corners computeCorners(float x1, float x2, float x3, float x4, float padTop, float padBottom) {
  Corners c;
  c.x1 = (x1 - u.adjOff0) * u.scale0 - padTop * (u.scale0 - 1.0);
  c.x2 = (x2 - u.adjOff0) * u.scale0 - padTop * (u.scale0 - 1.0);
  c.x3 = (x3 - u.adjOff1) * u.scale1 - padBottom * (u.scale1 - 1.0);
  c.x4 = (x4 - u.adjOff1) * u.scale1 - padBottom * (u.scale1 - 1.0);
  return c;
}

bool isCulled(Corners c, float queryTotalLength) {
  if (u.minAlignmentLength > 0.0 && queryTotalLength < u.minAlignmentLength) {
    return true;
  }
  float mOff = u.maxOffScreenPx;
  float rW = u.resolution.x;
  return max(c.x1, c.x2) < -mOff || min(c.x1, c.x2) > rW + mOff
      || max(c.x3, c.x4) < -mOff || min(c.x3, c.x4) > rW + mOff;
}`

export const FILL_VERTEX_SHADER = `#version 300 es
precision highp float;
precision highp int;

in vec4 a_inst0;
in uint a_inst1;
in vec4 a_inst2;
in vec4 a_inst3;

${UNIFORMS}
${SEGMENT_CONSTS}
${HELPERS}

smooth out vec4 v_color;
flat out float v_featureId;

vec3 hermiteEdges(float sX1, float sX2, float sX3, float sX4, float t, float isCurve) {
  float edge0, edge1, y;
  if (isCurve > 0.5) {
    float s = t * t * (3.0 - 2.0 * t);
    edge0 = mix(sX1, sX4, s);
    edge1 = mix(sX2, sX3, s);
    y = u.yTop + u.height * (1.5 * t * (1.0 - t) + t * t * t);
  } else {
    edge0 = mix(sX1, sX4, t);
    edge1 = mix(sX2, sX3, t);
    y = u.yTop + t * u.height;
  }
  return vec3(edge0, edge1, y);
}

void main() {
  float x1 = a_inst0.x, x2 = a_inst0.y, x3 = a_inst0.z, x4 = a_inst0.w;
  vec4 color = vec4(
    float(a_inst1 & 0xFFu),
    float((a_inst1 >> 8u) & 0xFFu),
    float((a_inst1 >> 16u) & 0xFFu),
    float(a_inst1 >> 24u)
  ) / 255.0;
  float featureId = a_inst2.x, isCurve = a_inst2.y;
  float queryTotalLength = a_inst2.z, padTop = a_inst2.w, padBottom = a_inst3.x;

  v_color = color;
  v_featureId = featureId;

  Corners c = computeCorners(x1, x2, x3, x4, padTop, padBottom);
  if (isCulled(c, queryTotalLength)) {
    gl_Position = vec4(0.0);
    return;
  }

  uint vid = uint(gl_VertexID);
  uint seg = vid / 6u;
  uint vertInSeg = vid % 6u;
  float topDiff = c.x1 - c.x2;
  float botDiff = c.x4 - c.x3;

  float t0, t1;
  if (topDiff * botDiff < 0.0) {
    float tCross = clamp(topDiff / (topDiff - botDiff), 0.01, 0.99);
    uint nUpper = clamp(uint(round(float(FILL_SEGS) * tCross)), 1u, FILL_SEGS - 1u);
    if (seg < nUpper) {
      t0 = float(seg) / float(nUpper) * tCross;
      t1 = float(seg + 1u) / float(nUpper) * tCross;
    } else {
      uint lSeg = seg - nUpper;
      uint nLower = FILL_SEGS - nUpper;
      t0 = tCross + float(lSeg) / float(nLower) * (1.0 - tCross);
      t1 = tCross + float(lSeg + 1u) / float(nLower) * (1.0 - tCross);
    }
  } else {
    t0 = float(seg) / float(FILL_SEGS);
    t1 = float(seg + 1u) / float(FILL_SEGS);
  }

  float t, side;
  switch (vertInSeg) {
    case 0u: t = t0; side = 0.0; break;
    case 1u: t = t0; side = 1.0; break;
    case 2u: t = t1; side = 0.0; break;
    case 3u: t = t1; side = 0.0; break;
    case 4u: t = t0; side = 1.0; break;
    case 5u: t = t1; side = 1.0; break;
    default: t = 0.0; side = 0.0; break;
  }

  vec3 e = hermiteEdges(c.x1, c.x2, c.x3, c.x4, t, isCurve);
  float edgeX = side > 0.5 ? e.y : e.x;
  vec2 clipSpace = vec2(edgeX, e.z) / u.resolution * 2.0 - 1.0;
  gl_Position = vec4(clipSpace.x, -clipSpace.y, 0.0, 1.0);
}
`

export const FILL_FRAGMENT_SHADER = `#version 300 es
precision highp float;
precision highp int;

${UNIFORMS}

smooth in vec4 v_color;
flat in float v_featureId;
out vec4 fragColor;

void main() {
  vec3 rgb = v_color.rgb;
  bool isHovered = u.hoveredFeatureId > 0.0 && abs(v_featureId - u.hoveredFeatureId) < 0.5;
  float baseAlpha = v_color.a * u.alpha;
  float finalAlpha = isHovered ? min(baseAlpha * 5.0, 0.35) : baseAlpha;
  if (isHovered) {
    rgb *= 0.7;
  }
  fragColor = vec4(rgb, finalAlpha);
}
`

export const FILL_FRAGMENT_SHADER_PICKING = `#version 300 es
precision highp float;

smooth in vec4 v_color;
flat in float v_featureId;
out vec4 fragColor;

void main() {
  uint id = uint(v_featureId);
  fragColor = vec4(
    float(id & 0xFFu) / 255.0,
    float((id >> 8u) & 0xFFu) / 255.0,
    float((id >> 16u) & 0xFFu) / 255.0,
    1.0
  );
}
`

export const EDGE_VERTEX_SHADER = `#version 300 es
precision highp float;
precision highp int;

in vec4 a_inst0;
in uint a_inst1;
in vec4 a_inst2;
in vec4 a_inst3;

${UNIFORMS}
${SEGMENT_CONSTS}
${HELPERS}

smooth out float v_dist;

const float STROKE_WIDTH = 1.0;

void main() {
  float x1 = a_inst0.x, x2 = a_inst0.y, x3 = a_inst0.z, x4 = a_inst0.w;
  float featureId = a_inst2.x, isCurve = a_inst2.y;
  float queryTotalLength = a_inst2.z, padTop = a_inst2.w, padBottom = a_inst3.x;

  v_dist = 0.0;

  bool isClicked = u.clickedFeatureId > 0.0 && abs(featureId - u.clickedFeatureId) < 0.5;
  if (!isClicked) {
    gl_Position = vec4(0.0);
    return;
  }

  Corners c = computeCorners(x1, x2, x3, x4, padTop, padBottom);
  if (isCulled(c, queryTotalLength)) {
    gl_Position = vec4(0.0);
    return;
  }

  uint vid = uint(gl_VertexID);
  uint vertsPerEdge = EDGE_SEGS * 6u;
  uint edgeIdx = vid / vertsPerEdge;
  uint vidInEdge = vid % vertsPerEdge;
  uint seg = vidInEdge / 6u;
  uint vertInSeg = vidInEdge % 6u;
  float t0 = float(seg) / float(EDGE_SEGS);
  float t1 = float(seg + 1u) / float(EDGE_SEGS);

  float t, side;
  switch (vertInSeg) {
    case 0u: t = t0; side = -1.0; break;
    case 1u: t = t0; side =  1.0; break;
    case 2u: t = t1; side = -1.0; break;
    case 3u: t = t1; side = -1.0; break;
    case 4u: t = t0; side =  1.0; break;
    case 5u: t = t1; side =  1.0; break;
    default: t = 0.0; side = 0.0; break;
  }

  float edge0_x, edge1_x, y;
  vec2 tangent;

  if (isCurve > 0.5) {
    float s = t * t * (3.0 - 2.0 * t);
    edge0_x = mix(c.x1, c.x4, s);
    edge1_x = mix(c.x2, c.x3, s);
    y = u.yTop + u.height * (1.5 * t * (1.0 - t) + t * t * t);
    float sPrime = 6.0 * t * (1.0 - t);
    float dy = u.height * 1.5 * (1.0 - 2.0 * t * (1.0 - t));
    float dx = edgeIdx == 1u ? sPrime * (c.x3 - c.x2) : sPrime * (c.x4 - c.x1);
    tangent = vec2(dx, dy);
  } else {
    edge0_x = mix(c.x1, c.x4, t);
    edge1_x = mix(c.x2, c.x3, t);
    y = u.yTop + t * u.height;
    float dx = edgeIdx == 1u ? c.x3 - c.x2 : c.x4 - c.x1;
    tangent = vec2(dx, u.height);
  }

  float edgeX = edgeIdx == 1u ? edge1_x : edge0_x;
  float tangentLen = length(tangent);
  vec2 normal;
  if (tangentLen > 0.001) {
    vec2 rawNormal = vec2(-tangent.y, tangent.x) / tangentLen;
    float outwardSign = (edgeIdx == 0u ? -1.0 : 1.0) * sign(c.x1 - c.x2);
    normal = rawNormal * outwardSign;
  } else {
    normal = vec2(0.0, 1.0);
  }

  vec2 pos = vec2(edgeX, y) + normal * side * STROKE_WIDTH;
  v_dist = side * STROKE_WIDTH;
  vec2 clipSpace = pos / u.resolution * 2.0 - 1.0;
  gl_Position = vec4(clipSpace.x, -clipSpace.y, 0.0, 1.0);
}
`

export const EDGE_FRAGMENT_SHADER = `#version 300 es
precision highp float;

smooth in float v_dist;
out vec4 fragColor;

const float STROKE_WIDTH = 1.0;

void main() {
  float d = abs(v_dist);
  float aa = fwidth(v_dist);
  float halfW = STROKE_WIDTH * 0.5;
  float edgeAlpha = 1.0 - smoothstep(halfW - aa * 0.5, halfW + aa, d);
  fragColor = vec4(0.0, 0.0, 0.0, edgeAlpha * 0.4);
}
`
