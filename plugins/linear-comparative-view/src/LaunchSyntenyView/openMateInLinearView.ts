 ...
 *
 * The locus is the mate side of `region` — the clicked block, or the anchor
 * panel's visible window on the feature's contig, or without one the whole
 * alignment — resolved the way the launch resolves it: a CIGAR walk, or
 * interpolation across a block that carries none, padded by the launch's
 * default window. One view per launching view and mate assembly, opened with
 * the session's annotation for the mate — the same view the MAF display's
 * per-sample items and a multi-way lane give.
 */
