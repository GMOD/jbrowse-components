...
export function executeDotplotWebGLGeometry({
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    const p11 = bpToPx({ self: hViewSnap, refName: f.refName, coord: f1s })
    const p12 = bpToPx({ self: hViewSnap, refName: f.refName, coord: f1e })
    const p21 = bpToPx({ self: vViewSnap, refName: f.mateRefName, coord: f2s })
    const p22 = bpToPx({ self: vViewSnap, refName: f.mateRefName, coord: f2e })
