...
test('a session with no assemblies cannot launch, and says why', async () => {
  // launching blank axes left the view on a spinner that never ended
  await setup({ assemblyNames: [] })
  expect(launchButton()).toBeDisabled()
  expect(
...
test('an assembly added after mount seeds the axes', async () => {
  const { session } = await setup({ assemblyNames: [] })
  act(() => {
    session.addAssembly({
      name: 'late',
      sequence: {
...
test('Swap axes transposes the pair and keeps its track selection', async () => {
  const { model } = await setup({
    tracks: [syntenyTrack('hg38_mm39', ['hg38', 'mm39'])],
  })
  goManual()
...
test('picking another Quick start track starts from its own axis order', async () => {
  await setup({
    assemblyNames: ['hg38', 'mm39', 'rn7'],
    tracks: [
      syntenyTrack('hg38_mm39', ['hg38', 'mm39']),
