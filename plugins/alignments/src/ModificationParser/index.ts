...
// fairly hot function so optimized for speed
export function getModProbabilities(feature: Feature) {
  let ML = (getTagAlt(feature, 'ML', 'Ml') as number[] | string) || []
  if (ML) {
    if (typeof ML === 'string') {
      ML = ML.split(',').map(e => +e)
    }
    const ret = new Array(ML.length)
    for (let i = 0; i < ML.length; i++) {
      ret[i] = ML[i]! / 255
    }
    return ret
  } else {
    return (getTagAlt(feature, 'MP', 'Mp') as string | undefined)
      ?.split('')
