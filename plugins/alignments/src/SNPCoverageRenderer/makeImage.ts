...
export function makeImage(
  ...
  if (soa) {
    ...
    for (let i = 0; i < bins.length; i++) {
      ...
      if (bin) {
        ...
        if (Math.floor(leftPx) !== Math.floor(prevLeftPx)) {
          reducedFeatures.push(new SimpleFeature({
            id: `${props.sessionId}-${start}`,
            data: {
              score: bin.depth,
              snpinfo: bin,
              start,
    ...
    for (const [key, skip] of Object.entries(skipmap)) {
      skipFeatures.push(new SimpleFeature({
        id: key,
        data: {
          type: 'skip',
          refName: region.refName,
...
export async function renderSNPCoverageToCanvas(
  ...
  const { height, regions, bpPerPx, soa } = props
  const region = regions[0]!
  const width = (region.end - region.start) / bpPerPx

  let p = props
  if (!soa && props.features) {
    const bins: BaseCoverageBin[] = []
    const skipmap = {}
    for (const feature of props.features.values()) {
      const start = feature.get('start')
      const end = feature.get('end')
      const bin = feature.get('snpinfo') as BaseCoverageBin
      for (let i = start; i < end; i++) {
        bins[i - region.start] = bin
      }
    }
    p = { ...props, soa: { bins, skipmap } }
  }


  const { reducedFeatures, skipFeatures, coords, items, ...rest } =
    await renderToAbstractCanvas(width, height, p, ctx =>
      makeImage(ctx, p),
    )
