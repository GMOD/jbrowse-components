...
export class WebGPUAlignmentsRenderer implements AlignmentsBackend {
  ...
  static async create(canvas: HTMLCanvasElement) {
    const device = await WebGPUAlignmentsRenderer.ensureDevice()
    if (!device) {
      return null
    }
    ...
    return new WebGPUAlignmentsRenderer(canvas, result.context, uBuf)
  }
  ...
  private static async ensureDevice() {
    ...
    if (WebGPUAlignmentsRenderer.device !== device) {
      WebGPUAlignmentsRenderer.device = device
      WebGPUAlignmentsRenderer.initPipelines(device)
    }
    return device
  }
  ...
  private static initPipelines(device: GPUDevice) {
    ...
    WebGPUAlignmentsRenderer.readPL = mkPL(READ_WGSL)
    WebGPUAlignmentsRenderer.gapPL = mkPL(GAP_WGSL)
    WebGPUAlignmentsRenderer.mismatchPL = mkPL(MISMATCH_WGSL)
    WebGPUAlignmentsRenderer.insertionPL = mkPL(INSERTION_WGSL)
    WebGPUAlignmentsRenderer.softclipPL = mkPL(SOFTCLIP_WGSL)
    WebGPUAlignmentsRenderer.hardclipPL = mkPL(HARDCLIP_WGSL)
    WebGPUAlignmentsRenderer.modPL = mkPL(MODIFICATION_WGSL)
    WebGPUAlignmentsRenderer.coveragePL = mkPL(COVERAGE_WGSL)
    WebGPUAlignmentsRenderer.snpCovPL = mkPL(SNP_COVERAGE_WGSL)
    WebGPUAlignmentsRenderer.modCovPL = mkPL(MOD_COVERAGE_WGSL)
    WebGPUAlignmentsRenderer.noncovPL = mkPL(NONCOV_HISTOGRAM_WGSL)
    WebGPUAlignmentsRenderer.indicatorPL = mkPL(INDICATOR_WGSL)
    WebGPUAlignmentsRenderer.arcPL = mkPL(ARC_WGSL, 'triangle-strip')
    WebGPUAlignmentsRenderer.arcLinePL = mkPL(ARC_LINE_WGSL, 'line-list')
    WebGPUAlignmentsRenderer.sashimiPL = mkPL(SASHIMI_WGSL, 'triangle-strip')
    WebGPUAlignmentsRenderer.connLinePL = mkPL(CONNECTING_LINE_WGSL)
    WebGPUAlignmentsRenderer.flatQuadPL = mkPL(FLAT_QUAD_WGSL)
  }
  ...
  ensureBuffers(_regionStart: number) {
    // no-op for WebGPU — regions are created on upload
  }

  uploadFromTypedArraysForRegion(regionNumber: number, data: ReadUploadData) {
    const device = WebGPUAlignmentsRenderer.device
    if (!device) {
      return
    }
  ...
  uploadArcsFromTypedArraysForRegion(
    ...
    const r = this.getOrCreateRegion(regionNumber, data.regionStart)
    r.arcBuffer?.destroy()
    r.arcBG = null
    r.arcCount = 0
    r.arcLineBuffer?.destroy()
    ...
    for (const block of blocks) {
      ...
      if (
        ...
        if (needsFeatureHighlight) {
          ...
          p.setPipeline(WebGPUAlignmentsRenderer.readPL!)
  ...
  private drawPileup(
    ...
    if (r.readBG && r.segmentCount > 0) {
      pass.setPipeline(WebGPUAlignmentsRenderer.readPL!)
    ...
    if (r.softclipBG && r.softclipCount > 0) {
      pass.setPipeline(WebGPUAlignmentsRenderer.softclipPL!)
      pass.setBindGroup(0, r.softclipBG)
      pass.draw(6, r.softclipCount)
    }
    if (r.hardclipBG && r.hardclipCount > 0) {
      pass.setPipeline(WebGPUAlignmentsRenderer.hardclipPL!)
    ...
    if (state.showModifications && r.modBG && r.modCount > 0) {
      pass.setPipeline(WebGPUAlignmentsRenderer.modPL!)
  ...
  private drawSashimi(
    ...
    pass.setPipeline(WebGPUAlignmentsRenderer.sashimiPL!)
  ...
  private drawConnectingLines(pass: GPURenderPassEncoder, r: GpuRegion) {
    if (r.connLineBG && r.connLineCount > 0) {
      pass.setPipeline(WebGPUAlignmentsRenderer.connLinePL!)
  ...
  private drawOverlayQuads(
    ...
    pass.setPipeline(WebGPUAlignmentsRenderer.flatQuadPL!)
