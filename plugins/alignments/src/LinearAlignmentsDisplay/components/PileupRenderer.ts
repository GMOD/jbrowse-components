...
export function renderPileup(
  ...
  gl.useProgram(renderer.readProgram)
  // WARNING: u_zero must be 0.0 — HP shader precision guard. See utils.ts.
  gl.uniform1f(renderer.readUniforms.u_zero!, 0)
  gl.uniform3f(
    renderer.readUniforms.u_bpRangeX!,
    bpStartHi,
    bpStartLo,
    regionLengthBp,
  )
  ...
  const regionHlIdx = state.highlightedFeatureId
    ...
    : -1
  gl.uniform1i(renderer.readUniforms.u_highlightedIndex!, regionHlIdx)
  const mode = state.renderingMode ?? 'pileup'
  const isChainMode = mode === 'linkedRead'
  gl.uniform1i(renderer.readUniforms.u_chainMode!, isChainMode ? 1 : 0)
  gl.uniform1i(
    renderer.readUniforms.u_flipStrandLongReadChains!,
    state.flipStrandLongReadChains !== false ? 1 : 0,
  )
  const showStroke = state.showOutline && state.featureHeight >= 4 ? 1 : 0
  gl.uniform1i(renderer.readUniforms.u_showStroke!, showStroke)

  gl.uniform3f(
    renderer.readUniforms.u_colorFwdStrand!,
  ...
  gl.uniform3f(renderer.readUniforms.u_colorPairLL!, ...colors.colorPairLL)
  gl.uniform3f(
    renderer.readUniforms.u_colorModificationFwd!,
    ...colors.colorModificationFwd,
  )
  gl.uniform3f(
    renderer.readUniforms.u_colorModificationRev!,
    ...colors.colorModificationRev,
  )
  gl.uniform3f(
    renderer.readUniforms.u_colorLongInsert!,
    ...colors.colorLongInsert,
  )
  gl.uniform3f(
    renderer.readUniforms.u_colorShortInsert!,
    ...colors.colorShortInsert,
  )
  gl.uniform3f(
    renderer.readUniforms.u_colorSupplementary!,
    ...colors.colorSupplementary,
  )
  gl.uniform3f(
    renderer.readUniforms.u_colorUnmappedMate!,
  ...
  if (state.showMismatches && buffers.gapVAO && buffers.gapCount > 0) {
    gl.useProgram(renderer.gapProgram)
    setCigarUniforms(
      renderer,
      renderer.gapUniforms,
      gl,
      domainOffset,
      state,
      coverageOffset,
    ...
    gl.uniform3f(
      renderer.gapUniforms.u_colorDeletion!,
      colors.colorDeletion[0],
      colors.colorDeletion[1],
      colors.colorDeletion[2],
    )
    gl.uniform3f(
      renderer.gapUniforms.u_colorSkip!,
      colors.colorSkip[0],
      colors.colorSkip[1],
      colors.colorSkip[2],
    )
  ...
  if (state.showMismatches && !state.showModifications) {
    if (buffers.mismatchVAO && buffers.mismatchCount > 0) {
      gl.useProgram(renderer.mismatchProgram)
      setCigarUniforms(
        renderer,
        renderer.mismatchUniforms,
        gl,
        domainOffset,
        state,
        coverageOffset,
  ...
  if (
    ...
    gl.useProgram(renderer.insertionProgram)
    setCigarUniforms(
      renderer,
      renderer.insertionUniforms,
      gl,
      domainOffset,
      state,
      coverageOffset,
    ...
    gl.uniform3f(
      renderer.insertionUniforms.u_colorInsertion!,
  ...
  if (buffers.softclipVAO && buffers.softclipCount > 0) {
    gl.useProgram(renderer.softclipProgram)
    setCigarUniforms(
      renderer,
      renderer.softclipUniforms,
      gl,
      domainOffset,
      state,
      coverageOffset,
    ...
    gl.uniform3f(
      renderer.softclipUniforms.u_colorSoftclip!,
  ...
  if (buffers.hardclipVAO && buffers.hardclipCount > 0) {
    gl.useProgram(renderer.hardclipProgram)
    setCigarUniforms(
      renderer,
      renderer.hardclipUniforms,
      gl,
      domainOffset,
      state,
      coverageOffset,
    ...
    gl.uniform3f(
      renderer.hardclipUniforms.u_colorHardclip!,
  ...
  if (
    ...
    // NOTE: softclip bases intentionally reuse the mismatch shader (same base-colored rect logic)
    gl.useProgram(renderer.mismatchProgram)
    setCigarUniforms(
      renderer,
      renderer.mismatchUniforms,
  ...
  if (
    ...
    gl.useProgram(renderer.modificationProgram)
    setCigarUniforms(
      renderer,
      renderer.modificationUniforms,
      gl,
      domainOffset,
      state,
      coverageOffset,
  ...
  if (state.highlightedChainIds.length > 0) {
    ...
    if (hlIdx >= 0) {
      gl.useProgram(renderer.readProgram)
      gl.uniform1i(renderer.readUniforms.u_highlightOnlyMode!, 1)
      gl.uniform1i(renderer.readUniforms.u_highlightedIndex!, hlIdx)
      gl.bindVertexArray(buffers.readVAO)
      gl.drawArraysInstanced(gl.TRIANGLES, 0, 9, buffers.segmentCount)
      gl.uniform1i(renderer.readUniforms.u_highlightOnlyMode!, 0)
    }
  ...
  if (state.selectedChainIds.length > 0) {
    ...
    if (bounds) {
      ...
      drawOutlineRect(renderer, gl, clip, 0, canvasWidth, regionLengthBp, state)
    }
