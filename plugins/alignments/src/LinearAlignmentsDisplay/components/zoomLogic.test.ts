...
console.log('\n=== Test 1: Domain <-> OffsetPx round-trip ===\n')
{
  ...
  const recoveredOffsetPx = computeOffsetPxFromDomain(
    domain,
    view.bpPerPx,
    contentBlocks,
  )
...
console.log('\n=== Test 2: Zoom preserves mouse position ===\n')
{
  ...
  const mouseBpBefore =
    initialDomain[0] + (initialDomain[1] - initialDomain[0]) * (mouseX / width)
  ...
  const mouseBpAfter =
    result.domain[0] + (result.domain[1] - result.domain[0]) * (mouseX / width)
  ...
  assertClose(
    ...
    mouseBpAfter,
    0.001,
    'Mouse position preserved after zoom',
  )
}
...
console.log('\n=== Test 3: Zoom then sync round-trip ===\n')
{
  ...
  const newOffsetPx = computeOffsetPxFromDomain(
    zoomResult.domain,
    zoomResult.bpPerPx,
    contentBlocks,
  )
  ...
  assertClose(
    ...
    zoomResult.domain[0],
    0.001,
    'Domain start preserved after sync',
  )
  assertClose(
    recoveredDomain[1],
    zoomResult.domain[1],
    0.001,
    'Domain end preserved after sync',
  )
}
...
console.log('\n=== Test 4: Multiple zoom steps ===\n')
{
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log(
      `Zoom ${i + 1}: domain=${domain.map(d => d.toFixed(1))}, bpPerPx=${bpPerPx.toFixed(4)}, mouseBp: ${mouseBpBefore.toFixed(1)} -> ${mouseBpAfter.toFixed(1)}`,
    )
    assertClose(
      mouseBpBefore,
      mouseBpAfter,
      0.01,
      `Zoom ${i + 1}: mouse position preserved`,
    )
  }
}
...
console.log('\n=== Test 5: Zoom + sync with contentBlock.offsetPx = 0 ===\n')
{
  ...
  assertClose(
    ...
    zoomResult.domain[0],
    0.001,
    'Domain start preserved',
  )
  assertClose(
    recoveredDomain[1],
    zoomResult.domain[1],
    0.001,
    'Domain end preserved',
  )
}
...
)
{
  ...
  if (domainDrift > 1) {
    ...
    console.log(
      '   the domain will drift. The drift is proportional to the change in blockOffsetPx.',
    )
  }
}
...
console.log('\n=== Test 7: Correct approach - domain as source of truth ===\n')
{
  ...
  for (let i = 0; i < 3; i++) {
    const mouseBpBefore =
      domainRef[0] + (domainRef[1] - domainRef[0]) * (mouseX / width)
    ...
    const mouseBpAfter =
      domainRef[0] + (domainRef[1] - domainRef[0]) * (mouseX / width)
    console.log(
      `Zoom ${i + 1}: domain=${domainRef.map(d => d.toFixed(1))}, mouseBp preserved: ${mouseBpBefore.toFixed(1)} == ${mouseBpAfter.toFixed(1)}`,
    )
    assertClose(
      mouseBpBefore,
      mouseBpAfter,
  ...
  console.log(
    '   we avoid the drift caused by contentBlocks.offsetPx changing.',
  )
}
...
console.log('\n=== Test 8: Zoom OUT behavior ===\n')
{
  ...
  for (let i = 0; i < 10; i++) {
    ...
    assertClose(
      ...
      mouseBpAfter,
      0.01,
      `Zoom out ${i + 1}: mouse position preserved`,
    )
  }
}
...
console.log('\n=== Test 9: Zoom OUT with negative offsetPx ===\n')
{
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (domain[0] < contentBlocks[0]!.start) {
      ...
      console.log(
        `  This maps to offsetPx = ${offsetPx.toFixed(1)} (negative means scrolled left of block start)`,
      )
    }
  }
}
