 ...
 *
 * Renders pileup, arcs, and cloud modes in a single WebGL canvas with shared
 * coverage. MobX is the source of truth for all state. The view's offsetPx and
 * bpPerPx drive coordinates, ensuring alignment with gridlines and other
 * components. A single autorun watches all relevant observables and renders via
 * WebGL when anything changes.
 */
const WebGLAlignmentsComponent = observer(function WebGLAlignmentsComponent({
  ...
  // Refs for non-MobX values needed by the render autorun
  const colorPaletteRef = useRef(colorPalette)
  colorPaletteRef.current = colorPalette
  // Refs for values used in wheel handler (registered via addEventListener)
  ...
  // --- Event handlers (plain functions, React Compiler handles memoization) ---
  ...
  function handleCanvasMouseMove(e: React.MouseEvent) {
    ...
    if (result.type === 'feature') {
      ...
      if (isChainMode) {
        const readName = result.resolved.rpcData.readNames[hit.index]
  ...
  function handleClick(e: React.MouseEvent) {
    ...
    if (result.type === 'feature') {
      model.setSelectedFeatureIndex(result.hit.index)
      model.selectFeatureById(result.hit.id)

      if (isChainMode) {
        const readName = result.resolved.rpcData.readNames[result.hit.index]
        const chainIndices = readName
          ? (model.chainIndexMap.get(readName) ?? [])
          : []
