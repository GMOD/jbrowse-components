...
function fullyPopulated() {
  return makePileupDataResult({
    ...
    // Position-aggregate passes upload a worker-packed buffer verbatim; the
    // count is what gates them, so the bytes only need to be non-empty.
    coverageGpuBinCount: 1,
    coveragePackedBuffer: new ArrayBuffer(8),
    snpPositions: new Uint32Array([START + 1]),
    snpPackedBuffer: new ArrayBuffer(16),
    interbaseCovPositions: new Uint32Array([START + 2]),
    interbasePackedBuffer: new ArrayBuffer(16),
    indicatorPositions: new Uint32Array([START + 3]),
    indicatorPackedBuffer: new ArrayBuffer(8),
    modCovPositions: new Uint32Array([START + 4]),
    modCovPackedBuffer: new ArrayBuffer(16),
  })
}
