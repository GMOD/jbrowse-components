...
Settings storage and the fetch/render tiering are `agent-docs/ARCHITECTURE.md`;
the arc band is
[reference/ARC_BAND.md](../../../../agent-docs/reference/ARC_BAND.md); layout
stays main-thread per
...
2-5 are auto-wired by MobX; **tier 1 is manual**, because the worker boundary
defeats tracking.
...
- **Tier 4 repaints the whole canvas**, so per-mousemove state must not be
  there. Hover is a React overlay; selection stays in `renderState` (rare, and
  belongs in SVG export).
...
`reconcileChainSuppAcrossRegions` (one molecule across regions) and then
`consensusChainStrandFrames` (molecules about each other), because the worker
frames on `primaryStrand` and **on a foldback the primary flag is arbitrary**.
Alternatives measured 58/52/61% agreement where the consensus reaches 100%.
...
data arrived — never gate first paint on a laid-out map, since a grouped fetch
...
`PileupTruncationRule`, deliberately inert and deliberately not an alert — as a
chip its press set `maxHeight` to 1,000,000, and reads collapsing onto the
bottom row is the cap working.
...
size read `configuredFeatureHeight` (`resolveConf`, promotable slot).
`fittedHeightPx` is a **pitch**, `featureHeight` a **body**; the volatile
bridging them breaks a MobX cycle, so don't collapse it.
...
- Chain layout is handed **neither `sortedBy` nor `largeFeaturesFirst`** — its
  rows are chains. Every ordering/row control curates itself out in chain mode;
  a new one that doesn't is a silent no-op, and a tag sort additionally
  refetches.
...
- On-screen and SVG export share `drawAlignmentBlocks`. Sashimi and linked-read
  ...
  with the export; sashimi's source is a model because the geometry depends on
  pan/zoom but **not** `scrollTop`.
- **No GPU pass can join two displayed regions** — one buffer per region,
  clipped to its own bp range. `bezierArcScope` is the one place deciding
  between `all`, `crossRegion` and `none`; read the getter, never
...
- A cross-region arc was never clipped away — each block projects bp through its
  ...
  globally-correct curve once in view space; its geometry is `arcMark`'s through
  `arcMarkFrom` rather than a lookalike, it is a separate z-layer above every
  canvas arc, and its hover writes `setHoverState` rather than a local key.
...
- **A band's height MINUS its reserved margin is floored at 0** where the
  ...
  declaration is the `.slang` one and the CPU side imports the generated twin
  (adr-051).
- `computeArcBand` is the single source of truth for the arc band, decoupled
  from `showCoverage`. Arc and sashimi strips are reserved **per section**;
  `coverageDisplayHeight` and the fit-height row budget stay global, since
  re-deriving them from `sections` routes the fit volatile back through the
  layout it feeds.
