...
describe('buildMismatchArrays ordering', () => {
  ...
  it("emits no Y array — rows are the layout tier's", () => {
    ...
    const arrays = buildMismatchArrays([mm(300, 65, 0), mm(100, 67, 1)])
  ...
  it('handles the empty and single cases', () => {
    expect(buildMismatchArrays([]).mismatchPositions.length).toBe(0)
    expect([...buildMismatchArrays([mm(42, 65, 0)]).mismatchPositions]).toEqual(
      [42],
    )
  })
