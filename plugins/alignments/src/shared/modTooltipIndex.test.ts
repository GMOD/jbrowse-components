...
function entriesAt(modifications: ModificationEntry[]) {
...
describe('buildModTooltipIndex', () => {
  it('returns undefined for no modifications', () => {
    expect(buildModTooltipIndex([])).toBe(undefined)
  })
  ...
  it('groups by position', () => {
    const at = entriesAt([mod({ position: 100 }), mod({ position: 200 })])
  ...
  it('finds every position, whatever order the calls arrived in', () => {
    ...
    const at = entriesAt(positions.map(position => mod({ position })))
  ...
  it('keeps a position’s rows in arrival order', () => {
    const at = entriesAt([
      mod({ noMod: true, color: 0xff888888 }),
      mod({ color: 0xff0000ff }),
    ])
