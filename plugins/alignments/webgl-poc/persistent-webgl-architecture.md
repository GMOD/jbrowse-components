...
This document outlines the architectural changes needed for a "true" WebGL
implementation where data is uploaded once and zoom/pan only updates uniforms.
...
1. **Worker-based rendering** - Rendering happens in web workers to avoid
   blocking main thread
2. **ImageBitmap transfer** - Workers can't access DOM, so they render to
   OffscreenCanvas and transfer ImageBitmap
3. **Block-based re-rendering** - Each "block" is an independent render unit
   that gets re-rendered on state changes
4. **Mobx reactions** - Any observable change triggers the render reaction

The WebGL context must live on the **main thread** to receive immediate uniform
updates. But rendering on the main thread could block the UI.
...
Instead of using `ServerSideRenderedBlock`, this display manages its own WebGL
canvas:
...
Fetch data for a larger region than visible, so zoom/pan within that region
doesn't need new data:
...
The view's zoom/pan would need to call the display's `updateUniforms()`
directly:
...
1. **Start with Path 1** - Get WebGL rendering working within existing
   architecture
2. **Measure performance** - See if GPU acceleration alone is sufficient
3. **If needed, evolve to Path 2** - Add persistent canvas for specific
   high-performance needs
4. **Consider GenomeSpy integration** - Their architecture already solves this;
   could potentially embed/adapt their rendering layer

The GenomeSpy approach (upload once, transform via uniforms) is the gold
standard, but requires either:

- Embedding GenomeSpy's renderer into JBrowse
- Significant refactoring of JBrowse's display architecture
- A hybrid where certain display types opt out of the standard rendering
  pipeline
