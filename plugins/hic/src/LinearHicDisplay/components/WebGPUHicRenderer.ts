...
export class WebGPUHicRenderer implements HicBackend {
  ...
  static async create(canvas: HTMLCanvasElement) {
    ...
    if (WebGPUHicRenderer.device !== device) {
      WebGPUHicRenderer.device = device
      WebGPUHicRenderer.initPipelines(device)
    }
    const result = await initGpuContext(canvas)
    if (!result) {
      return null
    }
    return new WebGPUHicRenderer(canvas, result.context, device)
  }
  ...
  private static initPipelines(device: GPUDevice) {
    ...
    WebGPUHicRenderer.bindGroupLayout = device.createBindGroupLayout({
      entries: [
        {
          binding: 0,
    ...
    const layout = device.createPipelineLayout({
      bindGroupLayouts: [WebGPUHicRenderer.bindGroupLayout],
    })
    const shaderModule = device.createShaderModule({ code: hicShader })

    WebGPUHicRenderer.pipeline = device.createRenderPipeline({
      layout,
  ...
  uploadData(data: {
    ...
    const device = WebGPUHicRenderer.device
    if (!device || !WebGPUHicRenderer.bindGroupLayout) {
      return
    }
  ...
  private rebuildBindGroup(
    ...
    if (!this.rampTexture) {
      // @ts-expect-error bindGroup will be set once ramp texture is uploaded
      this.gpuData = { instanceBuffer, bindGroup: null, instanceCount }
      return
    }

    const bindGroup = device.createBindGroup({
      layout: WebGPUHicRenderer.bindGroupLayout!,
      entries: [
  ...
  render(state: HicRenderState) {
    const device = WebGPUHicRenderer.device
    if (!device || !WebGPUHicRenderer.pipeline || !this.gpuData?.bindGroup) {
      return
    }
    ...
    pass.setPipeline(WebGPUHicRenderer.pipeline)
