...
const rendererCache = new WeakMap<HTMLCanvasElement, MultiWiggleRenderer>()
...
export class MultiWiggleRenderer {
  ...
  private uniformData = new ArrayBuffer(MULTI_UNIFORM_SIZE)
  ...
  static getOrCreate(canvas: HTMLCanvasElement) {
    ...
    if (!renderer) {
      renderer = new MultiWiggleRenderer(canvas)
  ...
  private static async ensureDevice() {
    ...
    if (MultiWiggleRenderer.device !== device) {
  ...
  private static initPipelines(device: GPUDevice) {
    ...
    MultiWiggleRenderer.bindGroupLayout = device.createBindGroupLayout({
      entries: [
        {
          binding: 0,
    ...
    const layout = device.createPipelineLayout({
      bindGroupLayouts: [MultiWiggleRenderer.bindGroupLayout],
    })
    const shaderModule = device.createShaderModule({ code: multiWiggleShader })

    MultiWiggleRenderer.fillPipeline = device.createRenderPipeline({
      layout,
    ...
    MultiWiggleRenderer.linePipeline = device.createRenderPipeline({
      layout,
      vertex: { module: shaderModule, entryPoint: 'vs_main' },
      fragment: {
        module: shaderModule,
  ...
  async init() {
    ...
    try {
      ...
      console.error('[MultiWiggleRenderer] WebGL2 fallback also failed:', e)
      return false
    }
  }
  ...
  uploadRegion(
    ...
    const interleaved = this.interleaveMultiInstances(sources, totalFeatures)
    ...
    const bindGroup = device.createBindGroup({
      layout: MultiWiggleRenderer.bindGroupLayout,
      entries: [
  ...
  renderBlocks(
    ...
    const pipeline = isLine
      ? MultiWiggleRenderer.linePipeline!
      : MultiWiggleRenderer.fillPipeline
  ...
  private writeUniforms(
    ...
    regionStart: number,
    numRows: number,
    state: MultiWiggleGPURenderState,
  ) {
  ...
  private interleaveMultiInstances(
    ...
    const buf = new ArrayBuffer(totalFeatures * MULTI_INSTANCE_BYTES)
    const u32 = new Uint32Array(buf)
    const f32 = new Float32Array(buf)
    let offset = 0
    for (const [rowIndex, source] of sources.entries()) {
      for (let i = 0; i < source.numFeatures; i++) {
        const off = (offset + i) * MULTI_INSTANCE_STRIDE
