...
export const wiggleColorSchema = ConfigurationSchema(
  'WiggleColor',
  {
    ...
    ...colorChannelSlots({
      ...
      scale:
        'how field becomes a colour: threshold cuts score at the domain and paints each side; linear and log fade from the colour at the cut out to the ends of the y domain, which is the density picture; categorical hands each source a palette entry; none paints value, keeping a field for a switch back; unset beside a field, it is linear with a ramp, categorical over source and threshold over score',
      domain:
        'for a threshold scale, the one cut point the two sides part at, empty meaning the origin; for a categorical scale over source, the sources that take the palette first, in order',
    }),
    ...colorPaletteSlot,
    ...colorRampSlot,
  },
