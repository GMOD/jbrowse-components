...
function normalizeScore(score: number, min: number, max: number, scaleType: string) {
  if (scaleType === 'log') {
...
export function getDensityColor(
  ...
  const t = Math.min(1, Math.max(0, Math.abs(norm - zeroNorm) / Math.max(maxDist, 0.0001)))
