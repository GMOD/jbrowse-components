...
describe('makeDensityRgbStringFn', () => {
  it('interpolates toward white for scores near zero', () => {
    ...
    const colorFn = makeDensityRgbStringFn(0, 1, SCALE_TYPE_LINEAR, 255, 0, 0)
  ...
  it('returns consistent strings (LUT cache hit)', () => {
    const colorFn = makeDensityRgbStringFn(-1, 2, SCALE_TYPE_LINEAR, 255, 0, 0)
    expect(colorFn(0.5)).toBe(colorFn(0.5))
  })

  it('works with log scale', () => {
    const colorFn = makeDensityRgbStringFn(0.1, 100, SCALE_TYPE_LOG, 0, 0, 255)
