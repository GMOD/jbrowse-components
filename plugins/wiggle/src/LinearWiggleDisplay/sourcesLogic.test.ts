...
describe('buildSources', () => {
  it('synthesizes overlay palette only in overlay mode', () => {
    ...
    const overlay = buildSources(editable, undefined, 'shared')
    expect(overlay.map(s => s.color)).toEqual([
      overlayColors[0],
      overlayColors[1],
      overlayColors[2],
    ])
    const rows = buildSources(editable, undefined, 'row')
  ...
  it('preserves explicit colors over palette synthesis in overlay mode', () => {
    ...
    const out = buildSources(editable, undefined, 'shared')
  ...
  it('keeps each overlay palette color across a subtree filter', () => {
    ...
    const unfiltered = buildSources(editable, undefined, 'shared')
    const out = buildSources(editable, ['source_0', 'source_5'], 'shared')
  ...
  it('keeps each group color across a subtree filter', () => {
    ...
    const unfiltered = buildSources(editable, undefined, 'row')
    const out = buildSources(editable, ['c'], 'row')
  ...
  it('layout color survives through to sources view', () => {
    ...
    const out = buildSources(editable, undefined, 'row')
  ...
  it('assigns same color to sources sharing a group, with a row each or not', () => {
    ...
    for (const mode of ['row', 'shared'] as const) {
      const out = buildSources(editable, undefined, mode)
  ...
  it('routes a group color to labelColor in density mode, leaving the ramp alone', () => {
    ...
    const out = buildSources(editable, undefined, 'density')
    for (const s of out) {
