...
for (const [label, nSamples, polyFrac] of [
  ...
  const variants = Array.from({ length: VARIANTS }, () =>
    makeGenotypes(nSamples, polyFrac),
  )
  ...
  bench('  old (inline if/else)', () => {
    for (const gt of variants) {
      countAllelesOld(gt)
    }
  })
  bench('  new (inline, local vars)', () => {
    for (const gt of variants) {
      countAllelesNew(gt)
    }
  })
...
)
{
  const samples = Array.from(
    { length: SAMPLES_LARGE },
    (_, i) => DIPLOID_POOL[i % DIPLOID_POOL.length]!,
  )
  ...
  bench('  bucket object (b.count0++)', () => {
    for (let i = 0; i < VARIANTS; i++) {
      countFastBucket(samples)
    }
  })
  bench('  local vars (count0++)', () => {
    for (let i = 0; i < VARIANTS; i++) {
      countFastLocals(samples)
    }
  })
}
