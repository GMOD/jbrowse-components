...
function old_packHaplotypesFromRaw(
  callGenotype,
  callGenotypePhased,
  ploidy,
  nSamples,
...
function new_packHaplotypesFromRaw(
  callGenotype,
  callGenotypePhased,
  ploidy,
  nSamples,
  ...
  for (let s = 0; s < nSamples; s++) {
    for (let p = 0; p < ploidy; p++) {
      if (Math.random() < missingFrac) {
        ...
        buf[s * ploidy + p] =
          alleles[Math.floor(Math.random() * alleles.length)]
      }
    }
  }
  return buf
...
function bench(label, fn, iterations = 5) {
  ...
  console.log(
    `  ${label.padEnd(52)} mean=${mean.toFixed(1).padStart(7)}ms  min=${min.toFixed(1).padStart(7)}ms  max=${max.toFixed(1).padStart(7)}ms`,
  )
  return mean
}
...
console.log(
  `Total genotype calls per round: ${(N_VARIANTS * N_SAMPLES).toLocaleString()}`,
)
...

{
  ...
  newMean = bench(
    'NEW: fillEncodedFromRaw into flat buffer (combined pass)',
    () => {
...

{
  ...
  newMean = bench(
    'NEW: packHaplotypesFromRaw (diploid fast path, single loop)',
    () => {
      for (const cg of variants) {
...
function merged_packHaplotypesFromRaw(
  callGenotype,
  callGenotypePhased,
  ploidy,
  nSamples,
...

{
  ...
  oldMean = bench(
    'OLD: packHaplotypesFromRaw (ploidy branch + phase check)',
    () => {
      for (let vi = 0; vi < variants.length; vi++) {
        old_packHaplotypesFromRaw(
          variants[vi],
          phaseArrays[vi],
          PLOIDY,
          N_SAMPLES,
  ...
  mergedMean = bench(
    'NEW: packHaplotypesFromRaw (single diploid loop, s<<1)',
    () => {
      for (let vi = 0; vi < variants.length; vi++) {
        merged_packHaplotypesFromRaw(
          variants[vi],
          phaseArrays[vi],
          PLOIDY,
          N_SAMPLES,
...

{
  ...
  for (let i = 0; i < r1.length; i++) {
    if (r1[i] !== r2[i]) {
      gpuErrors++
      if (gpuErrors === 1) {
        console.error(`  GPU pack mismatch at index ${i}: ${r1[i]} vs ${r2[i]}`)
      }
    }
  }
  if (gpuErrors === 0) {
    ...
    console.error(`  ${gpuErrors} GPU packing errors!`)
  }
...

{
  ...
  for (let t = 0; t < 20; t++) {
    ...
    for (let i = 0; i < 500; i++) {
      if (flat[i] !== oldEnc[i]) {
        console.error(
          `  RAW mismatch at sample ${i}: new=${flat[i]} old=${oldEnc[i]}`,
        )
        errors++
      }
    }
    if (
      newCounts.nHomRef !== oldCounts.nHomRef ||
      newCounts.nHet !== oldCounts.nHet ||
      newCounts.nHomAlt !== oldCounts.nHomAlt ||
      newCounts.nValid !== oldCounts.nValid
    ) {
      console.error(
        `  RAW count mismatch: new=${JSON.stringify(newCounts)} old=${JSON.stringify(oldCounts)}`,
      )
      errors++
    }
  }
  ...
  for (let t = 0; t < 20; t++) {
    ...
    for (let i = 0; i < 500; i++) {
      if (flat[i] !== oldEnc[i]) {
        ...
        console.error(
          `  STRING mismatch at sample ${i} (gt="${val}"): new=${flat[i]} old=${oldEnc[i]}`,
        )
        errors++
      }
    }
    if (
      ...
      console.error(
        `  STRING count mismatch: new=${JSON.stringify(newCounts)} old=${JSON.stringify(oldCounts)}`,
      )
      errors++
    }
  }
  ...
  for (let t = 0; t < 20; t++) {
    ...
    if (
      r1.nHomRef !== r2.nHomRef ||
      r1.nHet !== r2.nHet ||
      r1.nHomAlt !== r2.nHomAlt ||
      r1.nValid !== r2.nValid
    ) {
      console.error(
        `  PACK RAW count mismatch: new=${JSON.stringify({ nHomRef: r2.nHomRef, nHet: r2.nHet, nHomAlt: r2.nHomAlt, nValid: r2.nValid })} old=${JSON.stringify({ nHomRef: r1.nHomRef, nHet: r1.nHet, nHomAlt: r1.nHomAlt, nValid: r1.nValid })}`,
      )
      errors++
    }
    for (let w = 0; w < r1.words; w++) {
      if (
        r1.altH1[w] !== r2.altH1[w] ||
        r1.validH1[w] !== r2.validH1[w] ||
        r1.altH2[w] !== r2.altH2[w] ||
        r1.validH2[w] !== r2.validH2[w]
      ) {
