...
export class WebGPUVariantRenderer implements VariantBackend {
  ...
  static async create(canvas: HTMLCanvasElement) {
    ...
    if (WebGPUVariantRenderer.device !== device) {
      WebGPUVariantRenderer.device = device
      WebGPUVariantRenderer.initPipelines(device)
    }
    ...
    return new WebGPUVariantRenderer(canvas, result.context, uniformBuffer)
  }
  ...
  private static initPipelines(device: GPUDevice) {
    ...
    WebGPUVariantRenderer.bindGroupLayout = device.createBindGroupLayout({
      entries: [
        {
          binding: 0,
    ...
    const layout = device.createPipelineLayout({
      bindGroupLayouts: [WebGPUVariantRenderer.bindGroupLayout],
    })
    const shaderModule = device.createShaderModule({ code: variantShader })

    WebGPUVariantRenderer.pipeline = device.createRenderPipeline({
      layout,
  ...
  uploadRegion(
    ...
    const device = WebGPUVariantRenderer.device
    if (!device || !WebGPUVariantRenderer.bindGroupLayout) {
      return
    }
    ...
    const bindGroup = device.createBindGroup({
      layout: WebGPUVariantRenderer.bindGroupLayout,
      entries: [
  ...
  renderBlocks(
    ...
    const device = WebGPUVariantRenderer.device
    if (!device || !WebGPUVariantRenderer.pipeline) {
      return
    }
    ...
    for (const block of blocks) {
      ...
      pass.setPipeline(WebGPUVariantRenderer.pipeline)
