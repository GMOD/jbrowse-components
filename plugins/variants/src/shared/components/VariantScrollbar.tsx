...
// `ScrollChrome` bound to the shared multi-sample variant scroll geometry, so
// both variant displays render it from just the model. The scroll gesture
// itself lives in useVariantVirtualScroll.
//
// It takes the rows' own viewport — `availableHeight` at `rowsTopOffset`, since
// the bands stacked above the rows are pinned and don't scroll. The scrollbar
// had been taking the height without the offset, which put its track a band's
// worth too high: invisible while it was the only thing there, and the matrix
// display's `lineZoneHeight` defaults to 20, so this was the default state
