 ...
 * one genotype row per sample, each variant drawn at its honest genomic position
 * (so this panel shares the `coord_cartesian(xlim=)` contract with the 1-D
 * genomic tracks, unlike the matrix's site-index columns). Reuses the inline
 * `read_vcf_gt()` reader (Rsamtools scanTabix, no VariantAnnotation) and its
 * ref/het/hom/other/nocall cell classing; samples stay in VCF order (no
...
${v}_max_missing <- ${p.maxMissing}    # drop sites with more than this no-call fraction`,
  ...
  lev <- rev(gt$samples)          # draw the first VCF sample at the top
