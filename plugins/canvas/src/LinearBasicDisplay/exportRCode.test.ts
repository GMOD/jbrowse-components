...
test('emits a gene-model panel from pure ggplot2 primitives', () => {
  ...
  expect(f.helpers).toEqual(['read_gff', 'gene_layout'])
  expect(f.packages).toEqual(['rtracklayer', 'ggplot2'])
  expect(f.plotExpr).toContain(
    'gene_layout(read_regions(function(chrom, start, end) read_gff(genes, chrom, start, end), regions, c("start", "end")))',
  )
...
test('bed format selects the read_bed reader', () => {
  ...
  expect(f.helpers).toEqual(['read_bed', 'gene_layout'])
  expect(f.plotExpr).toContain(
    'gene_layout(read_regions(function(chrom, start, end) read_bed(genes, chrom, start, end), regions, c("start", "end")))',
  )
})
