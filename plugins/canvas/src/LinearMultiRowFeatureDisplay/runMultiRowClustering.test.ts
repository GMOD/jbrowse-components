...
// A run under a subtree filter is a run over the clade: the rows the filter
...
// The key `useFetch` serializes on every render of the open dialog, so it names
// the run arguments rather than the MST display node, whose snapshot carries a
// cohort's worth of rows.
