...
test('rows in `sources` order; color→ordinal; gaps are -1', () => {
  const matrix = buildMultiRowMatrix({
    ...
    features: [
      { row: 's1', start: 0, end: 10, colorKey: 'red' },
      { row: 's2', start: 0, end: 5, colorKey: 'blue' },
      // s3 has no features
    ],
  })
...
test('later feature on a row wins the bin (paint order)', () => {
  const [row] = buildMultiRowMatrix({
    ...
    features: [
      { row: 's1', start: 0, end: 10, colorKey: 'a' }, // a → 0
      { row: 's1', start: 0, end: 5, colorKey: 'b' }, // b → 1, overrides bin 0
    ],
  })
...
test('bins split across regions proportional to width', () => {
  const matrix = buildMultiRowMatrix({
    ...
    features: [{ row: 's1', start: 100, end: 110, colorKey: 'x' }],
  })
