...
test('parseCoarseLine_parses_tile_row', () => {
  ...
  expect(row).toEqual({ refStart: 0, refEnd: 10677, superOrd: 0, type: 'tile', hapCount: 0 })
})

test('parseCoarseLine_parses_second_tile', () => {
  const row = parseCoarseLine('ctgA\t10677\t20691\t113\ttile\t113-231')
  expect(row).toEqual({ refStart: 10677, refEnd: 20691, superOrd: 113, type: 'tile', hapCount: 0 })
})

test('parseCoarseLine_parses_v2_snarl_row', () => {
  const row = parseCoarseLine('grch38#0#chr20\t48000\t48010\t42\tsnarl\t12\t42,205')
  expect(row).toEqual({ refStart: 48000, refEnd: 48010, superOrd: 42, type: 'snarl', hapCount: 12 })
})

test('parseCoarseLine_parses_v2_chain_row', () => {
  const row = parseCoarseLine('CHM13#1#chr20\t49000\t99000\t42\tchain\t0\t42-203')
  expect(row).toEqual({ refStart: 49000, refEnd: 99000, superOrd: 42, type: 'chain', hapCount: 0 })
})
...
test('coarseRowsToGfa_emits_slines_with_type', () => {
  const rows = [
    ...
    { refStart: 10000, refEnd: 20000, superOrd: 100, type: 'snarl', hapCount: 5 },
  ]
...
test('coarseRowsToGfa_lni_is_ref_span', () => {
  const rows = [{ refStart: 5000, refEnd: 7500, superOrd: 42, type: 'snarl', hapCount: 3 }]
  const gfa = coarseRowsToGfa(rows)
  expect(gfa).toContain('LN:i:2500')
})

test('coarseRowsToGfa_hap_count_tag_only_when_nonzero', () => {
  const chain = coarseRowsToGfa([{ refStart: 0, refEnd: 1000, superOrd: 1, type: 'chain', hapCount: 0 }])
  expect(chain).not.toContain('hc:i:')
  const snarl = coarseRowsToGfa([{ refStart: 0, refEnd: 1000, superOrd: 1, type: 'snarl', hapCount: 12 }])
