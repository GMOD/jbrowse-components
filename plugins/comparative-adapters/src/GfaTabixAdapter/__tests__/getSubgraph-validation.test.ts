...
// chr20 fixtures are generated by tools/graph-truth-extractor/setup-chr20-validation.sh
// and gitignored due to size; skip these smoke tests when they aren't present.
const hasFixtures = fs.existsSync(`${FIXTURES}.pos.bed.gz.tbi`)
const testIf = hasFixtures ? test : test.skip
...
testIf('getSubgraph_chr20_small_region_produces_segments', async () => {
...
testIf('getSubgraph_chr20_medium_region_produces_graph_structure', async () => {
  const adapter = makeAdapter()
  const region = {
    refName: 'chr20',
    start: 10000,
...
testIf('getSubgraph_chr20_edges_reference_valid_segments', async () => {
  const adapter = makeAdapter()
  const region = {
    refName: 'chr20',
    start: 0,
...
testIf('getSubgraph_chr20_walks_reference_valid_segments', async () => {
  const adapter = makeAdapter()
  const region = {
    refName: 'chr20',
    start: 0,
...
testIf('getSubgraph_chr20_large_region_produces_many_segments', async () => {
  const adapter = makeAdapter()
  const region = {
    refName: 'chr20',
    start: 0,
...
testIf('getSubgraph_chr20_empty_region_returns_header', async () => {
  const adapter = makeAdapter()
  const region = {
    refName: 'chr20',
    start: 999_000_000,
