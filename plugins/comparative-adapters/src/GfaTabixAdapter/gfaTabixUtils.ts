...
export function parseSegsBytes(bytes: Uint8Array) {
...
export async function loadSegsIndex(shard: SegsShard) {
  if (!shard.idxPromise) {
...
export async function getSegsForRangeFromShard(
  shard: SegsShard,
  minSegOrd: number,
  maxSegOrd: number,
) {
  const idx = await loadSegsIndex(shard)
  const startOffset = Number(idx[minSegOrd] ?? 0n)
  const endOffset = Number(
    idx[Math.min(maxSegOrd + 1, idx.length - 1)] ?? 0n,
  )
  const length = endOffset - startOffset
  if (length <= 0) {
    return [] as SegRecord[]
  }
  const bytes = await shard.bgzf.read(length, startOffset)
  return parseSegsBytes(bytes)
}
...
export abstract class BaseGfaTabixAdapter extends BaseFeatureDataAdapter {
  ...
  protected abstract getSegsForRange(
    minSegOrd: number,
    maxSegOrd: number,
  ): Promise<SegRecord[]>
