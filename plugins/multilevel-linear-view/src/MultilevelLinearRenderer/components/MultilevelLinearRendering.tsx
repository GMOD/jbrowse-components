...
import { MultilevelLinearComparativeDisplay } from '../../MultilevelLinearComparativeDisplay'
...
function getResources(displayModel: MultilevelLinearComparativeDisplay) {
  const worker = !('type' in displayModel)
  if (!worker && isAlive(displayModel)) {
    const parentView = getContainingView(
      displayModel,
    ) as MultilevelLinearViewModel
...
function MultilevelLinearRendering({
  height,
  width,
  displayModel,
  ...
  width: number
  height: number
  displayModel: MultilevelLinearComparativeDisplay
      ...
      layoutMatches(
        features.map(level =>
          level
            ...
            .sort((a, b) => a.get('multilevelId') - b.get('multilevelId')),
        ),
        assemblyManager,
      ),
...
export default observer(MultilevelLinearRendering)
