...
// The R figure fragment a display contributes. It lives in `@jbrowse/display-kit`
// beside `ExportSvgDisplayOptions`, its exact sibling: `exportRCode()` and
// `renderSvg()` are two methods of one display contract, and a display plugin
// implementing either should not have to reach into a VIEW plugin for its type.
// Re-exported here because published plugins resolve it from this one.
export type { RTrackFragment } from '@jbrowse/display-kit/RExportFragment'
