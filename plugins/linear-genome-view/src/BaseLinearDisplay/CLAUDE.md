...
| `rpcProps()`                 | not defined | declare a method returning the literal RPC payload; every field it **returns** becomes a cache key via `SettingsInvalidate` (which watches the serialized payload, `rpcPropsCacheKey`, not the reads that built it — `serializeRpcProps` says why, and `installGlobalFetchAutorun` keys the global family on the same function). The mixin doesn't declare a base default so subclass return types stay narrow through MST `.views()` chains; subclasses extend via the standard super-capture pattern |
