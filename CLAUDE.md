...
- The ambient `typescript` devDependency stays on 6.x because
  `@typescript-eslint`/`ts-api-utils` haven't shipped TypeScript 7 support
  (peer range `<6.1.0`) — bumping it breaks `pnpm lint`. `pnpm typecheck`
  gets TS7's speed via an aliased `typescript7` devDependency
  (`npm:typescript@7`), invoked by path in the root `typecheck` script.
  `tsc --build` in individual package `build:esm` scripts still runs on the
  ambient 6.x. Once typescript-eslint ships TS7 support, drop the alias and
  bump `typescript` itself to 7.
