 ...
 * snapshot to hand to `createViewState` or `createLinearGenomeView` as
 * **`session`**, not `defaultSession` — that slot is the compiler-checked shape
 * for a session you author in TypeScript, and a decoded one is only known at
 * runtime, so it goes through the door that validates it as MST applies it.
