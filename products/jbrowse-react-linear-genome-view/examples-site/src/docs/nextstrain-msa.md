...
nucleotide mutations applied — a **gap-free MSA needs no aligner**. The generator
reconstructs that MSA for a subsample of tips (`<slug>_msa.fasta`) and prunes the
tree to the same tips (`<slug>.nwk`), both hosted on `jbrowse.org/demos`.
...
row-linked — collapse a clade in the tree and the alignment follows.
