...
`observer` (the [observe the visible view](../multiple-views/#observe-visible) example shows
this pattern), read one view's `bpPerPx`/`offsetPx`, and call the matching
actions on the other. Both `viewState` objects are ordinary state-tree nodes, so
