...
This uses the managed [`<LinearGenomeView>`](../setting-up-the-view/#with-init) — `assembly`,
`tracks`, and `init` are passed as plain props and the component owns the
engine. The CRAM track is fetched directly over HTTP range requests from S3; no
...
[web worker RPC](../with-web-worker/) to keep parsing off the main thread.
