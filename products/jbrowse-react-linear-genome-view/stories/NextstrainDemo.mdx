...
A demo of the embedded LGV browsing the SARS-CoV-2 genome in a style
similar to [Nextstrain](https://nextstrain.org) — gene coloring matches
their published palette, and an "Entropy score" track shows per-base
sequence diversity.
...
This demo uses `FromConfigAdapter` for both the sequence and the
feature tracks: the data is supplied inline as JSON rather than fetched
from BAM/BigWig/GFF3 files. That makes it convenient for embedding
on-the-fly analyses, generated reports, or small reference datasets —
no flat files to host.
...
(largish, since it inlines the SARS-CoV-2 sequence and entropy
scores). Summarized below:
