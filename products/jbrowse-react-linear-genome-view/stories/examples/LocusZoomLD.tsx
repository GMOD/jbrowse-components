...
const NCBI_REFSEQ_TRACK = {
  ...
  adapter: {
    ...
    gffGzLocation: { uri: 'https://jbrowse.org/ucsc/hg19/ncbiRefSeq.gff.gz' },
    index: {
      indexType: 'CSI',
      location: { uri: 'https://jbrowse.org/ucsc/hg19/ncbiRefSeq.gff.gz.csi' },
    },
  },
  displays: [
    {
...
export const LocusZoomLD = () => {
  const [state] = useState(() =>
    createViewState({
      assembly,
      tracks: [GIANT_BMI_TRACK, NCBI_REFSEQ_TRACK],
      // GWASPlugin draws the Manhattan plot; VariantsPlugin registers the
      // PlinkLDTabixAdapter that the LD coloring reads.
      plugins: [GWASPlugin, VariantsPlugin],
      defaultSession: {
...
// the lead SNP. The GWAS file is read straight from LocusZoom's hosted demo
// data (its '#chrom pos rsid …' tabix layout, no end column) and the LD from
// their headerless PLINK '--r2' file. The index SNP auto-tracks the top
// genome-wide hit (rs1121980 at FTO); right-click any SNP to re-anchor LD to it.
const BASE = 'https://jbrowse.org/demos/gwas'
...
const GIANT_BMI_TRACK = {
  ...
  adapter: {
    ...
    scoreColumn: 'neg_log_pvalue',
    bedGzLocation: { uri: \`\${BASE}/gwas_giant-bmi_meta_women-only.gz\` },
    index: { location: { uri: \`\${BASE}/gwas_giant-bmi_meta_women-only.gz.tbi\` } },
  },
  displays: [
    {
      type: 'LinearManhattanDisplay',
      ...
      ldAdapter: {
        type: 'PlinkLDTabixAdapter',
        ldLocation: { uri: \`\${BASE}/plink.ld.tab.gz\` },
        index: { indexType: 'TBI', location: { uri: \`\${BASE}/plink.ld.tab.gz.tbi\` } },
      },
    },
  ],
}
    ...
    createViewState({
      ...
      defaultSession: {
        ...
        view: {
          ...
          init: {
            assembly: 'hg19',
            loc: 'chr16:53,700,000..53,900,000', // FTO/BMI locus
            tracks: ['giant_bmi_ld', 'ncbi_refseq_hg19'],
          },
        },
      },
