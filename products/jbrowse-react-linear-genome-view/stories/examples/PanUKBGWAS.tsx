...
function makeTrack(phenotypeId: string) {
  return {
    type: 'GWASTrack',
    trackId: 'panukb_gwas',
    ...
    adapter: {
      type: 'GWASAdapter',
      scoreColumn: 'neglog10_pval_meta_hq',
...
function GenomeView({ phenotypeId }: { phenotypeId: string }) {
  const [state] = useState(() =>
    createViewState({
      assembly,
      tracks: [makeTrack(phenotypeId)],
      plugins: [GWASPlugin],
      defaultSession: {
        name: 'Pan-UKB GWAS',
        view: {
...
export const PanUKBGWAS = () => {
  ...
  return (
    <div>
      ...
      <GenomeView key={phenotypeId} phenotypeId={phenotypeId} />
    </div>
  )
}
