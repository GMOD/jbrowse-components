...
When using JBrowse 2, information about the current app state such as what
views are open is stored in a session. It is possible to configure a JB2 app
with a `defaultSession`, which will be loaded when the app first is loaded.
...
not be optimial for programmatic usage.
...
To create a default session using JB2, load whatever tracks and data you want
to be displayed into a session with a single linear genome view open. Export
the session from JB2 and save it as `session.json` in your app.
