...
`@jbrowse/react-linear-genome-view` doesn't have the app-level control needed
to properly execute an OAuth 2.0 login flow.

There are some instances where InternetAccounts could be useful for
non-external resources too though. For example, you can use a custom
InternetAccount to override the `fetch` behavior of a particular track,
allowing you to customize where and how the data is retrieved.
