 ...
 * query param or hash fragment. The value carries jbrowse-web's `encoded-`
 * prefix, so the same string is also what that app's `?session=` accepts.
 *
 * Where `getSnapshot(viewState.session)` gives you the session to store, this
 * gives you the session to *put in a link*: compressed for the URL, and
