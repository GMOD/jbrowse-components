...
function guessTrackType(adapterType: string): string {
...
await yargs(process.argv.slice(2))
  .command(
    ...
    async run => {
      ...
      let { trackType, trackId, name, a: assemblyNames } = run
      ...
      const loadType =
        (load as 'copy' | 'inPlace' | 'move' | 'symlink' | undefined) ||
        'inPlace'
