...
local file where it already sits. Omit --load entirely for URLs. For a local
file the matching index is whichever conventional spelling is actually beside it
— <file>.bai/.crai/.tbi, a <file>.csi, or the reads.bai form Picard writes
beside reads.bam. A URL cannot be checked, so it gets <file>.bai/.tbi; pass
--indexFile for anything else.
...
what it cannot draw in a corner notice rather than refusing the track. This
