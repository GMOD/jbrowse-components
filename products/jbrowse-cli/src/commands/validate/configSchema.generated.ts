...
export const configJsonSchema: Record<string, unknown> = JSON.parse(`
{
  ...
  "description": "The JBrowse 5.0.0-beta.9 config.json: assemblies, tracks, connections, site-wide configuration and a default session. Generated from the registered types by \`pnpm autogen\`.",
  "type": "object",
  "properties": {
    "$schema": {
      "type": "string",
  ...
  "$defs": {
    ...
    "FeatureField": {
      "type": "string",
      "description": "A field the display reads off each feature: a name, a dotted path into a structured field (\`INFO.SVTYPE\`), or a \`jexl:\` expression over \`feature\`, which the display evaluates per feature."
    },
    "CssColor": {
      "type": "string",
      ...
      "anyOf": [
        ...
        },
        {
          ...
          "properties": {
            ...
            "domain": {
              "description": "for a threshold scale, up to eight cut points, sorted, empty meaning one at the origin; for a categorical scale over source, the subtracks outside a group that take the range first, in order.",
              "anyOf": [
                {
                  "type": "array",
                  "items": {
        ...
        "displayDefaults": {
          ...
          "properties": {
            ...
            "rows": {
              "anyOf": [
                {
                  "$ref": "#/$defs/LinearMultiSampleVariantDisplaySlots/properties/rows"
                },
                {
      ...
      "anyOf": [
        {
          "type": "string",
          "pattern": "\\\\.ix([?#].*)?$",
          "description": "A trix index as its \`.ix\` path or URL, the same as \`{ uri }\`: the type follows from the extension, and \`assemblyNames\` from the track it sits on or the config's one assembly."
        },
        {
          "type": "object",
