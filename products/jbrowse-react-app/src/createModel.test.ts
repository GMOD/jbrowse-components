...
test('a plugin class is installed on the main thread', () => {
  const { pluginManager } = createModel({ runtimePlugins: [TestPlugin] })
...
test('a loadPlugins record keeps the definition the RPC worker boots from', () => {
  const { pluginManager } = createModel({
    runtimePlugins: [{ plugin: TestPlugin, definition }],
  })
...
test('bundled core plugins are not treated as runtime plugins', () => {
  const { pluginManager } = createModel({ runtimePlugins: [] })
