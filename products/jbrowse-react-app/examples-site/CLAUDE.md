...
`@jbrowse/react-app2`, `@jbrowse/core/*`, `@jbrowse/plugin-*`, `@mui/material`,
`react`, `mobx-react`. No relative import into this site's own helpers. Check
with:
...
The only hit today is `../volvox-config.json`, which is the allowed exception:
**bulk data** may be imported, because inlining a 72 KB config would bury the
code the page is about. Data only, never code.
