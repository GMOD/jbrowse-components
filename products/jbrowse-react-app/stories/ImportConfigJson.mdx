...
loaded. This is typical of low-level components: more flexibility, slightly more
code.
...
handle the JSON natively, and the config ships alongside your JavaScript — no
runtime fetch.
=======
...
If the config lives on a server (or differs per environment, per user, per
route), fetch it before constructing `createViewState`. Wrap `viewState` in
`useState`/`useEffect` so React can render once the fetch resolves.

<Story of={WithFetchConfigJson} />

Use static import for shipped configurations and runtime fetch for configs that
are loaded on demand or generated server-side.
