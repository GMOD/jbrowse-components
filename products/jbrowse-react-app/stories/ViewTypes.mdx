...
spreadsheet. `init` takes the `assembly` and a `uri` to the file — note `uri`
is on `init`, not a track config.
...
between the breakpoints of split/translocated reads. `init.views` is an array
of `{ loc, assembly, tracks }` — one per stacked panel.
