...
export default function RootModel(
  ...
  return (
    types
      ...
      // TODOAUTH: will add another addDisposer(self, autorun) which will go through the internetaccounts in the config and add them to the statemodel
      .actions(self => ({
        afterCreate() {
          Object.entries(localStorage)
            ...
            .filter(
              ([key, _val]) =>
                key.indexOf(self.configPath || 'undefined') !== -1,
            )
            .forEach(([key, val]) => {
              try {
