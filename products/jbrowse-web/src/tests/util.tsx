...
export async function exportAndVerifySvg({
  ...
  await waitFor(() => {
    ...
    const fileSaverCalls =
      (saveAs as unknown as jest.Mock).mock?.calls?.length ?? 0
    expect(coreCalls + fileSaverCalls).toBeGreaterThan(0)
  }, actualDelay)

  const svg = ((coreSaveAs as unknown as jest.Mock).mock?.calls[0] ??
    // eslint-disable-next-line @typescript-eslint/no-deprecated
    (saveAs as unknown as jest.Mock).mock?.calls[0])![0].content[0]
