...
async function createSvInspectorViewWithInit(spec: {
  assembly: string
  uri?: string
  fileType?: string
}) {
  ...
  const view = await session.launchView('SvInspectorView', spec)
...
// must keep it.
test('snapshot forwards the launch blob to the child spreadsheet synchronously', async () => {
  const { view } = await createSvInspectorViewWithInit({
    assembly: 'volvox',
    uri: 'test_data/volvox/volvox.dup.vcf.gz',
  })
