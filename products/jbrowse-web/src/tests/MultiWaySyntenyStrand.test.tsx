...
// A reference-anchored PAF read as all-vs-all: no peach/cacao rows. The pair
// fetch threw the adapter's no-such-pair error, logged once per pair on every
// refetch, before the display composed the gutter through the anchor anyway
test('a star PAF composes its mate gutter without a failed pair fetch', async () => {
  const config = structuredClone(baseConfig) as { tracks: unknown[] }
  config.tracks.push({
    type: 'SyntenyTrack',
    trackId: 'three_way_star',
    name: 'grape-anchored peach and cacao',
    assemblyNames: ['grape', 'peach', 'cacao'],
    adapter: {
      type: 'MultiGenomePAFAdapter',
      assemblyNames: ['grape', 'peach', 'cacao'],
      pafLocation: { uri: 'three_way_star.paf', locationType: 'UriLocation' },
    },
  })
  ...
  const display = await waitFor(
    () => {
      const d = view.tracks[0]?.displays[0] as
        | MultiWaySyntenyDisplayModel
        | undefined
      expect(d?.laneLinks?.get('peach|cacao')?.links).toEqual([])
      return d!
    },
    { timeout: 30000 },
  )
  expect(
    jest
      ...
      .mock.calls.filter(call => `${call[0]}`.includes('one lane failed')),
  ).toEqual([])
  expect(display.pairLinks.has('peach|cacao')).toBe(true)
}, 60000)
