  ...
  tests: [
    {
      ...
      //
      // What is asserted is the throw and the page error, not the warning
      // count. The root is detached in the effect cleanup and destroyed on a
      // later task, and destroying it is loud: measured here, one reload logs
      // ~48 liveliness warnings whichever deferral is used (48 at 0ms, 49 at
      // 5s), because killing an MST tree invalidates computeds inside it that
      // undisposed observer() reactions still watch. That is a known residual
      // with its own entry in agent-docs/TODO.md, and every read in it is of a
      // scalar or a reference on a tree nothing renders.
      //
      // Not destroying at all is what zeroes that count, and it is not
      // available: the whole plugin-facing tree hangs off this root, so it
      // drops every `beforeDestroy` in it — which is how #5618 left
      // jbrowse-plugin-apollo's websockets open.
      //
      ...
      name: 'a plugin reload leaves the app working and throws on no dead node',
      fn: async page => {
        const throwShape: string[] = []
        const pageErrors: string[] = []
        const notFound: string[] = []
        page.on('console', msg => {
          const text = msg.text()
          if (text.includes('must be done on the initializing phase')) {
            throwShape.push(text.split('\n')[0]!)
          }
        })
        ...
        // that the superseded root really dies is pinned in jsdom, by
        // src/tests/rootModelTeardown.test.tsx — nothing here can see isAlive
        if (throwShape.length) {
          throw new Error(
            `${throwShape.length} unmaterialized-child throw(s) across the reload:\n  ${throwShape.join('\n  ')}`,
          )
        }
