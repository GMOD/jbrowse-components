  ...
  tests: [
    {
      ...
      fn: async page => {
        const url = appendGpuParam(
          `http://localhost:${PORT}/?config=test_data%2Fconfig_demo.json&session=share-XyL52LPDoO&password=861E4`,
        )
    ...
    },
    {
      ...
      fn: async page => {
        const url = appendGpuParam(
          `http://localhost:${PORT}/?config=test_data%2Fconfig_demo.json&session=share-pjaAq1hNxB&password=Z9teR`,
        )
        await page.goto(url, { waitUntil: 'networkidle0', timeout: 60000 })
        await waitForLoadingToComplete(page)
