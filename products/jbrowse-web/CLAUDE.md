...
- **`pnpm review-snapshots-web` rebuilds its page on every load**, so editing
  ...
  protocol, note drafts and repaint properties with the website's screenshot
  review via `@jbrowse/browser-test-utils/reviewApp` — fix a bug in either
  page's card, but fix a protocol bug in the shared client, and run
