...
matches with aliases resolved, so `contigB` comes back as the canonical `ctgB`.
Pass one and you get one kind of hit. RefName hits are returned first and capped
well below any sensible list length, so a genome with ten thousand scaffolds
cannot push the gene hits off the end.
...
which is why this page's genes track is `gff3tabix_genes` and not the
`volvox_genes` every other page here uses. An index and a config that disagree
resolve locations perfectly and then quietly fail to turn a track on.
...
responses can land out of order — a slow `EDEN` overwriting a fast `EDEN.1`
