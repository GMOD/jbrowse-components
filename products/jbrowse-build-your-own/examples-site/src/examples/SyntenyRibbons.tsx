 ...
 * order.
 *
 * The async spelling, because this session names `LinearSyntenyView`: a view
 * type's state model is a dynamic import until something asks for it, and the
 * synchronous `createViewState` requires every type its session names to be
 * loaded already. Awaiting is the whole difference — `createViewStateAsync`
 * loads what the session names, then builds the same tree.
 *
...
// The engine is built asynchronously (see makeView), so this stands the demo up
// once it exists. Splitting it out rather than gating inside the demo is what
// keeps every hook there — the width measurement, the palette, the pan/zoom —
// reading a view model that is already there.
