#!/usr/bin/env node
import fs from 'fs'
// @ts-expect-error - registerHooks is not typed in @types/node yet
...
// Dynamic imports ensure the CSS hook is registered before these modules load
const [
  { default: yargs },
  { parseArgv, standardizeArgv },
  { renderRegion },
  { setupEnv },
  { convert },
] = await Promise.all([
