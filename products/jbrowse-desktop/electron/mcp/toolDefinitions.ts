...
Build with a spec: genomes, tracks, views and layout go on screen in one jb.loadSessionSpec(spec) call. Then change what is open as a document: jb.setSession(edited snapshot) patches views by id, jb.addView(spec) adds one, jb.fitToWindow() makes it all fit. Screenshot a VISUAL change (styling, layout, a figure) and whenever a settle reports notReady or offscreen — and READ the image, do not just take it. For show/hide/reorder/navigate/fit the settle plus jb.sessionSummary() is the verification, far cheaper than an image. Verify data with jb.getFeatures, not the picture.
