...
FIRST call docs topic "live-model": short, with working examples and every jb member. Then "recipes" has a verified snippet for most asks, and "hosted-data" the config URL for any UCSC or GenArk assembly when nothing is open. Orient with jb.sessionSummary() and never assume state carried over between calls.

Build with a spec: genomes, tracks, views and layout go on screen in one jb.loadSessionSpec(spec) call. Then change what is open as a document: jb.setSession(edited snapshot) patches views by id, jb.addView(spec) adds one, jb.fitToWindow() makes it all fit. After changing anything, screenshot AND read the image: a wrong trackId, empty region or dropped settings key renders as a plausible browser with something quietly missing. Verify data with jb.getFeatures, not the picture.

Introspect, never guess: jb.listTracks() for trackIds; jb.describeSlots(jb.trackModel('x').activeDisplay.configuration) for a display's settings keys. Every result carries "logs" (console output) and "notifications" (toasts since your previous call). Settle results (jb.waitReady, and what jb.addTrack and jb.loadSessionSpec return) add "notReady" (views that failed to initialize, tracks whose display is not drawing; these raise no toast and look fine in a screenshot) and "offscreen" (session taller than the window: screenshot with fullPage: true). A thrown error names the line in your code.

Traps: mutate the model only via actions. Write display settings with track.applyDisplaySettings(settings), never raw assignment; an unknown key lands in its "unapplied" list, so read the report. Data files may spell refNames differently than the assembly: jb.getFeatures handles it. Read a fresh view's region with await jb.visibleRegions(viewId).`
...
export const MCP_TOOLS = [
  {
    name: 'run_javascript',
    handledBy: 'renderer',
    description: `Run an async JavaScript function body inside JBrowse Desktop against the LIVE session. READ docs topic "live-model" BEFORE your first call: working examples, every jb member. Your code receives these arguments: "jb" (the helper library; prefer it to the raw model), "session" (the live mobx-state-tree session; re-read it each call, a new config replaces it), "rootModel", "pluginManager", "signal" (aborts at timeoutMs; check it in long loops). Mutate the model only through actions.

jb in brief. Orient: jb.sessionSummary() first (views, tracks, assemblies, visible regions); jb.inspect(path?) walks the live model by dot-path, listing getters, actions and modelType. Tracks: jb.listTracks(search?, limit?) answers { total, tracks }; jb.trackModel(trackId, viewId?) is the shown track's live model; jb.describeSlots(conf) lists every settings key a display accepts; track.applyDisplaySettings(settings) answers { applied, unapplied, failed }; jb.addTrack({ location, index?, assembly?, name?, show?, viewId?, settleMs? }) takes a path or URL. Views: jb.loadSessionSpec(spec, settleMs?) builds them and replaces the session (docs topic "session-spec"); jb.addView(oneViewSpec) adds one beside what is open; jb.setSession(document) rewrites the session as a document (jb.mst.getSnapshot(session), edited: a view keeping its id is patched in place, loc navigates it, a { trackId } entry in its tracks opens that track); jb.fitToWindow() shrinks what is open until it fits; jb.view(viewId?); await jb.visibleRegions(viewId?). Data: jb.getFeatures({ trackId, loc?, assembly?, viewId?, regions?, byteLimit? }) returns live Feature objects (visible region by default); aggregate in code, return only the answer. Wait: await jb.waitReady(ms) after mutations. With several views open, the helpers throw naming the candidates: pass viewId from jb.sessionSummary().

Whatever you "return" comes back serialized with "logs" (console output), "notifications" (toasts) and "pageErrors"; a throw names the line in YOUR code. State persists on globalThis.`,
    inputSchema: {
      type: 'object',
      properties: {
        code: {
