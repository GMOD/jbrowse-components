...
Traps: mutate the model only via actions. Write display settings with track.applyDisplaySettings(settings), never raw assignment; an unknown key lands in its "unapplied" list, so read the report. A track too tall for the window wants heightMode "fit" or "grow", not displayMode "compact". Data files may spell refNames differently than the assembly: jb.getFeatures handles it. A fresh view throws "width undefined" until it mounts: read its region with await jb.visibleRegions(viewId).`
