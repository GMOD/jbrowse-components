...
- tracks: jb.listTracks(search?) answers { total, tracks } (the catalog, each with trackId, name, type, adapterType, assemblyNames — not a bare array); jb.trackModel(trackId, viewId?) (the shown track's live model); jb.describeSlots(conf) (every settings key a display accepts — introspect rather than guess); track.applyDisplaySettings(settings) — a model action targeting the track's activeDisplay — for in-place styling with legacy-key handling (e.g. { displayMode: "compact" }); it reports { applied, unapplied, failed } — "failed" is the one that means you got it wrong, "unapplied" also lists keys that are not config slots; jb.addTrack({ location, index?, assembly?, name?, show?, viewId?, settleMs? }) (local path or URL, format inferred; a list of bigWig locations becomes one stacked MultiQuantitativeTrack; settleMs 0 skips its settle, for several adds and one jb.waitReady)
