...
Introspect, never guess: jb.listTracks() for trackIds; jb.describeSlots(jb.trackModel('x').activeDisplay.configuration) for a display's settings keys — an unknown settings key is not an error, it lands in applyDisplaySettings' "unapplied" list, so read the report. Every result carries "logs" (what the code console.logged) and "notifications" (toasts the session raised since your previous call, with level — each reported once); settle results (jb.waitReady, and what jb.addTrack and jb.loadSessionSpec return) add "notReady" (views that failed to initialize — an assembly the config does not have — and tracks whose display is not drawing — over the fetch-size gate, or errored; these raise no toast and look fine in a screenshot) and "offscreen" (the session is taller than the window, so a viewport screenshot cuts views off — screenshot with fullPage: true, or shrink tracks): read all of them. A thrown error names the line in your code and the output printed before it.

A track too tall for the window is a height STRATEGY, not displayMode: a display may take heightMode "fit" (squash the content into the height slot) or "grow"; displayMode "compact" only shrinks each feature and will not tame a deep stack. describeSlots lists both.

Traps: mutate the MST model only via actions; write display settings with track.applyDisplaySettings(settings) (never raw assignment). view.launchTrack on an already-shown track silently applies nothing — applyDisplaySettings is the update path. Data files may spell refNames differently than the assembly ("ctgA" vs "contigA"): jb.getFeatures handles it; raw adapter code must call jb.renameRegionsIfNeeded first. A freshly created view throws "width undefined" from its region getters until it mounts and navigates: read them with await jb.visibleRegions(viewId), which waits for both, rather than view.initialized, which is already true while visibleRegions is silently empty. Aggregate large results in code; do not return thousands of raw features.`
...
In scope: "session" (the live mobx-state-tree session model), "rootModel", "pluginManager", "signal" (an AbortSignal that fires when timeoutMs expires — check it in long loops so a timed-out call actually stops), and the helper library "jb":
...
- tracks: jb.listTracks(search?, limit?) answers { total, tracks } (the catalog, each with trackId, name, type, adapterType, assemblyNames — not a bare array); jb.trackModel(trackId, viewId?) (the shown track's live model; throws saying whether the id is unknown or the track is merely not shown); jb.describeSlots(conf) (every settings key a display accepts — introspect rather than guess); track.applyDisplaySettings(settings) — a model action targeting the track's activeDisplay — for in-place styling with legacy-key handling (e.g. { displayMode: "compact" }); it reports { applied, unapplied, failed } — "failed" is the one that means you got it wrong, "unapplied" also lists keys that are not config slots; jb.addTrack({ location, index?, assembly?, name?, show?, viewId?, settleMs? }) (absolute local path or URL, format inferred; a list of bigWig locations becomes one stacked MultiQuantitativeTrack; settleMs 0 skips its settle, for several adds and one jb.waitReady)
- views: jb.view(viewId?) is the open view; views nest and several can be open, so jb.view(), jb.trackModel, jb.visibleRegions, jb.addTrack and jb.getFeatures over a visible region throw naming the candidates rather than picking the first when more than one view could answer — pass viewId from jb.sessionSummary(). jb.loadSessionSpec(spec, settleMs?) builds views declaratively (docs topic "session-spec" is the full reference; e.g. { views: [{ type: "LinearGenomeView", assembly, loc, tracks: [...] }] }; it settles for settleMs, default 30000, and reports what is still not ready); session.layoutViews(spec) arranges the OPEN views into panels without replacing the session (same tree shape as a spec layout — { direction: "horizontal", children: [{ views: [id] }, ...] } — but its leaves name view ids or indexes into session.views; it turns workspaces mode on and applies the stated order, which the lower-level session.applyLayoutSpec does not); view.navToLocString("BRCA1") navigates (gene names go through text search); await view.launchTrack(id) / view.hideTrack(id); await jb.visibleRegions(viewId?) is the visible region as numbers ({ assemblyName, refName, start, end })
- data: jb.getFeatures({ trackId, loc?, assembly?, viewId?, regions?, byteLimit? }) (or jb.getFeatures(trackId, loc?, { assembly?, viewId?, byteLimit? })) returns the track's live Feature objects (visible region by default; refNames renamed; read on the same worker as the track's display, so its parsed index is reused; a region over the byte gate throws rather than answering short, and byteLimit raises the gate for a read you mean) — aggregate in code, return only the answer
- waiting: await jb.waitReady(ms) after mutations, before reading render state (its result carries the session's error notifications, a "notReady" list of views that failed to initialize and tracks that settled without drawing, and "offscreen" when the session is taller than the window)
