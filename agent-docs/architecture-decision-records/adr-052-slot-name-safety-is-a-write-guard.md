...
| ------ | -------------: | --------: | ------: | ------------: |
| source |            629 |       123 |     506 |         80.4% |
| tests  |            613 |       174 |     439 |         71.6% |
| all    |          1,242 |       297 |     945 |         76.1% |
...
- **The audit script and its baseline stay.** Their value is diagnostic —
  ...
  than a number to drive toward. The count invites a sweep, and the split by
  slot name says the sweep would mostly be the wrong work:
...
| root config              | `theme`, `extraThemes`, `defaultDriver`, `workerCount`, `shareURL` |               8 |
| everything else          | 28 other names                                                     |              38 |

<!-- END GENERATED MEASUREMENT config-read-gap-populations -->

The track/assembly row files each read under the display or widget that
contains it, but no display schema can narrow a read against the containing
track or a session track config handed in. The root-config row is blocked:
that schema is assembled from the plugin manager at runtime, and a base taken
from `pluginManager.getDisplayType(…).configSchema` poisons it through the
definition merge. The tail is mostly factories left at
`AnyConfigurationSchemaType`, plus the `frozen` slots that never leave. The
gate fails only when the count grows, so **re-baseline in the same commit as
any improvement**.
