...
Neither survived being measured on this machine (the table below): one probe is
0.4 ms, one mint-and-revoke is under 0.1 ms.
...
- **A loop that never awaits** takes `createAbortBreakpoint(signal)` and writes
  ...
  `scheduler.yield` where the runtime has it, a `MessageChannel` task
  otherwise, and a zero timer under Node, where a port turn runs no due timer.
...
which ships, drops it. The uncancelled columns of the two yield arms carry a
neighbour's load from the run this was recorded on, and are to be re-read on a
quiet machine before anyone quotes an overhead from them.
