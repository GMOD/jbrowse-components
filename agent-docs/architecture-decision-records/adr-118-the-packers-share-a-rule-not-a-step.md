...
| -------- | ------: | -----: | ------------: | --------: |
| layout   | 200,000 |  9.6ms |            48 |     1.00x |
| control  | 200,000 |  9.5ms |            48 |     0.99x |
| stack    | 200,000 | 42.7ms |           214 |     4.45x |
| step     | 200,000 | 26.2ms |           131 |     2.73x |
| features | 200,000 |   11ms |            55 |     1.15x |

<!-- END GENERATED MEASUREMENT pileup-layout-vs-stack -->

**The packing is not what costs.** `step` — the shared step over a feature
list already built — is 2.73x, and `features` alone is 1.15x, so materialising
a `SimpleFeature` per read costs about what the whole of today's layout costs
before the step has run. A pileup is a `Uint32Array` of positions from the
worker and a `Uint16Array` of rows every downstream pass indexes by read; the
transform stage reads a `Feature[]` and answers a `Feature[]` that must then be
asked, one `get('row')` at a time, for what it decided. That is the boundary,
and it is the same boundary ADR-114 found on the encoder side at 3.11x.
...
- **Porting `computeLayout`'s plain path to `stack`.** The only (a) in the
  audit, and the measurement is above: 4.45x, for a path the display reaches
