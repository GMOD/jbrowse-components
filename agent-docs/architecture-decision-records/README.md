...
| [065](adr-065-display-readiness-selectors.md) | Proposed | Display readiness is a data attribute, and `data-testid` should stop mutating — the `-done`/`_done` suffix is an internal convenience that never reached the published contract, and its ergonomic advantage is recoverable as a helper |
