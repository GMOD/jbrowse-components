...
| [075](adr-075-the-isoform-cap-runs-in-the-worker.md) | Accepted | The per-gene isoform cap collapses in the worker's layoutSubfeatures and puts the expanded-gene set in the RPC cache key, reversing the main-thread design ideas/canvas-glyph-system.md argued for — the worker→main boundary carries no isoform structure to relayout over |
