...
the bin above it; a feature with no value, a VCF's `[undefined]` included,
files under `''` and paints `NO_CATEGORY_COLOR`; text that is no number files under `NOT_A_NUMBER_LABEL`
...
**A key lists every bin once anything painted.** The field carries
`closed: true`, and `derivedColorScale` adds each domain value no painted row
names. The bins are the scale, where a categorical domain is only an order, and
Manhattan's, the alignments' and the LD keys already list theirs from the
declaration. A region with nothing painted keys nothing, as a categorical
colour's does, and the no-value and not-a-number rows appear once something
paints them.

**`colorFieldOf` answers either field; `categoricalColorField` stays
categorical.** The canvas display's `paintedColorField` paints and keys, its
...
- The Color by menu offers no threshold. Edit as JSON and the config write one,
  and a round trip through Default or Solid color and back to the attribute
  returns the field on `categorical`, its cuts then read as an order, as
...
- **`linear` and `log` on this display.** An unpinned ramp needs its domain
  ...
  Reversed by ADR-167, once the main thread held the scale.
