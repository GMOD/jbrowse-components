...
22 overridable hooks. **Sitting on the default** is what a display that does not override one gets.
...
commit's data under the earlier viewport. The comparative wrapper's key is
`comparativeFetchKey` — the display's `currentFetchKey` plus the adapter
config, since neither display's key carries an adapter term and an adapter
edit would otherwise wake the autorun into a decline — and its `dataCurrent`
compares the same function's output against the same stamp, so the gate and
the export gate cannot disagree on an axis. This exists because a **secondary** fetch
