...
22 registered displays declare 235 config slots, 42 MST properties and 53 volatiles between them — counting what each display's own directory declares.
...
| `MultiWaySyntenyDisplay` | `plugins/linear-comparative-view` | 12 | 3 | 12 |
| `LDDisplay` | `plugins/variants` | 11 | 0 | 2 |
| `LinearHicDisplay` | `plugins/hic` | 8 | 2 | 3 |
| `LGVSyntenyDisplay` | `plugins/linear-comparative-view` | 7 | 2 | 0 |
...
| `ScoreScaleMixin()` | Score axis, written in the config slots. `scoreAxisConfigSchemaFields`. Brings `ScoreAxisMixin` plus `scaleType` / `autoscaleType` / `minScore` / `maxScore` / `manual*` / `numStdDev` and their setters, i.e. the whole `ScoreScaleModel` interface the shared score menu and `SetMinMaxDialog` consume | `LinearAlignmentsDisplay`, `WiggleScoreConfigMixin` |
| `ScoreAxisMixin()` | Score axis, without a home for it. Nothing — no config slots. Brings the derived half of the score-menu contract: `*Bound` / `hasManualScoreBounds` / `defaultScoreDomain` / `valueScales` / `axes`, over the `scaleType` and `manual*` a composer answers | `LinearMarkDisplay` |
