...
| `LayoutTreeSidebarMixin()` | The dendrogram sidebar with its arrangement in display state — `layout`, `clusterTree`, `clusterProvenance` and `subtreeFilter` — as the multi-row feature and MAF displays still keep it. The same arrangement API as `TreeSidebarMixin` over those props, until each display moves onto the `rows` config object | `LinearMafDisplay` |
