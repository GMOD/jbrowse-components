...
24 overridable hooks. **Sitting on the default** is what a display that does not override one gets.
...
| --- | --- | --- |
| `rpcProps()` | `rpcPropsCacheKey` moves -> every loaded region's `regionFetchKey` stamp is stale, and `SettingsInvalidate` -> `clearAllRpcData()` | refetch every region |
...
`rpcPropsCacheKey | adapterConfigKey | zoomFetchKey`, the same three axes the
global family's `currentFetchKey` carries, so a settings change reads as stale
through this compare and not only through `SettingsInvalidate`'s cleared
coverage map. `FetchVisibleRegions` calls it per region and refetches the ones
that fail. A display states its rule as two hooks, which are two different
questions:
