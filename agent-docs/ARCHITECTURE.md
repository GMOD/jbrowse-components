...
20 registered displays declare 250 config slots, 45 MST properties and 60 volatiles between them — counting what each display's own directory declares.
...
| `GlobalFetchMixin()` | `RegionTooLargeMixin`, `RenderLifecycleMixin`, `KeyedFetchMixin` | `LDTrackDisplay`, `LinearArcDisplay`, `LinearHicDisplay`, `LinearPairedArcDisplay`, `MultiWaySyntenyDisplay` |
