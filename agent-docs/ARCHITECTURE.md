...
- Three fetch foundations, every one on `FetchMixin`: `MultiRegionDisplayMixin`
  (per region, its own autoruns) and `GlobalFetchMixin` (one dataset, display
  installs its own autorun) for the **LGV** displays, and
  `ComparativeFetchMixin` (`@jbrowse/synteny-core`) for the two views that draw
  onto a canvas the view owns. The last two share `KeyedFetchMixin`'s
  `currentFetchKey` / `loadedFetchKey` compare, and each display installs its
  own autorun over the shared `installFetch` skeleton.
...
20 registered displays declare 186 config slots, 42 MST properties and 58 volatiles between them — counting what each display's own directory declares.
...
| `LinearWiggleDisplay` | `plugins/wiggle` | 5 | 2 | 0 |
...
display that lives in an LGV, and a third (`ComparativeFetchMixin`,
`@jbrowse/synteny-core`) the two comparative views' displays:
...
`installFetch` pieces. `RegionTooLargeMixin`'s gate is derived and opt-in on
the LGV two (see [the region-too-large
...
**The comparative row was a footnote until 2026-09.**
[ADR-054](architecture-decision-records/adr-054-comparative-displays-keep-their-own-fetch.md)
kept `LinearSyntenyDisplay` and `DotplotDisplay` off `FetchMixin` on four
grounds, and
[ADR-105](architecture-decision-records/adr-105-the-comparative-displays-compose-fetchmixin.md)
records how each lapsed: `ComparativeFetchMixin` composes `KeyedFetchMixin`, so
the two keyed families share one freshness pair, one rotation, one loading
flag, one cancel and one retry, and `installComparativeFetchAutorun` is a
declaration over the same pieces `installGlobalFetchAutorun` is — plus the
refName rename a `run` here is handed, minus the viewport-change cancel lapse
(the viewport *is* the fetch input there). What still makes them a shape is
the canvas: both put their `RenderLifecycleMixin` *above* the display, so one
canvas is shared by several displays and is laid out by the model that owns it.

That is what makes them a shape rather than a variation: keyed
...
| `rpcProps` | no `SettingsInvalidate` autorun at all, so no user setting ever refetches (correct for `LinearReferenceSequenceDisplay`, indistinguishable from an omission for anyone else) | `alignments/LinearAlignmentsDisplay`, `canvas/LinearBasicDisplay`, `canvas/LinearMultiRowFeatureDisplay`, `gccontent/LinearGCContentDisplay`, `gwas/LinearManhattanDisplay`, `hic/LinearHicDisplay`, `maf/LinearMafDisplay`, `variants/LDDisplay`, `variants/LinearMultiSampleVariantDisplay`, `variants/shared`, `wiggle/LinearWiggleDisplay`, `wiggle/MultiLinearWiggleDisplay` |
| `fetchNeeded` | nothing is ever fetched | `alignments/LinearAlignmentsDisplay`, `canvas/LinearBasicDisplay`, `canvas/LinearMultiRowFeatureDisplay`, `gwas/LinearManhattanDisplay`, `linear-comparative-view/LGVSyntenyDisplay`, `maf/LinearMafDisplay`, `sequence/LinearReferenceSequenceDisplay`, `variants/shared`, `wiggle/LinearWiggleDisplay`, `wiggle/MultiLinearWiggleDisplay` |
...
| `paintInert` | same, for a paint that is never coming — both fetch families fill it with `foundationPaintInert` (a failed fetch, a standing user cancel, an empty viewport), so a display outside them owes its own | `display-kit` |
...
| `byteGateAdapterPath` | the estimate and the budget both describe the track’s own `adapter` — wrong for a display that reads a different file at different zooms, and the one hook such a display overrides, since `byteGateAdapterConfig` is the config at this path | — |
...
already put `fetchInert` there, applied to three identical volatiles that had
nothing keeping them identical (the comparative one joined with ADR-105). Chord
...
`zoomFetchKey` needs no rule, because MST throws on a getter declared inside
...
- Don't answer "which map holds this region" with `zoomFetchKey`. Presence is
