...
20 registered displays declare 187 config slots, 42 MST properties and 61 volatiles between them — counting what each display's own directory declares.
...
| `LinearMultiRowFeatureDisplay` | `plugins/canvas` | 11 | 3 | 1 |
...
| `hoveredFeature` | `undefined` forever, so the view container never feeds `session.hovered` from this display and nothing outside it — a linked view, the hover-synced sidebar — learns what is under the cursor | `alignments/LinearAlignmentsDisplay`, `arc/shared`, `canvas/LinearBasicDisplay`, `canvas/LinearMultiRowFeatureDisplay`, `gwas/LinearManhattanDisplay`, `linear-comparative-view/MultiWaySyntenyDisplay`, `variants/shared`, `wiggle/shared` |
| `clearHoveredFeature` | a no-op, right for a display that derives its hover from the live pointer; a display that STORES one and sits here keeps naming what used to be under the cursor after a zoom, a scroll or the too-large banner, since the viewport-change clear both foundations install calls this | `alignments/LinearAlignmentsDisplay`, `arc/shared`, `canvas/LinearBasicDisplay`, `canvas/LinearMultiRowFeatureDisplay`, `gwas/LinearManhattanDisplay`, `linear-comparative-view/MultiWaySyntenyDisplay`, `variants/LinearMultiSampleVariantDisplay`, `variants/shared`, `wiggle/shared` |
