...
| `clearDisplaySpecificData` | a no-op, so `clearAllRpcData` drops `loadedRegions` and nothing else — the payload store keeps its entries and the refetch overwrites them region by region, which is what a display that wants stale data drawn under the refetch (ADR-006) is choosing, and what a display holding a second store has silently not cleared | `alignments/LinearAlignmentsDisplay`, `maf/LinearMafDisplay`, `variants/shared` |
