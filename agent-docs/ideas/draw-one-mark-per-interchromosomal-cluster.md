...
description: an N-pair translocation draws N marks per side, each stroked and hovered as though it alone carried N reads, because mate pairs never share a coordinate and so `arcKey` coalesces nothing. A representative member's own coordinate invents nothing — but which one is a figure-changing call, and the merge crosses the mark, the hit test, the tooltip and the shader.
---

# Draw one mark per interchromosomal cluster

Moved out of [TODO.md](../TODO.md) on 2026-08-26, when the backlog was cut to
what v5.0.0 turns on. It changes what every published translocation figure
looks like, which is the last thing to take inside a release window, and three
position rules are unchosen.
