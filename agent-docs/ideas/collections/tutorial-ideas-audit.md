...
What survives the growth is the demand evidence, the priorities and the dead
ends; re-check a gap against `website/docs/tutorials/` before writing to it. The
...
- `readpair_heatmap.md` (16250c4b58) - the Cue-style read-pair contact heatmap,
  dropped with its figures, specs, gallery card, and build scripts.
...
Two of the lists below moved without closing:
...
The topic lists in this doc came from what the code supports. This is what people
...
- **Long-read isoforms.** `NA12878-DirectRNA...minimap2.sorted` as a
  whole-genome BAM plus a chr1 CRAM subset, unused.
...
`tutorials/mappability_qc.md` is the worked example: what looked like an hg19-only story off
...
a section — see [lightweight-toolkit](../waiting-on-a-call/lightweight-toolkit.md) for what the
