...
| [canvas-glyph-system](canvas-glyph-system.md) | The per-glyph `{layout, emit}` registry rejected for four grounded reasons, and the lighter layout-switch variant skipped with it. Read before proposing that a glyph's layout and emit live in one file. |
