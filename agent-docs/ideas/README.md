...
| [cross-region-arcs-view-space-pass](cross-region-arcs-view-space-pass.md) | If the cross-region arc set ever outgrows its SVG overlay, one extra GPU pass reusing arc.slang unmodified draws it — packed against layout pixels rather than bp, scissored to the band over the whole canvas. What makes the substitution legal, and the two costs it carries. |
