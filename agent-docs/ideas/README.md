...
| [display-test-harness](display-test-harness.md) | The ten per-display testEnv.ts harnesses and their divergent session shims — what the duplication has already cost, and the three homes a shared one could take. Read before writing an eleventh, or before unifying them. |
