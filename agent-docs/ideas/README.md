...
| [sa-hops-in-the-bezier-overlay](sa-hops-in-the-bezier-overlay.md) | The bezier connector overlay never reads SA tags, so a read whose middle segments are off screen gets a direct arc between the two that are left — a junction that does not exist, drawn as a plain inversion, while the arc band suppresses it and the split view dashes it. Plus the one-ended hops nobody draws, and the shared `Chain` type that lost the caller it was proposed for. |
