...
| [hic](hic.md) | A user-draggable color threshold, an A/B compartment log-ratio mode, and a chromosome-pair selector for inter-chromosomal contacts — which nothing detects today, contrary to what this doc used to claim. |
...
| [sa-hops-in-the-bezier-overlay](sa-hops-in-the-bezier-overlay.md) | What is left of the SplitThreader-style multi-hop proposal now that the derivative-allele picker has shipped — the bezier overlay drops a read's off-screen SA hops, and the shared `Chain` type lost the caller it was for. |
