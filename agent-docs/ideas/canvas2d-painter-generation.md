...
| dotplot | `line` · dotplot | antialias, capsule, colorPack, hpmath | premultiplied | arrays (Float64 absolute cumBp) | lineWidth + lineCap round + moveTo/lineTo/stroke | B | capsule; `capsuleDist` is load-bearing for the round cap and `lineCap: 'round'` is its twin. The packed buffer is window-relative f32 where the painter uses Float64 — B changes the painter's precision to the GPU's, which the 2026-09 mark port deliberately did not do |
| gwas | `point` · manhattan | antialias, hpmath, colorPack, pointGlyph | default | arrays | rect / arc / triangle / diamond paths + fill | B | shape chosen in the vertex stage (bar, crisp square, disc, tri, diamond); `glyphEdgeAlpha` derivative is AA only; `valueToYPx` and `pointDrawsBar` exported; the GPU bar carried a `BAR_OVERDRAW_PX` the painter lacked until 2026-09-08, when it was measured against its own hairline claim and removed |
