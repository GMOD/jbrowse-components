...
- [Config package cleanups declined](#config-package-cleanups-declined-2026-08-audit) —
  the `fullConfSnapshot` throw and the config editor's slot enumeration
...
- [Opening a web share link in Desktop](#opening-a-web-share-link-in-desktop) —
  measured: the snapshot applies but drops `sessionTracks` in silence, so this
  is a translation and not a decode
...
- [Pangenome graph figures not yet shot](#pangenome-graph-figures-not-yet-shot) — chrM
  as a whole-graph file, SMN1/SMN2, and carriage at the graph's own granularity
...
## A contribution to `pangenome/jbrowse-visualization`

Their guide stops at one MAF view via `wgatools` + `mafchunk` + `maf2bed` plus a
plugin-store install. `@jbrowse/plugin-maf` is in core now, and the graph,
synteny, variants and depth projections are all possible off the same pggb
output — so the guide is describing a harder version of something that got
easier. A draft exists at
`~/scratch/jbrowse-pangenome/jbrowse-visualization-contribution.md` and carries
its own check-before-sending list. Nothing has been sent, and sending it is a
decision about representing the project rather than an implementation task.
...
- **A chromosome-scale band, config only.** A `LinearWiggleDisplay` on the
  ...
  a visible spike.
- **Still open from the CHM13 figure:** the synteny launch (two openable
  contributors is now true, and `jbrowse.org/ucsc/hg38/liftOver/hg38ToHs1.over.pif.gz`
  is hosted and used by `test_data/hg38_hs1_synteny`), and highlight-into-the-donor
  view. Neither has a figure yet.
