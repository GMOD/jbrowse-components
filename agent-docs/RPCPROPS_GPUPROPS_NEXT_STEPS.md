...
Also not migrated: `MultiLGVSyntenyDisplay`. It has `rpcProps` +
`SettingsInvalidate` but its upload/draw still uses raw hand-written autoruns
rather than `startMultiRegionGpuLifecycle` — a separate migration from this one.
