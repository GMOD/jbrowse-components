...
- `SvgCanvas.transformRect` — negative scale anchored rects on the wrong corner.
  Latent (nothing scales negatively today), and only fixed for `rotation === 0`
  — a mirrored *and* rotated rect still lands wrong. Don't reach for a negative
  ctx scale to mirror something that also rotates.
- **hic** reversed regions, mixed orientations included — the worker mirrors each
  contact within its own region's span (`regionOffsets.ts` `mirrorUInRegion`), so
  the renderers and `computeRenderTransform` stay forward-only. **Mirror the data
  upstream, once** is the shape to copy for a rotated render: it keeps both
  backends and the SVG export correct for free.
