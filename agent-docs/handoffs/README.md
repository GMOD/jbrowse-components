...
| [grammar-layer-zoom-and-rows](grammar-layer-zoom-and-rows.md) | The planned next phase of the grammar mark layer after ADR-125 (2026-09-16), in order — a measured comparison of the wiggle display and the mark display over one BigWig, now that both read one tier; a row lane on bar and point so MultiQuantitativeTrack is a marks config with no new shape; the price of encoding a non-drawing layer; and line as the first new shape, with wiggle as its second consumer. Read before adding a mark shape, attaching the mark display to another track type, or touching what a layer encodes off-screen. |
