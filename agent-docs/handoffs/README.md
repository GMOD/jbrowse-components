...
| --- | --- |
| [arc-foot-bound-and-the-y-offset-band](arc-foot-bound-and-the-y-offset-band.md) | The track-y-offset label-box research is done and refutes its own idea doc's recommended fix, so that thread waits on an approach decision, not on more digging. The breakend-foot region bound that used to share this file landed. |
