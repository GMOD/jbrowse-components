...
| --- | --- |
| [config-write-surface](config-write-surface.md) | The configuration write API — `setConf` takes the path `getConf` takes and a config node's props come off the schema definition, both landed. Two questions are left open, and the first of them needs Colin rather than an experiment. |
