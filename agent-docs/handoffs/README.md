...
| --- | --- |
| [arc-foot-bound-and-the-y-offset-band](arc-foot-bound-and-the-y-offset-band.md) | The breakend-foot region bound is written but UNCOMMITTED in the worktree with one bad import, so the tree neither builds nor reverts cleanly — it needs a one-line import fix and the checks below. Beside it, the track-y-offset label-box research is done and refutes its own idea doc's recommended fix, so that thread waits on an approach decision, not on more digging. |
