...
| --- | --- |
| [alignments-misc](alignments_misc.md) | Alignments items still open after the modification-extract round: an MN-length check for hard-clipped supplementaries, one pileup layout for single and multi-region, and whether arcs inherit the read colour field. |
| [arabidopsis](arabidopsis.md) | Pointer to the pangenome work on the 1001-genomes demo. |
...
| [identity-coloring](identity_coloring.md) | After the identity-colouring round: figures to re-shoot, demo configs to redeploy, and whether an identity scale with labels replaces the multi-row legend slot for itemRgb tracks. |
| [introgression](introgression.md) | Pointer to the wheat introgressions work in the wheat-introgressions-2 worktree. |
| [inversion](inversion.md) | Proposal to read an inversion straight by splitting the mate row at the breakpoints and reversing only the inverted span. |
| [json-addtrack](json_addtrack.md) | Improvements to the figure-recipe dialog and the addtrack fences' tabs, after the four bugs were fixed. Unresolved figure track ids and remembering the reader's tab would do the most for readers; none is started. |
| [mark](mark.md) | Report from the mark row-model round: fixes for review findings 1–7 (wiggle shorthand move, shared binColumns, MarkGetRowSources RPC), what a mark display over a multi-BigWig draws, and where the code disagreed with the brief. |
| [mstfork](mstfork.md) | The half-done MST fork linear-array reconcile fix in ~/src/mst-linear-array-reconcile, and what resuming it needs. |
| [multirow](multirow.md) | Report from the MAF GPU port, the shared colour-key rule and GPU row table stage 1, with the next steps in order. |
...
| [restack](restack.md) | Report from the video and tutorial follow-up batch: the re-filmed tours, the retired gallery thumbnails, and the one video whose story still needs rewriting. |
| [review](review.md) | Brief for reviewing the link-mark round: the size channel, the x2 mate form, the mate transform step and render-core linkMark. |
...
| [search](search.md) | Text search round: hub (connection) track search, BigBedTextSearchAdapter for UCSC hub gene search on branch worktree-text-search-improvements, and what was left undone. |
...
| [wheel](wheel.md) | Report from the wheel-zoom fix: eased notches, the full requested zoom at any spin speed, and a 1.8x Ctrl+wheel notch. |
