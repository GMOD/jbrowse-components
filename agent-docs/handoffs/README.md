...
| [canvas-plugin-simplifications](canvas-plugin-simplifications.md) | A 2026-09-04 read of every non-test file in plugins/canvas for simplifications, refactors and bugs. The worker-side dedupes landed on canvas-simplify (one commit, green on typecheck and all 140 canvas suites); the main-thread list below is what the read found and did not get to, led by the layout memo's hand-kept cache-key list, which is the one item that is a latent-bug class rather than a tidy-up. No confirmed bug was found. |
