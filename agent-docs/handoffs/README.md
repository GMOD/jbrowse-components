...
| --- | --- |
| [derivative-allele-thread](DERIVATIVE_ALLELE_THREAD.md) | Live state of the multihop / derivative-allele review thread — what landed 2026-08-25, the one check left red by someone else, and the two things still open. Delete once check-docs is green and the pluggable-element lineage question is answered. |
| [oom-recovery-2026-08-25](OOM_RECOVERY_2026-08-25.md) | A multi-agent run on 2026-08-25 died to an out-of-memory kill around 11:40. Nothing was lost — seven branches hold 21 unlanded commits across six live worktrees, and the four worktrees that still held uncommitted work at the crash are committed in place. This is the index of those threads, what each is waiting on, which seven worktrees are safe to remove, and the three checks the run left red on main (fixed here). Delete once the seven branches have landed or been declined. |
