...
| [multiway-gpu-backend](multiway-gpu-backend.md) | MultiWaySyntenyDisplay is moving from React+SVG to the GPU→WebGL→Canvas2D render path. The model half landed 2026-08-26 — lane frames are settle-time decisions with incumbents, and a pan is one translate — so the display already draws under the data × view-transform contract a backend needs. What is left is the backend itself, and this file says which existing stack draws the ribbons unchanged, what the gene lanes need that nothing draws yet, and the order to take it. |
