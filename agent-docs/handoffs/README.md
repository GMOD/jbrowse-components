...
| --- | --- |
| [shape-library-audit-remainder](shape-library-audit-remainder.md) | What the 2026-08-29 eight-way audit of the shader/shape library and the surrounding 100 commits found and nobody has committed to yet — two subagent branches whose work was never read back, six confirmed defects with file:line and a failure scenario each, and one colour unification that is a judgement call because it moves rendered output. Waiting on someone to pick the branches up or re-derive them, and on a decision about the synteny viridis. |
