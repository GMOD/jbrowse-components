...
| [categorical-channel-review-followups](categorical-channel-review-followups.md) | What a 2026-09-18 review of ADR-131's stage 2 and 3 found and left open — a settings editor write that saves an unloadable colour object, displayDefaults routing a key to displays whose colour objects differ, scale switches that drop the authored field, four colour objects on three normalizers, and smaller API and docs loose ends. Read before touching the facet or colour sub-schemas, displayDefaults routing, or the config editor's sub-schema path. |
