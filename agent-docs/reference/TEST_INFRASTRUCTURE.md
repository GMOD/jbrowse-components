...
selects nothing. A change under `config/jest/`, `jest.config.js`,
`babel.config.cjs` or `pnpm-lock.yaml` runs everything, since none of those is in
a footprint.
...
suites. That is why test-related used to leave `products/jbrowse-web` out
wholesale, and three suites went red on main in one week from it
(`ConfigSlotDefaults`, `AlignmentsFilters`, `ReversedRegionLabels`). Each of the
three executes the module whose change broke it, so a footprint selects it.

Measured over the 263 recent commits that touched source, summing each selected
suite's wall time:
...
| selection                  | median suite-seconds | mean suite-seconds | commits at ≤30s | commits over 300s | skips web suites it reaches |
| -------------------------- | -------------------: | -----------------: | --------------: | ----------------: | --------------------------- |
| static                     |                 772s |               616s |              74 |               183 | no                          |
| static, no web             |                 195s |               222s |              74 |                73 | yes                         |
| footprint                  |                 142s |               441s |             113 |               121 | no                          |
| footprint + compile filter |              **38s** |               376s |             131 |               105 | no                          |

<!-- END GENERATED MEASUREMENT test-selection-strategies -->

The mean stays high because the distribution is bimodal. About 40% of commits
touch a module every app-level suite executes at import — render-core's marks,
a plugin's `index.ts`, a config schema — and those still select most of the
~380 suites that carry the app graph. Narrowing that half means making those
modules lazy, not selecting harder: function-level selection off V8 precise
coverage added ~27% to every run and moved the mean from 441 to 415, because
most changed lines in those modules are top-level.
