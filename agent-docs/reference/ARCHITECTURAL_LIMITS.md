...
### The uniform ring is allocated at 2048 slots whatever a frame uses

**Status:** Open, and ranked behind the MSAA target on the same measurement.

`WebGPUHal` allocates `MAX_UNIFORM_SLOTS` (2048) ring slots per display at
construction, as a GPU buffer and as a `Uint8Array` staging copy, each slot
sized by the largest uniform struct a registered pass declares. Alignments'
640-byte struct aligns to 768, so the ring costs 1.57 MB each side per
alignments display, and a frame uses
4<!--m:uniform-ring-occupancy.6-alignments-tracks-dpr-2.peakSlots--> slots.
`products/jbrowse-web/browser-tests/probe-uniform-ring.ts` takes the table by
wrapping the page's `createBuffer`, `createTexture` and `queue.writeBuffer`:
...
**The slot count is the oversized term, not the slot size.** Packing
`readCategoryColor` from `float4[23]` to `uint4[6]` takes the struct to 368
bytes and the slot to 512; it compiles on both backends (`colorPack.slang` says
which form segfaults) and was declined, since it saves
6.29MB<!--m:uniform-ring-occupancy.6-alignments-tracks-dpr-2.packSaves--> across
six tracks and gives up the generated `setUniformReadCategoryColor` for
hand-written word arithmetic. Sizing the ring to what a frame uses recovers
nearly all of it with no shader change, but only as a growth path — overrunning
the cap drops the write and its draws read another batch's uniforms, with no
error — so recreate the buffer and every bind group in `passBindGroups` rather
than lowering the constant. The worst case is unmeasured: a grouped view
multiplies the writes by up to `MAX_GROUPS` (40) and a multi-region view by the
block count, and the 4 above is two groups and two blocks.

**Retire when** a real session is shown to be short of GPU bytes and the MSAA
target, at
223.31MB<!--m:uniform-ring-occupancy.6-alignments-tracks-dpr-2.msaa--> in the
same session, has been dealt with first.
...
`getLDMatrixFromPlink` lays out every pair the file names in the viewport as
one `Float32Array` of `n*(n-1)/2` cells, which `RenderLDData` transfers whole to
...
returning wrong r² on real hardware. The estimator went — plink does it, and
does it right — and those limits went with it. What remains is the layout, and
