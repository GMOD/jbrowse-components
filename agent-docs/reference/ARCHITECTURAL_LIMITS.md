...
**Status:** Accepted on the cap. The size it cost is retired (2026-09-26).

`WebGPUHal` gives each display a uniform ring, a GPU buffer and a `Uint8Array`
staging copy with one slot per `writeUniforms` in a frame, each slot sized by
the largest uniform struct a registered pass declares. It starts at
`INITIAL_UNIFORM_SLOTS` (16) and doubles mid-frame when a frame writes more: the
draws already encoded bind the outgrown buffer, which takes the slots staged so
far at submit and is released after it (`webgpuHalUniformRing.test.ts`). Past
`MAX_UNIFORM_SLOTS` (2048) a write is dropped and its draws read another
batch's uniforms, with no error; `UNIFORM_SLOT_WARN_AT` warns at half.

The ring used to be allocated at all 2048 slots on construction. Measured that
way, alignments' 640-byte struct aligned to 768 and cost 1.57 MB each side per
display, while a frame used
...
**The slot count was the oversized term, not the slot size**, which is why the
ring grows rather than packing a struct: `readCategoryColor` from `float4[23]`
to `uint4[6]` would have saved
...
**The worst case is unmeasured**: a grouped view multiplies the writes by up to
`MAX_GROUPS` (40) and a multi-region view by the block count, and the band adds
three per region feed and three per block for each section.

**Retire when** a frame's write count is bounded below the cap by construction,
or reaching the cap fails loudly rather than drawing wrong pixels.
