...
| [r-export](R_EXPORT.md) | The LGV "Export R script" exporter — exportR.ts HELPERS/assembleRScript/RTrackFragment architecture, the add-a-track recipe, prioritized next steps, the Rscript verification technique, and the read_index/NA-row gotcha. Read when extending the R script exporter. |
