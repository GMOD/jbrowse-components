...
- **Fold `PileupMark` into render-core's mark shapes** (`spanMark`, a `cell`
...
- **Convert arc, the single-sample variant display, GC content or the synteny
  displays onto render-core's mark shapes** — probed 2026-09-05, one read-only
...
- **Seven mark-system proposals from the 2026-09-05 render-core review** —
  ...
  *`MarkShape.antialiased` as a boolean on the shape*: a shader property one
  ...
  [INTERACTION_PERF.md](INTERACTION_PERF.md) measured that any `fillText`
...
- **Memoize the multi-way lane ticks per lane, so a change in one lane stops
  rebuilding every lane's ticks** — measured and declined 2026-09-05, from the
...
- **Cue's four SV channels as `.hic` contact tracks, and the tutorial built on
  them** — the tutorial, its two build scripts under scripts/, its demo config
...
- **`AlignmentsContactAdapter`, computing those channels from a BAM/CRAM in a
  worker so a Hi-C track needs no precomputed `.hic`** — written, tested and
...
- **Move the pure-logic suites off jsdom onto the `node` test environment** —
  ...
  pass in [TEST_INFRASTRUCTURE.md](TEST_INFRASTRUCTURE.md). It rests on jsdom
...
- **Consolidating the three implementations of "has JBrowse finished
  rendering"** — proposed 2026-09-01 after `jb.waitReady` made a third, and
