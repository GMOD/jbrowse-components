...
counts in that order, so the multi-row and multiway synteny displays' `domain`
...
| Bands by a field                   | `groupOTU`, then a facet                           | —                                                      | the variant displays' `facet: { field, domain }`, resolved over `layout` when the rows are read — so a cross-band drag snaps back, and the band yields while a cluster tree describes the rows, as the multi-row display's `rowGroups` partition does |
