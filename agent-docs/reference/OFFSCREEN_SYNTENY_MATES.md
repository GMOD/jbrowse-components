...
of the row it was pointing at. A class C hit carries `mateCumBp`, where the
facing row draws those alignments, and its click `centerAt`s there instead.
...
**A contig displayed several times over** resolves to the copy the drawn span
sits in: `drawnPlacement` reads the region index off `pxToBp`, and both
`centerAt` and `flyToCenter` take it.
