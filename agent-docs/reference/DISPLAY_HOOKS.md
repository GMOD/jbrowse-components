...
| `paintInert` | same, for a paint that is never coming — both fetch families fill it with `foundationPaintInert` (a failed fetch, a standing user cancel, an empty viewport), so a display outside them owes its own | `display-kit`, `dotplot-view/DotplotView` |
