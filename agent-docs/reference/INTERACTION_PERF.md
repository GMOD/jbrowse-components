...
**One clause of that verdict expired when sashimi arcs stopped being
default-off, and was fixed on 2026-08-30.** `showSashimiArcs` is a promotable
slot with `promotedBase: true` (`configSchema.ts:571`), so it draws wherever
coverage does, and `sashimiArcSections` read `view.visibleRegions` — a fresh
...
a list of nulls. It hands back one shared empty array now:
`ZoomRenderCensus.test.tsx`'s alignments arm measures **21 renders over 20 zoom
frames → 0**, taking that view's whole observer count from 21.6 to 20.6 per
frame for one track. `crossRegionArcSections` had the identical defect, found by
...
subtree logs **64 `attr:d` patches and zero structural mutations** — the arc
paths are keyed by `sashimiArcKey`, so React pools them and patches one
attribute per arc per frame. That is the shape `ScalebarCoordinateLabels` was
*fixed into* below, not the shape it was fixed from.
