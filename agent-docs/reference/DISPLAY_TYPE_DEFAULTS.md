...
| Adopters: `featureHeight` / `heightMode` / `colorBy` / `mismatchAlpha` / `linkedReads` / `readConnections` / `readConnectionsDown` / `showSashimiArcs` / `sashimiArcsMode` / `showSashimiLabels` / `hideNonCanonicalJunctions` / `showSoftClipping` / `showLegend` | `plugins/alignments/src/LinearAlignmentsDisplay/{configSchema,model}.ts` |
