...
through `clearAllRpcData → cancelFetch → controller.abort()` — was fired by hand
80ms into the octave refetch to answer the second half of the question. The
`token stopped at` column is that 80ms plus the check's own latency:
...
**It is not clean, and it is bounded.** Measured before ADR-122, when the
extract consulted the stop through a checker gated at 50 ms backing off to
500 ms, so a fetch of a few hundred ms stopped inside the gate window ran to
completion and resolved with data the main thread dropped in `ctx.isStale()`.
Three of the four rows overran, by 74–217ms of worker time. The gate is gone:
the abort now lands at the extract's next yield and `report()` reads the signal
on every call. Not re-measured since; nothing here changes with the bin.
