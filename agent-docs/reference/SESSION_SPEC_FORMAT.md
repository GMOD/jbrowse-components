...
| explicit display types named           |    18 |
| distinct keys on object entries        |    89 |
| keys on two or more display types      |    27 |
...
| facet               |      18 |             3 |
| summaryScoreMode    |      17 |             2 |
| jexlFiltersSetting  |      16 |             2 |
| scatterPointSize    |      15 |             3 |
| domain              |      13 |             2 |
| showLegend          |      13 |             3 |
| runClustering       |      13 |             5 |
| displayCrossHatches |      11 |             2 |
| showTree            |      10 |             3 |
| subtreeFilter       |       9 |             2 |
| layout              |       9 |             2 |
| resolution          |       8 |             2 |
...
`color`, `colorBy`, `groupBy`, a y-scale spelled `scales`, a mark spelled
`defaultRendering`, a filter spelled `jexlFilters`, and row order spelled
`runClustering` / `sortRowsBy` / `domain`. The concepts are shared and the
census shows authors using them as shared.

`scales` is one key on four display types because every quantitative display
