...
| -------------------------------------- | ----: |
| session specs                          |   362 |
| views, nested ones included            |   576 |
| session tracks carried inline          |   176 |
| track entries: bare id                 |   105 |
| track entries: tuple                   |    89 |
| track entries: object                  |   572 |
| explicit display types named           |    17 |
| distinct keys on object entries        |    86 |
| keys on two or more display types      |    22 |
| keys used exactly once                 |    19 |
| keys per object entry, median          |     2 |
| keys per object entry, 90th percentile |     4 |
...
90th percentile is four. The largest entries in the corpus are alignments
...
corpus writes, about a quarter appear on two or more display types, and that
quarter is exactly the set a grammar would call encoding and scale: `height`,
`color`, `colorBy`, `groupBy`, a y-scale spelled `scales`, a filter spelled
`jexlFilters`, and row order spelled `runClustering` / `sortRowsBy` / `domain`.
The concepts are shared and the census shows authors using them as shared.

`scales` is one key on three display types because every quantitative display
...
- *First-class `Scale` objects, across the displays.* **Taken**: every
  quantitative display writes `scales.y`, built by one factory in
  `@jbrowse/wiggle-core` with the members that display draws
  ([ADR-142](../architecture-decision-records/adr-142-one-value-scale-object.md),
  generalising the mark display's
  [ADR-141](../architecture-decision-records/adr-141-one-y-scale-the-displays.md)).
  The declined half was a fixed object shared by all of them, which puts an
  `autoscale` on Manhattan and a `symlog` type where no shader places one.
...
inline; a figure's spec is typically under twenty lines; and 380 of the site's
