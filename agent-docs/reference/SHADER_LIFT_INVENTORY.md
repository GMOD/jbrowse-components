...
Scanned 40 shaders with entry points. 68 functions
are inside the emitter's subset, of which **49 are exported**.
...
| type 'vec3' is outside the supported scalar subset | 12 | `arcColor`, `arcColorByIndex`, `arcMarkerColorByIndex`, `baseColor`, `bpRange`, `categoryPaletteColor`, … |
