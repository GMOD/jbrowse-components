...
`Gff3TabixAdapter` and `GtfTabixAdapter` are absent on purpose too. The
library's chunk-size table measures GFF text at 0.43-1.18x across every chunk
size, because reassembling text that inflates ~18x is serial work on the calling
thread that no worker count reaches (`@gmod/bgzf-filehandle`
docs/worker-pool.md). The pool bought them four workers per RPC worker and
nothing else.

## What the pool does since `@gmod/bgzf-filehandle` 6.7

- **It starts no workers until a chunk of at least `POOL_MIN_DECOMPRESSED_BYTES`
  (0.4MB inflated) reaches it.** Smaller chunks inflate on the calling thread,
  where the library measured most two- to four-block chunks losing to the round
  trip. A session whose views only ever read small chunks never boots one.
- **No read waits on a worker.** While workers boot, reads inflate in process.
  A worker that raises `error` or sends an undeserializable reply turns the pool
  off for good, and its in-flight request goes back to the calling thread.
- **A worker that never starts is left alone**, and adopted if it starts later.

Before 6.7 the first read awaited all four workers' `ready`, with nothing
listening for `error`, so one worker that never booted left every tabix and BAM
read in that RPC worker pending for the life of the page — a track stuck at
"Downloading features 0%". Capping Firefox's `dom.workers.maxPerDomain` at 4
reproduces it on a volvox GFF3 tabix track; the library's docs have the
Firefox bug that rules out giving up on a worker by terminating it.
...
- Since 6.7 the pool also stays down until a chunk of 0.4MB inflated arrives,
  so the check needs a view that reads one — the 1kg VCF above does, volvox does
  not, and zero `blob:` workers on volvox is the pool working.
...
`unzipChunkSlice` also declines the pool outright for a chunk under
`POOL_MIN_DECOMPRESSED_BYTES` (0.4MB inflated) — so a fixture small enough to
have only such chunks measures nothing, silently.
