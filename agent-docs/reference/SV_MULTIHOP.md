...
sorted, then swept, opening a new cluster when the endpoint is more than the
tolerance from the one that OPENED the current cluster. Three properties in
`computePaths.test.ts` pin it, each sweeping the offset rather than testing one
pair, plus a translation-invariance check over the real COLO829 records.

**The cluster rule is a leader sweep, and that is load-bearing.** Linking each
endpoint to the PREVIOUS one instead — single linkage, which is what
`sv_multihop.py` does for its own dedup — lets clusters chain, and this data has
a real case: COLO829's two chr9 fold-back junctions sit **28 bp apart**, and
jittered reads between them bridge the two into one cluster, merging two alleles
into one candidate. `sv_multihop.py` has the same exposure on any callset with a
re-break that close; it has not been checked.
