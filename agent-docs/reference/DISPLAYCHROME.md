...
15 display types are registered for `LinearGenomeView`: 15 on `DisplayChrome`. On export, all 15 reach `SvgChrome`.
...
`data-display-id`. The GC content display and the wiggle display are one such
pair.
...
component and inherits the alignments export; `LinearVariantDisplay` and the GC
content display likewise. A display splitting its own model across
...
`painted` on its model for the same reason its `displayPhase` lives there — a
