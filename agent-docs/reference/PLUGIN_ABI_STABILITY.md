...
- **A shader's text is reached through `SOURCE`, and a pass carries a loader
  ...
  the strings and no `SOURCE`, and `slangPass` throws on it when the plugin
  declares the pass, naming the regeneration. Without that check each GPU rung
  threw on the missing `source` and the display drew on Canvas2D, slower and
  otherwise unexplained. No
