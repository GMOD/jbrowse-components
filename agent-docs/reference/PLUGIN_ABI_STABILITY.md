...
`reExports.generated.json`, and spliced into
...
17 subpaths the published `exports` map no longer serves, against what 4.3.0 published.
...
None of this is checked against a published bundle: the removals table and
`scripts/check-published-plugins.ts` both work from module exports, so neither
reaches a plugin `exports` object or the session. The script does also evaluate
...
- **A shader's text is reached through `SOURCE`, and a pass carries a loader
  rather than three strings** (`@jbrowse/render-core/slangPass`, `/hal`,
  `/computePipeline`). `ShaderModule` takes `SOURCE` where it took
  `WGSL_SOURCE`, `GLSL_VERTEX` and `GLSL_FRAGMENT`; `PipelineDescriptor` takes
  `source` where it took `wgslSource`, `glslVertex` and `glslFragment`;
  `makeComputePipelineCache` takes the kernel's `SOURCE` where it took the WGSL
  string; `WebGL2Hal`'s constructor is private and `WebGL2Hal.create` loads the
  GLSL and builds one; and the served `shaders/coverage*` subpaths export
  `SOURCE` in place of the strings. A shader regenerated by
  `jbrowse-build-shaders` has the new shape, so a plugin that rebuilds passes
  its module to the host's `slangPass` as before. One generated earlier carries
  the strings and no `SOURCE`, and that fails quietly: the descriptor's `source`
  is undefined, each GPU rung the ladder reaches throws on it, and the display
  draws on Canvas2D with a console warning per rung, slower and otherwise
  unexplained. No
  plugin checkout in `~/src/jb2plugins` builds a shader on 2026-09-21; the one
  that does is `example-plugins/score-example`, regenerated in the same commit.
  `MOCK_SHADER_SOURCE` joins the `hal` subpath for a pass built only for a
  `MockHal`. **Opt-out: none; regenerate the shader.**

- **A pass's WebGPU bind-group layout is built from its shader's `BINDINGS`**
  (`@jbrowse/render-core/hal`, `/slangPass`). Each `ShaderBinding` carries
  `stages`, the entry points that read it, and the layout shows the binding to
  exactly those, where the HAL used to build two fixed layouts whatever the
  shader read. `ShaderModule.BINDINGS` and `PipelineDescriptor.bindings` are
  required, and `TextureBinding` no longer carries `textureBinding` /
  `samplerBinding`, whose indices are in `bindings`. A shader regenerated by `jbrowse-build-shaders` carries both; one
  generated earlier has no `stages`, and the WebGPU rung throws building its
  layout, so the display draws on WebGL2 with a console warning. **Opt-out:
  none; regenerate the shader.**
