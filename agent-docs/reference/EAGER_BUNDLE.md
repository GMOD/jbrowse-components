...
Two consequences for callers, both of which fail loudly rather than silently:

- **Showing a track is async.** `launchTrack`/`launchToggleTrack` load the picked
  display's model and then show it; the synchronous `showTrack` throws a message
  naming them. Every user-driven path goes through the async pair.
- **A snapshot naming a type must be preloaded.** `await
  ...
  `tracks[].displays[]`, resolving display aliases to their registered type.
