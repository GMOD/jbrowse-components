---
name: eager-bundle
description: What every JBrowse host downloads and evaluates before it can run, why plugin registration makes most of it unavoidable, and the measured census of what still holds Material UI there. Read before touching a plugin `exports` object or a state model's imports.
kind: measurement
---
...
**Grouping would not move first paint either, measured.** Over a throttled
network, pinned to WebGL2, an alignments display's GLSL arrives seconds before
its data, and first paint waits on the data:

<!-- BEGIN GENERATED MEASUREMENT shader-text-first-paint -->

_Generated by `pnpm autogen` — edit the source, not this block._

| network | GLSL chunks | last GLSL chunk | last data request | first paint |
| ------- | ----------: | --------------: | ----------------: | ----------: |
| Fast 4G |          18 |          6697ms |            9729ms |      9903ms |
| Slow 4G |          18 |         27277ms |           38408ms |     38553ms |

<!-- END GENERATED MEASUREMENT shader-text-first-paint -->
...
<!-- BEGIN GENERATED MEASUREMENT config-schema-construction -->

_Generated by `pnpm autogen` — edit the source, not this block._

| phase                  |      ms | MST types built | config slot types | distinct slot types |
| ---------------------- | ------: | --------------: | ----------------: | ------------------: |
| module eval            |       — |           1,147 |               385 |                   — |
| plugin registration    |  32.7ms |           2,267 |               812 |                   — |
| 262 track config nodes | 159.4ms |               1 |                 0 |                   — |
| all three              |       — |           3,415 |             1,197 |                 222 |

<!-- END GENERATED MEASUREMENT config-schema-construction -->

A config slot is a `stripDefault(union(JexlString, valueModel), default)` pair,
and most of those pairs are duplicates of one another:
1,197<!--m:config-schema-construction.all-three.slots--> slot types are built for
222<!--m:config-schema-construction.all-three.distinct--> distinct ones, because
`createBaseTrackConfig(pluginManager)` runs once per track type and rebuilds the
~35 base slots each time. Interning `ConfigSlot` on
`(type, model, defaultValue)` does remove them — prototyped, it cuts the types
built to 1464 — and registration does not move: 30.8 ms against 30.2 ms, median
of seven runs either way, and the heap after it reads 145.5 MB in both arms. The
ceiling on that fix is the
32.7ms<!--m:config-schema-construction.plugin-registration.ms--> the whole phase
takes.

The third row is where config time actually goes, and it is not schema
construction: instantiating 262 track config nodes costs five times the whole
registration phase and builds one type between them. Schema construction is per
track *type*, node instantiation is per track, and `configSchemaCost.test.ts`
pins that by creating 262 and 524 and checking the second builds no more types
than the first.

**In a production build the per-track cost holds and scales linearly.** The
same snapshots created in headless Chrome, through the app's own track union:

<!-- BEGIN GENERATED MEASUREMENT config-nodes-in-browser -->

_Generated by `pnpm autogen` — edit the source, not this block._

| track config nodes |      ms | ms per track |
| -----------------: | ------: | -----------: |
|                262 |  61.8ms |       0.24ms |
|              2,620 | 669.1ms |       0.26ms |

<!-- END GENERATED MEASUREMENT config-nodes-in-browser -->

A hub-sized config of a few thousand tracks therefore spends most of a second
of main-thread time on config nodes before anything draws. That is the number to
weigh if config time is ever chased: registration is paid once per plugin set,
and this once per track.

The MST fork's own profile counts ~20k unions per session load. That is not this
workload — a session builds
