...
- [TAFFY_INDEX_GAPS_HANDOFF.md](guides/TAFFY_INDEX_GAPS_HANDOFF.md), taffy's `.tai` indexes a block by row 0 only, so a second reference row is unreachable (0.038% of the pggb demo). Minimal reproducer inside. Read before "fixing" `reroot_maf.py`
