...
- Remaining `colorBy` parity: **per-base quality**. Modifications/methylation
  (MM/ML) is done (`bam_modifications`/`mod_colors`, see table above) but doesn't
  reproduce bisulfite methylation (reference-dependent C→T, no MM tag), the
  5mC/5hmC winner-take-all collapse, or per-type
  `shownModifications`/`hiddenModifications` filtering (all types render; filter
  the `mm` data.frame in R). Also still: CIGAR insertion/deletion markers
  (deletions currently render as continuous aligned sequence because `read_bam`
  uses reference-based coords) and position sort (`sortedBy`).
