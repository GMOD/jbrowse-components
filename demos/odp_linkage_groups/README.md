...
Seven ortholog tables from the Schultz et al. 2023 Dryad deposit
...
The seven assemblies are `ChromSizesAdapter` over `<code>.chrom.sizes`, the
first two columns of `samtools faidx` on the deposit's FASTAs (and, for
Ephydatia, on the renamed bitbucket FASTA the build script fetches). The
sessions draw no sequence, so the hosted copy skips the 2 GB of FASTA the build
script's `add-assembly --load copy` writes.
