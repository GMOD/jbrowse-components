...
### A computed that rebuilds an array invalidates everything downstream

The lever above is how many components re-render per frame, and the commonest
way to add one is a MobX computed that hands back a fresh array of the same
content. The view rebuilds `visibleRegions` on every pan and zoom frame, so
anything deriving from it re-runs per frame whether or not the answer moved —
which is why the view publishes its content's right edge as a scalar and the
alignments display compares the region set structurally before merging junctions
against it.

The sashimi overlay is the worked example. Its splice-junction merge answers to
loaded data, the region set on screen and two filter settings; only the
projection to screen x answers to the pan. Folded into one computed, a gesture
paid for both and owed one. Split, at real RNA-seq junction counts:

<!-- BEGIN GENERATED MEASUREMENT sashimi-frame-split -->

| region copies | junction entries merged | merge + project (was) | project only (is) | control | of the frame the merge was |
| ------------: | ----------------------: | --------------------: | ----------------: | ------: | -------------------------: |
|             1 |                     651 |               0.516ms |           0.271ms | 0.530ms |                    **47%** |
|             3 |                   1,953 |               0.838ms |           0.236ms | 0.861ms |                    **72%** |
