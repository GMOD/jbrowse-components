...
| [`fadeThinAlignmentsMode`](/docs/models/linearsyntenyview#property-fadethinalignmentsmode) | Fade a sub-pixel-thin ribbon's opacity by its on-screen width, so an unfiltered whole-genome view doesn't read as a full-opacity hairball. 'auto' fades once a display is dominated by sub-pixel ribbons and leaves a sparse comparison at full alpha; 'on'/'off' pin it. Resolved view-wide by `fadeThinAlignments`. |
| [`followAnchorIndex`](/docs/models/linearsyntenyview#property-followanchorindex) | Which genome row drives the others while `followSynteny` is on. |
| [`followMatchOrientation`](/docs/models/linearsyntenyview#property-followmatchorientation) | While following, flip a row whose placing alignment runs the other way from the anchor's, so the two pan in the same direction. |
| [`followSynteny`](/docs/models/linearsyntenyview#property-followsynteny) | Move the non-anchor genome rows to whatever region aligns to the anchor row, re-resolved through the synteny data each time the anchor settles. Mutually exclusive with `linkViews`, which locks the rows in pixels and drifts as indels accumulate (see setRowSyncMode). |
| [`hideUnlabelled`](/docs/models/trackcolorsmixin#property-hideunlabelled) | Under a text-column mode, draw only the rows that carry a label. |
...
| [`minAlignmentLength`](/docs/models/linearsyntenyview#property-minalignmentlength) | Hide alignment blocks shorter than this many bp, which cuts whole-genome hairball noise. |
| [`minimized`](/docs/models/baseviewmodel#property-minimized) | collapse the view to its header bar, keeping it in the session rather than closing it |
