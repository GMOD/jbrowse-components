...
| [`detailConnectorHeight`](/docs/models/lineargenomeview#property-detailconnectorheight) | Height of the bands the trapezoids between this view's detail levels are drawn in, dragged by any one of them. One number for the stack: the bands are a ladder the eye reads down, and a rung of its own height reads as a difference in the data rather than in the drawing. |
| [`detailLevels`](/docs/models/lineargenomeview#property-detaillevels) | Closer views of the same locus stacked under the tracks, widest first, so the page zooms in as it reads down. Each is a LinearGenomeView with tracks of its own whose regions, width and centre this view drives; its window width is the one thing it keeps. See `detailLevels.ts`. |
