...
| `drawCurves` | Draw the ribbons as bezier curves rather than straight chords. Writes the `drawCurves` config slot on every synteny track the launch opens; omit it to leave each track at its configured value. |
