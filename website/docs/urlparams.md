...
| [`followAnchorIndex`](/docs/models/linearsyntenyview#property-followanchorindex) | Which genome row drives the others while `rowSync` is 'follow'. |
...
| [`rowSync`](/docs/models/linearsyntenyview#property-rowsync) | How the genome rows track each other. 'link' replays a pan or zoom of one row onto the others in pixels, which drifts as indels accumulate. 'follow' moves the non-anchor rows to whatever region aligns to the anchor row, re-resolved through the synteny data each time the anchor settles. |
