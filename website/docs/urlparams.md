...
| `drawCurves` | Draw the ribbons as bezier curves rather than straight chords. Writes the promotable `drawCurves` config slot on every synteny track this init opens; omit it to follow the viewer's session-wide default (straight when nothing is pinned). |
