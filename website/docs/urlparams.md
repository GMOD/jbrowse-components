...
| [`minimized`](/docs/models/baseviewmodel#property-minimized) | collapse the view to its header bar, keeping it in the session rather than closing it |
| [`offscreenMateMode`](/docs/models/linearsyntenyview#property-offscreenmatemode) | How much of what the view cannot draw a ribbon for it goes and finds. 'off' draws no marks. 'query' marks the upper panel of each pair from the fetch it already runs, so it is a repaint. 'both' also queries the file from the lower panel and marks it too, which is a fetch input and a second query per pair. |
