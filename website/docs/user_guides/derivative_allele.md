...
<Figure caption="Top: the reconstruction dialog over the COLO829 tumor pileup it was computed from, each route's segments drawn to scale under its read count. The top route's junctions are validated calls in the COLO829 truth set; the second route's two reads lack a piece the aligner did not place. Bottom: the linear synteny view the top route opens, hg38 above and the reconstructed allele below, one ribbon per segment." src="/img/cancer_sv/derivative_autogenerated.png" />
...
- **Look at the top two rows.** Where a published junction is recovered it is
  the first or second row in every comparison below. A route further down the
  list is unlikely to be the one you came for.
- **A low-count row that skips a piece of a higher one** is usually reads in
  which the aligner did not place that piece.
...
COLO829's ONT reads scored twice, against the ONT calls made from the same reads
and against the multi-platform truth set
([Valle-Inclán et al. 2022](https://doi.org/10.1016/j.xgen.2022.100139)), and
HG008-T against the C-GIAB draft benchmark
([Wagner et al. 2026](https://doi.org/10.64898/2026.05.01.722316)), at every
junction:

| Event size       | COLO829 ONT, vs ONT calls | COLO829 ONT, vs truth set | HG008-T HiFi, vs benchmark |
| ---------------- | ------------------------- | ------------------------- | -------------------------- |
| < 1 kb           | 1 / 9                     | 2 / 12                    | 4 / 40                     |
| 1 - 10 kb        | 6 / 10                    | 4 / 10                    | 17 / 26                    |
| 10 - 100 kb      | 16 / 16                   | 12 / 14                   | 17 / 18                    |
| > 100 kb         | 17 / 17                   | 14 / 16                   | 54 / 54                    |
| interchromosomal | 11 / 11                   | 9 / 13                    | 14 / 14                    |

The calls made from the same reads are the easiest comparison; the truth set is
the independent one, and most of the large events it holds that the reads do not
reach were seen only by short reads in that study. Where a junction is
