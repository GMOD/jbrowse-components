...
<Figure caption="Human (hg38) vs chimp (panTro6) at LMNB2 from a UCSC liftOver chain, with RepeatMasker on both genomes and track labels in 'offset' mode. Top: 'Colored indels' paints the human/chimp indels over the match ribbon (yellow insertions, blue deletion); they fall on Alu/L1/MER repeats. Bottom: 'Transparent indels' leaves the same indels as see-through gaps." src="/img/synteny_human_chimp_cigar_modes.png" links="Colored indels=synteny_human_chimp_colored,Transparent indels=synteny_human_chimp_transparent" />
