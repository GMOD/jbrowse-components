...
view for "how does this region compare across many genomes at once" — the kind
of multiz alignment UCSC publishes (e.g. the human 470-way or the _C. elegans_
26-way).

JBrowse reads three file formats — MAF (tabix-indexed BED), BigMaf, and bgzipped
TAF (taffy) — all opened as a single track. To add one, use **File → Open
track** and point it at the data, or load a track an administrator has already
...
per-species identity wiggle) — plus **Off**. By default it swaps in only when
you zoom out past base level, where individual bases are no longer legible;
uncheck **Auto-switch by zoom** to pin it on at every zoom level.
...
inversion — only a genuine intra-scaffold strand flip is marked. It is an
...
residue is drawn on each codon once you zoom in far enough to read it. This
answers "did the protein change here?" directly, across every species at once.
