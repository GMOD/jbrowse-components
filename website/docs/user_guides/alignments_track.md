...
so a gene's coverage lands entirely in one band and the other stays at the
floor.

<Figure caption="K562 PacBio Iso-Seq grouped by strand over the alpha-globin cluster. Every gene in frame but the last is transcribed off the forward strand, so the forward band carries the depth and the reverse band sits at the floor; LUC7L at the right edge runs the other way and is what the reverse band does carry." src="/img/alignments/strand_split_depth.png" />
