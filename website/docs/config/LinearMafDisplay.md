...
| <span id="slot-domain">**domain**</span><br>`stringArray` = <code>[]</code> | row order: the species listed come first, in this order, and the rest keep the tree's order. The guide tree is drawn only in its own leaf order, so a domain that moves a species hides the dendrogram until the row order is reset |
