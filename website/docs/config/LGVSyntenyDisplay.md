...
| <span class="slot-group">Inherited from [LinearAlignmentsDisplay](../linearalignmentsdisplay)</span> | <span class="slot-group-count">45 slots</span> |
...
| <span id="slot-symlogconstant">**symlogConstant**</span><br>[`number`](/docs/config_guides/slot_types#number) = <code>0</code> | Width of symlog's linear region around zero, in depth units. The default 0 means "derive from the domain" (a thousandth of the visible max). 1 makes symlog exactly log(depth+1)<br>_advanced_ |
