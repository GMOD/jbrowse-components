...
| <span id="slot-facet">**facet**</span><br><code>facetConfigSchema</code> | In-track stacked grouping, one labelled section per value: a read dimension (`strand`, `firstOfPairStrand`, `pairOrientation`, `splitRead`, `mapq`, `mateAssembly`), a tag (`tags.HP`), or any other field. `{ field: "tags.HP", domain: ["2", "1"] }` stacks the listed values first. See [Facet](Facet). |
