...
| <span id="slot-scales">**scales**</span><br>[ValueScale](../valuescale) | The coverage band's value scale — the one thing on this display drawn against an axis, the pileup having no value and the arcs their own `arcBandYScale`. `symlogConstant` starts at 1 rather than the wiggle family's 0, which makes symlog exactly `log(depth+1)`: the knee sits at one read, the smallest depth there is. |
...
| <span id="slot-arccolor">**arcColor**</span><br>[AlignmentsArcColor](../alignmentsarccolor) | What colours the read-connection arcs and the read cloud: `"pairOrientation"` paints them by pair orientation whatever the reads show, and `""` takes the reads' `color` field where an arc paints it. |
