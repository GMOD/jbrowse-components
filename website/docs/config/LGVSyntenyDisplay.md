...
| <span id="slot-groupby">**groupBy**</span><br>[`frozen`](/docs/config_guides/slot_types#frozen) = <code>null</code> | In-track stacked grouping, e.g. `{ type: "strand" }` to pre-group reads by strand, or `{ type: "tag", tag: "HP", domain: ["2", "1"] }` for one section per tag value with the listed values stacked first in that order (null = ungrouped)<br>_advanced_ |
