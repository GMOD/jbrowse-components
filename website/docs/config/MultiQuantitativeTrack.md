...
| Slot | Description |
| --- | --- |
| <span id="slot-displays">**displays**</span><br><code>multiQuantitativeDisplaySchemas(pluginManager)</code> | As on [every track](../basetrack#slot-displays), except that a `LinearWiggleDisplay` entry defaults to `rows: 'source'`, `summaryScoreMode: 'avg'` and `height: 200`. |
