...
| <span id="slot-domain">**domain**</span><br>`stringArray` = <code>[]</code> | the order a text column's labels take: the labels listed here first, the rest sorted; a label's colour is its position, so this moves the key and the alignments together; left empty the labels stay in the order the fetches first met them |
