...
| <span id="slot-color">**color**</span><br>[FeatureColor](../featurecolor) | The main fill of each feature: a CSS color or a jexl expression (`"goldenrod"`, `"jexl:…"`), or `{ field, domain, palette }` to paint each value of a field its own palette color, with a key. |
