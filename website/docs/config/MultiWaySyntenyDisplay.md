...
| <span id="slot-bridgeskippedlanes">**bridgeSkippedLanes**</span><br>[`boolean`](/docs/config_guides/slot_types#boolean) = <code>true</code> | join a group across a lane that places nothing for it, to the next lane down that does, at half the ribbon opacity. A ribbon otherwise joins adjacent lanes only, so a sparse lane mid-stack cuts every chain running through it |
