...
| <span id="slot-showlegend">**showLegend**</span><br>[`boolean`](/docs/config_guides/slot_types#boolean) = <code>true</code> | show the color key naming the anchor lane's drawn colors, and the ribbon strand colors where `ribbonColorBy` paints them. Derived from what is on screen, so it draws nothing until the `color` slot resolves to more than one color — a flat default color has nothing to key. Defaults to on |
