...
| <span id="slot-ribboncolor">**ribbonColor**</span><br>[RibbonColor](../ribboncolor) | `"rgba(130,130,130,0.3)"` paints every ribbon; `{ field: "strand" }` the record's strand, `{ field: "identity" }` a preset ramp and `{ field: "group" }` a declared column. See [RibbonColor](RibbonColor). |
