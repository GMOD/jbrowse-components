...
| <span id="slot-ribboncolor">**ribbonColor**</span><br><code>ribbonColorConfigSchema</code> | `"rgba(130,130,130,0.3)"` paints every ribbon; `{ scale: "strand" }` a synteny scheme; `{ field: "group" }` a declared column. See [RibbonColor](RibbonColor). |
