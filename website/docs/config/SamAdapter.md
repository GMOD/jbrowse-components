...
plugin.
[View source](https://github.com/GMOD/jbrowse-components/blob/main/plugins/alignments/src/SamAdapter/configSchema.ts).
...
`sample.sam` infers `SamAdapter` and `AlignmentsTrack` on its own, so the same
...
There is no index, so the whole file is loaded and held in memory — this is for
small files (a BLAT result, a handful of assembled contigs, a test case). Use
BAM or CRAM for anything sequencing-scale.
