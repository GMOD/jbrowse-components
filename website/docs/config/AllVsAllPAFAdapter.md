...
synteny rows so the reference sits between the others, use a complete all-vs-all
if the project publishes one, or view a larger cohort as a multiple alignment
...
`"adapter": { "type": "AllVsAllPAFAdapter", ... }`. It also accepts the
