...
| <span id="slot-colordomain">**colorDomain**</span><br>`stringArray` = <code>[]</code> | The order the colour field's values take, in the key and in the palette: the values listed here come first, the rest follow sorted. Left empty the values sort on their own, so a domain is how a scan names the tier order its readers expect. |
