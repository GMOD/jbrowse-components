...
| Slot | Description |
| --- | --- |
| <span id="slot-color">**color**</span><br>[ManhattanColor](../manhattancolor) | `"goldenrod"` or a `jexl:` callback paints every point; `{ field: "population" }` gives each value a palette colour with a key; `{ scale: "ld" }` colours by r² to the index SNP. See [ManhattanColor](ManhattanColor). |
