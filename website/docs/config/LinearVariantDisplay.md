...
| <span id="slot-showlegend">**showLegend**</span><br>[`maybeBoolean`](/docs/config_guides/slot_types#the-maybe-types) = <code>true</code> _promotable_ | show the display's color key when it has one (the `legend` slot, or a variant track's consequence-impact / SV-type presets). |
