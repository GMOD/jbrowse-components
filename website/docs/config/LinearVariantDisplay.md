...
| <span id="slot-groupby">**groupBy**</span><br>[`frozen`](/docs/config_guides/slot_types#frozen) = <code>null</code> | In-track stacked grouping: `{ type: "strand" }` packs forward-strand features into one labelled section above the reverse-strand ones, with unstranded features last. `null` (the default) is ungrouped.<br>_advanced_ |
