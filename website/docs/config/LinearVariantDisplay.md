...
| <span id="slot-facetfield">**facetField**</span><br>[`string`](/docs/config_guides/slot_types#string) = <code>''</code> | feature field (or jexl expression) to group by, one labelled section per value; `strand` for one per strand |
| <span id="slot-facetdomain">**facetDomain**</span><br>`stringArray` = <code>[]</code> | facetField values whose sections stack first, in order |
