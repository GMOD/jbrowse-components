...
| <span id="slot-indextype">**indexType**</span><br>[`stringEnum`](/docs/config_guides/slot_types#stringenum) (TBI, CSI) = <code>'TBI'</code> | index flavor for the entries this adapter derives, i.e. the ones `indexLocationMap` does not name. An index the map names says which kind it is by its `.csi`/`.tbi` extension, so one map can mix the two. `CSI` is required for a reference longer than 512 Mb, which TBI cannot address. |
