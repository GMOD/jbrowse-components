...
| <span class="slot-group">Inherited from [LinearWiggleDisplay](../linearwiggledisplay)</span> | <span class="slot-group-count">18 slots</span> |
...
| <span id="slot-scales">**scales**</span><br><code>wiggleValueScale()</code> | The y scale the plot is drawn through: `type`, `domainMin`, `domainMax`, `autoscale`, `numStdDev`, `numQuantile` and `symlogConstant`. |
| <span id="slot-showlegend">**showLegend**</span><br>[`boolean`](/docs/config_guides/slot_types#boolean) = <code>true</code> | Draw the score color ramp in density mode. Defaults to on |
