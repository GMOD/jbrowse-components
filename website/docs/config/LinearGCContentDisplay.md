...
| <span class="slot-group">Inherited from [LinearWiggleDisplay](../linearwiggledisplay)</span> | <span class="slot-group-count">21 slots</span> |
...
| <span id="slot-minbarheight">**minBarHeight**</span><br>[`number`](/docs/config_guides/slot_types#number) = <code>1</code> | Bar plot ("xyplot"/"multixyplot") only: shortest a bar may draw, in px, measured from the origin the bars pivot around. A bin whose score sits exactly on that origin otherwise draws a zero-height bar, painting nothing, so a covered stretch of zeros looks identical to a hole with no data. 0 restores that. Overlay multi-wiggle ignores it: every source shares one row there, so the floored bars would land on each other and only the last one drawn would show<br>_advanced_ |
