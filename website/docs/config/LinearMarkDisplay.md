...
| <span id="slot-marksencodingcolorscale">**marks.encoding.color.scale**</span><br>[`maybeStringEnum`](/docs/config_guides/slot_types#the-maybe-types) (none, categorical, linear, log) | How `field` becomes a colour. `categorical` hands out palette entries per distinct value; `linear` and `log` read the value through `domain` into `ramp`. `none` paints `value`, keeping a `field` for a switch back. Unset beside a `field`, it is `linear` with a `ramp` and `categorical` without. |
