...
| <span id="slot-marksencodingcolorscale">**marks.encoding.color.scale**</span><br>[`stringEnum`](/docs/config_guides/slot_types#stringenum) (none, categorical, linear, log) = <code>'none'</code> | How `field` becomes a colour. `categorical` hands out palette entries per distinct value; `linear` and `log` read the value through `domain` into `ramp`. `none` paints `value`. Left out beside a `field`, it is `linear` with a `ramp` and `categorical` without. |
...
| <span id="slot-marksencodingglyphscale">**marks.encoding.glyph.scale**</span><br>[`stringEnum`](/docs/config_guides/slot_types#stringenum) (none, categorical) = <code>'none'</code> | `categorical` hands a glyph from `range` to each distinct value of `field`; `none` draws `value`. Left out beside a `field`, it is `categorical`. |
...
| <span id="slot-marksfacetfield">**marks.facet.field**</span><br>[`string`](/docs/config_guides/slot_types#string) = <code>''</code> | The categorical feature field whose values each take their own band of rows, one section per value, named by a chip. The rows themselves come from a `stack` step grouped by the same field — the facet is the offset between one group's rows and the next, so a section stacks under the ones above it rather than over them. Sections order by `domain`, then the way every in-track grouping orders, digits by magnitude, and the tail past 40 merges into one. Empty for a display with no facet. Writing `facet: 'type'` directly on the mark lands here. |
| <span id="slot-marksfacetdomain">**marks.facet.domain**</span><br>`stringArray` = <code>[]</code> | Optional section order: the values listed stack first, in this order, and the rest follow sorted. Left off, the sections are every value the data holds, sorted. A listed value the data lacks takes no section. |
...
| <span id="slot-marksfacet">**marks.facet**</span><br><code>markFacetSchema</code> | The row facet: a field, or an object naming it, whose values each take their own band of rows with a chip over it. A reader hides a section from its chip. |
