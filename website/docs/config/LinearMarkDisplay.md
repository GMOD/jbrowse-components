...
| <span id="slot-scalesytype">**scales.y.type**</span><br>[`stringEnum`](/docs/config_guides/slot_types#stringenum) (linear, log) = <code>'linear'</code> | How the axis reads its domain. The ticks, the cross-hatches and the shader's placement all come from it. |
...
| <span id="slot-scalesy">**scales.y**</span><br><code>markValueScaleSchema</code> | The one value scale every mark's `encoding.y` is read through — the plot's, not a mark's, the way a grammar of graphics gives one scale per aesthetic. |
...
| <span id="slot-scales">**scales**</span><br><code>markScalesSchema</code> | The scales the marks are read through, owned by the display rather than by a mark: `y` alone, and every mark's `encoding.y` shares it. |
