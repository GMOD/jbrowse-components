...
| <span id="slot-markstransformas">**marks.transform.as**</span><br>`stringArray` = <code>[]</code> | The field a `formula` or `coverage` step writes, the two fields a `bin` step writes its edges to, or the field a `flatten` step writes each element's index to. A single name may be written as a string. |
