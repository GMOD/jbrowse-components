...
| <span id="slot-marks">**marks**</span><br><code>markListSchema([])</code> | The marks to draw, in order — a later one paints over an earlier one. Each is a `mark` and an `encoding`. |
