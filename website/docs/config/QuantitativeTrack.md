...
| <span id="slot-textsearchingtextsearchadapter">**textSearching.textSearchAdapter**</span><br><code>optionalTextSearchAdapter</code> | a per-track name search index, normally a `TrixTextSearchAdapter` over what `jbrowse text-index --tracks` built; `{ uri: 'genes.ix' }` is enough, searching this track's assemblies. Without one, this track's features are only findable through an assembly-wide search adapter. |
