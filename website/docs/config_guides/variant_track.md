...
- **Computed from a VCF** — add the `LDDisplay` to a regular `VariantTrack`.
  LD is computed on the fly from the genotypes in the visible region, so no
  extra files are needed. Filtering (minor allele frequency, HWE, call rate,
  jexl) and signed-LD values are available because the raw genotypes are
  present.
...
The adapter also accepts a `uri` shorthand (`"adapter": { "type":
"PlinkLDAdapter", "uri": "study.ld" }`), which expands to the `ldLocation` slot
above.
...
This adapter accepts a `uri` shorthand too; it assumes a sibling
`<uri>.tbi` index.
...
overlay, legend, layout), but the filtering and signed-LD options only appear
for the VCF-computed display, since pre-computed PLINK data carries only the
final R²/D' values. See the autogenerated
[LDTrack](/docs/config/ldtrack), [PlinkLDAdapter](/docs/config/plinkldadapter),
and [PlinkLDTabixAdapter](/docs/config/plinkldtabixadapter) docs for the full
slot reference.
