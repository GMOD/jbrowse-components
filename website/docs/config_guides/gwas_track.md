...
The Manhattan plot score is in **-log₁₀(p) units**. If your file stores raw
p-values instead, don't transform them by hand — set the adapter's
...
If you prefer to bake the transform into the file, add a `-log₁₀(p)` column:
...
`index.location` (defaults to a sibling `<file>.tbi`; set
...
`ldAdapter`), and `showSignificanceLines` / `genomeWideSignificance` /
`suggestiveSignificance` control the cutoff lines. See the autogenerated
