...
awk 'NR==1 {print $0"\tneg_log_pvalue"; next}
     {printf "%s\t%.6f\n", $0, -log($5)/log(10)}' results.tsv > results.scored.tsv
mv results.scored.tsv results.tsv
```

Then prefix the header so tabix skips it, sort the data rows by chromosome and
...
```json
{
  "adapter": {
    "type": "GWASAdapter",
    "bedGzLocation": {
      "uri": "https://yourhost/study.bed.gz"
    },
    "index": {
      "location": {
        "uri": "https://yourhost/study.bed.gz.tbi"
      }
    },
    "ldAdapter": {
