...
(`.bai` for a `.bam`, `.tbi` for a tabix `.gz`, etc.). Use the full slot form
(e.g. `bamLocation` + `index.location`) only when the index is named
differently, lives elsewhere, or is a `.csi` index, in which case also set
`index.indexType: "CSI"`.
...
\* Htsget support is limited. `HtsgetBamAdapter` is the only htsget adapter, and
it is not as well exercised as BAM/CRAM. Use a regular indexed BAM/CRAM file
where possible.
