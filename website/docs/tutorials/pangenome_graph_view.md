...
reference's coordinates, as synteny, variants, alignment or depth (the
[pggb tutorial](/docs/tutorials/pangenome_ecoli) builds all four). This one is
...
Everything below uses that five-strain _E. coli_ pangenome, so no step here
needs a genome you do not already have.

**Setup:** a separate plugin (details below), and the E. coli graph the pggb or
Minigraph-Cactus tutorial builds.
...
can query by strain. minigraph and `gfatools` are in the cactus image that the
[Minigraph-Cactus tutorial](/docs/tutorials/pangenome_cactus) uses:
...
- [Pangenome (pggb)](/docs/tutorials/pangenome_ecoli), which builds the graph
  these figures use and its four linear projections
- [Pangenome (HPRC)](/docs/tutorials/pangenome_hprc), the same view on a
  464-haplotype human graph
