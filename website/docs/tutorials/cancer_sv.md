...
<Figure caption="Top: the candidate list over the tumor pileup it was computed from, each row a route through the reference that COLO829 reads cross in the same order and orientation. Bottom: the synteny view that row draws, reference above and allele below, a ribbon per segment. The chr3 arm is most of the allele's length, so the two short inserts and the inverted chr3 return are the crossing ribbons at its right-hand end, named on the track along the allele. Under that, every read in the window placed on the allele's own axis: blue where a read follows the path from end to end, orange where it leaves, with the break drawn as the gap between its boxes. The gene track from the launching view comes along onto the reference panel." src="/img/cancer_sv/derivative_autogenerated.png" />
...
Under the allele are the reads themselves, placed on its axis. A path lays
reference intervals end to end, which is a map from reference coordinates onto
the allele, and a read's alignment is already a map from the read to the
reference; composing the two puts the read on the allele without an aligner and
without the allele's bases. A read whose alignments follow the path draws as one
unbroken bar, and one that leaves it draws as boxes with the jump between them,
colored by which it is. The track name counts the two.
...
instead of clipping at it, which is what the reads lane is for, and that the
segments land in the genes the event is supposed to involve. The segment sizes are the other check, and the reason this
...
<Figure caption="Top: the candidate list at the chr9 fold-back anchors. The path labels name the same chromosome twice, once inverted, which is the fold-back written out. Bottom: the top path drawn, with two windows of chr9 on the reference panel and the allele below. The ribbons cross instead of running parallel, which is what a segment returning inverted looks like, and the track under the allele names the interval each arm came from. The lane under that is the reads on the allele: a read crossing the turnaround lands as one blue bar spanning both arms, and the orange ones are reads whose chain goes somewhere this path does not." src="/img/cancer_sv/foldback_reconstruction.png" />
