...
searched for by hand: with the tumour reads open at a breakpoint, the alignments
track menu's `Reconstruct derivative allele...` groups the reads in view by the
path their split alignments describe and offers each path with the number of
reads that independently describe it.

<Figure caption="Top: the candidate list over the clipping it was computed from, each row a route through the reference that some number of COLO829 reads cross in the same order and orientation, ranked by that support. Two rows, because the junctions form a cycle and the reads enter it at either chr3 breakpoint. Bottom: the synteny view drawn by choosing the top row, with the reference loci and the gene track carried over on the upper panel and the path laid out along the lower one, a ribbon per segment. The crossed ribbon is the segment the reads traverse inverted." src="/img/cancer_sv/derivative_autogenerated.png" />

Two candidates come back rather than one, and that is the cycle showing up in
the reads. The three junctions close on themselves, so a read entering at one
chr3 breakpoint crosses chr10 before chr12 and a read entering at the other
crosses them the other way round. Both routes are real traversals of the same
junction set, and the second is the one `derive` reconstructs below. An allele
and its reverse complement are folded together before the counting, so a pair of
rows like this means two routes, not one route read from both ends.
...
support. Picking a supporting read out of the pileup and running
`Linear read vs ref` on it is then the single-molecule evidence behind one
candidate. Whatever else was open in the view comes along onto the reference
panel, so the path is read against the genes it runs through.

The lower panel has no sequence track. This reconstruction is the allele's
order and orientation, taken from where the reads' alignments start and stop; it
does not recover the bases at a junction. That is the next section.
