...
<Figure caption="Top: the candidate list over the tumor pileup it was computed from, each row a route through the reference that COLO829 reads cross in the same order and orientation. Bottom: the synteny view that row draws, reference above and allele below, a ribbon per segment. The chr3 arm is most of the allele's length, so the two short inserts and the inverted chr3 return are the crossing ribbons at its right-hand end, named on the track along the allele. The gene track from the launching view comes along onto the reference panel." src="/img/cancer_sv/derivative_autogenerated.png" />
...
<Figure caption="K562 STAR-Fusion calls as chords, colored by junction-read support. Red is the reciprocal chr9/chr22 pair plus one intrachromosomal call on chr6; the fan converging on chrM, between chrY and chr1, is the artifact tail." src="/img/cancer_sv/k562_starfusion_triage.png" />
