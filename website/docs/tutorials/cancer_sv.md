...
<Figure caption="Top: the candidate list at the chr9 fold-back anchors. The path labels name the same chromosome twice, once inverted, which is the fold-back written out, and each row's strip draws it: two blocks of one color whose arrows point at each other. Bottom: the top path drawn, with two windows of chr9 on the reference panel and the allele below. The ribbons cross instead of running parallel, which is what a segment returning inverted looks like, and the track under the allele names the interval each arm came from." src="/img/cancer_sv/foldback_reconstruction.png" />
