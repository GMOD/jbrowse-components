...
<Figure caption="The same 300 bp of K12 at the colanic acid cluster, banded on the ruler, cut from the two graphs this build produces. Left, the minigraph rGFA: one 4.4 kb backbone segment spans the whole band, four alternate segments of 6-154 bp hang off it, and the cyan 16.4 kb node is the next backbone segment along, which the one-hop cut reaches: it joins the backbone at the vertex beside the 88 bp bubble, and its other end is where the loaded subgraph stops. Right, the pggb graph over that same 300 bp: a node at every variant, the off-reference ones drawn dark. Both lanes run over the whole minigraph cut rather than the band, colored by reference position, so each minigraph node is its own block above it; the pggb pane is one hue throughout because all of it sits inside the band. The node and edge counts are in each header." src="/img/pangenome/graph_resolution.png" links="minigraph=pangenome/graph_resolution_minigraph,pggb=pangenome/graph_resolution_pggb" />
