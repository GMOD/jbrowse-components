...
per repeat, so the second pick is **Partition by... → repClass**
<!-- menu-path-ok --> in the same menu. It lists the columns the loaded features

carry, which is where the classes come from.
