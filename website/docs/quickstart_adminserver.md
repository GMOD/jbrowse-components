...
This guide covers configuring JBrowse 2 — adding assemblies and tracks —
through the graphical admin interface.
...
All changes are written to the JBrowse config file (usually `config.json`).
This only works while the `admin-server` is running — it is **not meant for
production use**.
