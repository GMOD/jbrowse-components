...
`SPEC_KEYS`, `EXAMPLE_PLUGIN_TREE`, `SHADER_EXPORTS`). Each renders from a JSDoc
tag, a registration, or a manifest at the definition site — document a new one
by tagging the source. Everything else under `docs/` is hand-written.
