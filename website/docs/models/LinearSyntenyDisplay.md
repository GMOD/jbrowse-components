...
| <span id="getter-loading">**loading**</span><br><code>boolean</code> | First load: no data has arrived yet. Deliberately not `&& fetching` — that would blink the overlay off during the pre-fetch debounce gap. Excludes error so error UI and loading UI never show simultaneously. Drives the full striped LoadingOverlay. | LinearSyntenyDisplay |
