...
| Member | Description | Defined by |
| --- | --- | --- |
| <span id="action-setrpcdata">**setRpcData**</span><br><code>(data: DotplotRpcData, fetchKey: string) =&gt; void</code> | Commits a fetch result, tagged with the key it was fetched for. The loading flags are deliberately NOT touched here: this runs as `installComparativeFetchAutorun`'s `commit`, whose `finally` clears `fetching` and the status line under the same staleness guard. Clearing them here too meant one of the two comparative displays wrote `fetching` directly instead of through `setFetching`, for no effect the skeleton wasn't about to have anyway. | DotplotDisplay |
