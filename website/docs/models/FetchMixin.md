...
| <span id="action-makestatuscallback">**makeStatusCallback**</span><br><code>() =&gt; StatusCallback</code> | An RPC `statusCallback` bound to this display: forwards progress to the shared `statusMessage`, guarded so a callback that fires after the node is torn down (RPCs resolve their status stream asynchronously) is a safe no-op, and throttled through the display-wide window. Pass directly as the `statusCallback` RPC arg instead of re-inlining the guard at every call site.<br><br>Declared this early only so `runFetch` can put one on every `FetchContext`; its sibling `makeRegionStatusCallback` needs `setRegionStatus` and so stays below. |
