...
| Member | Description |
| --- | --- |
| <span id="getter-gpurenderingbackend">**gpuRenderingBackend**</span><br><code>SyntenyRenderingBackend &#124; undefined</code> | Typed accessor for the slot-mixin-owned `currentRenderingBackend`. All synteny displays within the level upload their geometry to the same backend and render onto one canvas. |
...
| <span id="getter-geometrybydisplaykey">**geometryByDisplayKey**</span><br><code>Map&lt;number, SyntenyInstanceData&gt;</code> | Per-display GPU geometry keyed by displayKey. The upload autorun diffs this map — new entries upload, vanished entries evict. |
