...
| [dataLoaded](#getter-dataloaded)                                       | Getters   | GlobalFetchMixin                              | Overridable hook (default false): a subclass returns true once its single global dataset has actually been fetched — even when the fetch committed an empty result.                                                                          |
...
| [svgReady](#getter-svgready)                                           | Getters   | GlobalFetchMixin                              | Global-display analog of `MultiRegionDisplayMixin.svgReady`: true once an off-screen (SVG) export can read final data.                                                                                                                       |
