...
| [isCacheValid](#method-iscachevalid)                                 | Methods    | LinearManhattanDisplay                                | Manhattan features are 1:1 with the underlying SNPs (pre-transformed -log10 p values) and don't downsample by zoom, so cached data is valid at any bpPerPx.                                                                                                                                        |
...
```ts
type isCacheValid = (_displayedRegionIndex: number) => true
```
