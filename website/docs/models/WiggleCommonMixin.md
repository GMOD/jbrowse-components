...
| <span id="getter-zoomfetchkey">**zoomFetchKey**</span><br><code>string</code> | Strict zoom equality (adr-008): the worker bins scores to the requested bpPerPx, so data fetched at another zoom is the wrong summary, however well the viewport still sits inside it.<br><br>Here rather than on `ScoreFieldConfigMixin`: the rule is about what a fetch returns, and `LinearManhattanDisplay` composes that mixin while fetching untransformed SNPs. | WiggleCommonMixin |
| <span id="getter-symlogconstant">**symlogConstant**</span><br><code>number</code> | Raw `symlogConstant` slot; `0` means "derive from the domain". Resolve it with `resolveSymlogConstant` once the domain is known.<br><br>Here rather than on the score config because only the wiggle schemas declare the slot, and `getConf` answers `undefined` silently for a composer that does not. | WiggleCommonMixin |
