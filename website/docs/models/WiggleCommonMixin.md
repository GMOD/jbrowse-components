...
| <span id="getter-valuescale">**valueScale**</span><br><code>ValueScale &#124; undefined</code> | <span data-pagefind-ignore>Overridable hook (default none): the scale this display draws its y through. A display that answers it gets its axis and its cross-hatches placed by `DisplayChrome` and `renderDisplaySvg`, and `ticks` derived below; one with several axes (the multi-wiggle's rows) leaves it unset and lays its own out.</span> | [ScoreScaleMixin](../scorescalemixin#getter-valuescale) |
| <span id="getter-ticks">**ticks**</span><br><code>YScaleTicks &#124; undefined</code> | <span data-pagefind-ignore>The axis, derived from `valueScale`: where each tick lands in the plot's own pixel space.</span> | [ScoreScaleMixin](../scorescalemixin#getter-ticks) |
