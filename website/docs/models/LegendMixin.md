...
| <span id="getter-legendtop">**legendTop**</span><br><code>number</code> | Overridable hook (default 0): px from the top of the display box the key starts at, on screen and in the export alike. A display that already draws something in that corner — Hi-C's resolution box, the multi-wiggle's score caption — answers its clearance. |
