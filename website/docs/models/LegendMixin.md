...
| <span id="getter-showlegenddisplaytypedefault">**showLegendDisplayTypeDefault**</span><br><code>Pin</code> | The pin over the legend checkbox's state: filled when that state is this display type's default. `showLegendCheckboxItem` takes it as its `pin`. |
