...
| <span id="getter-rendersdisplays">**rendersDisplays**</span><br><code>boolean</code> | <span data-pagefind-ignore>Overridable hook (default true): whether this view's mounted body renders its displays. A linear genome view collapsed to its ruler mounts the scalebar and none of its tracks, so a display in it can no more paint than one in a view scrolled off screen.</span> | [BaseViewModel](../baseviewmodel#getter-rendersdisplays) |
