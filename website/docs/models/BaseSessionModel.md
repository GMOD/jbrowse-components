...
| <span id="action-setscrollzoomhintcount">**setScrollZoomHintCount**</span><br><code>(n: number) =&gt; void</code> | spend one of the scroll-to-zoom prompt's showings (see `canShowScrollZoomHint`). A setter rather than an increment so a test can put the budget wherever it needs it. | BaseSessionModel |
| <span id="action-endscrollzoomhints">**endScrollZoomHints**</span><br><code>() =&gt; void</code> | stop offering the scroll-to-zoom prompt for the rest of the session.<br><br>For an answer, as opposed to a raise going unanswered: the user found the setting, or waved the prompt away rather than letting it time out. The budget is for a user who might not have noticed, and none of those is that user — spending it a raise at a time would go on interrupting someone who has already replied. | BaseSessionModel |
