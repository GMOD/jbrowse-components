...
| <span id="getter-colorlegendchips">**colorLegendChips**</span><br><code>ColorChip[]</code> | Legend rows naming the overlaid tracks — one per track with its palette color, and only under `colorBy: 'track'`, since every other mode has a fixed legend of its own. |
