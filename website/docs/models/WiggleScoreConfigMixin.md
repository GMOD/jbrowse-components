...
| <span id="getter-showcrosshatches">**showCrossHatches**</span><br><code>boolean</code> | Whether the score-axis cross hatches draw: `scales.y.grid`, never in density mode, which has no height axis to rule and no toggle in its menu. | WiggleScoreConfigMixin |
