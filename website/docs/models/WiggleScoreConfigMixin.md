...
| --- | --- | --- |
| <span id="getter-symlogconstant">**symlogConstant**</span><br><code>number</code> | Raw `symlogConstant` slot; `0` means "derive from the domain". Lives here rather than on `ScoreScaleMixin` because the slot does: only the wiggle shaders implement symlog, and the alignments coverage band composes that mixin against a config schema that never declares it. Resolve it with `resolveSymlogConstant` once the domain is known. | WiggleScoreConfigMixin |
