...
| <span id="property-resultnoun">**resultNoun**</span><br><details><summary><code>resultNoun: types.optional(types.enumeration(['hit', 'product']…</code></summary><pre><code>resultNoun: types.optional(types.enumeration(['hit', 'product']), 'hit')</code></pre></details> | what one result is called. A BLAT result is a hit; an hgPcr result is a product, which is a band on a gel — the word a bench scientist reasons in, and "2 hits" understates what two of them mean for a PCR run |
