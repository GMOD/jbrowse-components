...
| <span id="getter-ribboncolors">**ribbonColors**</span><br><code>Map&lt;string, string&gt;</code> | each alignment's resting fill under the view's `color` and `alpha`, by feature id. A chromosome mode paints the ideogram colour of the chromosome it joins, so a ribbon matches the arc it leaves | ChordSyntenyDisplay |
| <span id="getter-ribbonfill">**ribbonFill**</span><br><code>(feature: Feature) =&gt; string</code> | the resting fill of each ribbon | ChordSyntenyDisplay |
| <span id="getter-drawnfeatures">**drawnFeatures**</span><br><code>Feature[] &#124; undefined</code> | what the ribbons draw: the visible alignments at least the view's `minAlignmentLength` long on their own side | ChordSyntenyDisplay |
