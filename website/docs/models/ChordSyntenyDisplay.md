...
| <span id="getter-shapes">**shapes**</span><br><code>RibbonShape[]</code> | each drawn alignment as the four angles its ribbon visits and the fill it rests in; an end off the circle drops the ribbon | ChordSyntenyDisplay |
...
| <span id="method-shapepathfor">**shapePathFor**</span><br><code>(feature: Feature) =&gt; string &#124; undefined</code> | a drawn feature's outline as an SVG path, for anything that has to find a chord on screen without a DOM node to find | ChordSyntenyDisplay |
