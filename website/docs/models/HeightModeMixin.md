...
| ------------------------------------------------ | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [heightMode](#getter-heightmode)                 | Getters | The resolved track-height strategy (`fixed`/`grow`/`fit`). Promotable sentinel slot: getConfResolved walks the pinned-track -> session-default -> `fixed` cascade and never returns the `inherit` sentinel.                                                                            |
| [fitTargetHeight](#getter-fittargetheight)       | Getters | The drag-resizable track height as stored in the config slot — the fit target the fit/grow layout scales or packs content into. Read there instead of the reactive `height` getter to break the grow-mode cycle (`height`->grownHeight->layout->height). Equals `height` in fixed/fit. |
| [autoHeight](#getter-autoheight)                 | Getters | `grow` mode as a boolean, derived from the unified `heightMode` slot.                                                                                                                                                                                                                  |
| [fitHeightToDisplay](#getter-fitheighttodisplay) | Getters | `fit` mode as a boolean, derived from the unified `heightMode` slot.                                                                                                                                                                                                                   |
