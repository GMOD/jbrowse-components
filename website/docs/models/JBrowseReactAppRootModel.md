...
| <span id="action-setdefaultsession">**setDefaultSession**</span><br><code>() =&gt; void</code> | <span data-pagefind-ignore>Starts a new session from the config's defaultSession. An `id` in it (a pasted session export carries one) is dropped, since sessions are autosaved by id and each new one would otherwise overwrite the last.</span> | [BaseRootModel](../baserootmodel#action-setdefaultsession) |
