...
| [heightBeforeExpand](#volatile-heightbeforeexpand)                 | Volatiles  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [squeezeToDisplayHeight](#volatile-squeezetodisplayheight)         | Volatiles  | Squeeze-to-display-height mode (the "Fit to display height" menu preset): laid-out glyphs are uniformly shrunk so every row fits the track height without scrolling. Volatile — a view preference like scrollTop; picking any feature-height preset turns it off. The `height` config slot is never written, so it stays pure user intent. Only ever shrinks (scale <= 1); a track that already fits is left untouched.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| [baseLaidOutDataMap](#getter-baselaidoutdatamap)                   | Getters    | Layout at the current display mode, before any fit-to-height squeeze. The reference for both `squeezeScale` (which reads its height) and the final `laidOutDataMap`; kept separate so the fit scale derives from the unscaled content height and can't feed back on itself.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| [squeezeScale](#getter-squeezescale)                               | Getters    | Uniform vertical squeeze factor for squeeze-to-display-height mode: makes the whole stack fit the track height without scrolling. Shrink-only (<= 1); 1 whenever the squeeze is off, content already fits, or there's nothing to lay out. Measured off the unsqueezed base layout so it can't feed back on itself.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| [laidOutDataMap](#getter-laidoutdatamap)                           | Getters    | What every consumer (hit test, GPU upload, React render) reads. The `baseLaidOutDataMap` by reference unless the squeeze is shrinking the stack, in which case each region is cloned and scaled to exactly fill the track height. Returning the base map by reference off the squeeze path keeps the incremental-layout upload diff and Y-morph idle check intact.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| [canExpand](#getter-canexpand)                                     | Getters    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| [expandToFit](#action-expandtofit)                                 | Actions    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [setSqueezeToDisplayHeight](#action-setsqueezetodisplayheight)     | Actions    | Enter/leave squeeze-to-display-height mode. Entering scrolls to the top (a squeezed stack has no scroll, so a stale scrollTop would leave the GPU canvas painted at an invalid offset). The `laidOutDataMap` getter does the actual squeeze reactively.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [collapseFromExpand](#action-collapsefromexpand)                   | Actions    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [clearHeightBeforeExpand](#action-clearheightbeforeexpand)         | Actions    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| [setAutoHeight](#action-setautoheight)                             | Actions    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| [showContextMenuForFeature](#action-showcontextmenuforfeature)     | Actions    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| [resizeHeight](#action-resizeheight)                               | Actions    | A manual drag-resize means the user wants a fixed height; turn off auto-fit first, otherwise the CanvasAutoHeight autorun snaps the height back on the next layout change and the drag appears to do nothing. Also drop any expand-to-fit marker — once the user sets a height by hand, restoring to the pre-expand height is stale (same reasoning as setAutoHeight).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
layout change and the drag appears to do nothing. Also drop any expand-to-fit
marker — once the user sets a height by hand, restoring to the pre-expand height
is stale (same reasoning as setAutoHeight).
