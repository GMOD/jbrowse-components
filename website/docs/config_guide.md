---
title: Config guide
sidebar_label: Overview
---

How to configure the `config.json` that drives a session. For copy-paste
recipes, see the [Cookbook](/docs/cookbook).

## Getting started

- [Deploying JBrowse Web](/docs/config_guides/deploying)
- [config.json format](/docs/config_guides/intro)

## Core configuration

- [Assemblies](/docs/config_guides/assemblies)
- [Authentication](/docs/config_guides/authentication)
- [Connections](/docs/config_guides/connections)
- [Supported file types](/docs/config_guides/file_types)
- [Plugins](/docs/config_guides/plugins)
- [Config slot types](/docs/config_guides/slot_types)
- [Tracks](/docs/config_guides/tracks)

## Track types

- [Alignments track](/docs/config_guides/alignments_track)
- [GWAS track](/docs/config_guides/gwas_track)
- [Hi-C track](/docs/config_guides/hic_track)
- [MAF track](/docs/config_guides/maf_track)
- [Multi-quantitative tracks](/docs/config_guides/multiquantitative_track)
- [Quantitative tracks](/docs/config_guides/quantitative_track)
- [Synteny track](/docs/config_guides/synteny_track)
- [Variant track](/docs/config_guides/variant_track)

## Callbacks and customization

- [Customizing feature colors](/docs/config_guides/customizing_feature_colors)
- [Customizing feature details](/docs/config_guides/customizing_feature_details)
- [FromConfig adapters](/docs/config_guides/from_config)
- [Using jexl callbacks](/docs/config_guides/jexl)

## Other features

- [Avoiding stale config](/docs/config_guides/avoiding_stale_config)
- [Default session](/docs/config_guides/default_session)
- [Disabling analytics](/docs/config_guides/disable_analytics)
- [Text searching](/docs/config_guides/text_searching)
- [Coloring/theming](/docs/config_guides/theme)
- [Hierarchical track selector](/docs/config_guides/track_selector)
