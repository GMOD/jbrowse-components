...
- **The GPU rewrite.** We rewrote the entire rendering system to draw on the
  GPU, so you get smooth scroll zooming and much better responsiveness. The
...
[developer guides](https://jbrowse.org/jb2/docs/developer_guide). Every
screenshot and video in the docs is autogenerated, so they never go stale, and
...
(which was our term for rendering static tiles on the web worker). The
[v5 upgrade guide](https://jbrowse.org/jb2/docs/developer_guides/upgrading_v5)
lists that migration and the smaller ones — contact us if you need any
assistance.
