...
The difference this makes is easiest to see on a zoom. The old renderer bound
its output to a particular `bpPerPx`, so every zoom threw the drawing away and
fetched it again; the new one re-projects reads it already holds. Zooming into
loaded data now shows no loading indicator at all, which is why the column below
has nothing to divide. Panning is the harder case — the region is new to both
architectures, so both must go to the network, and what is left is render cost:
...
Cold initial load is fetch-dominated — both versions fetch in workers — so it
moves least, between 1.3× and 2.2× on the cases that measured cleanly.

Read those as one workstation, one locus, and a simulated read corpus, on the
WebGL2 path rather than WebGPU. The pan ratios are the robust part; the absolute
figures behind them are a single run each. The harness, the corpus and the
caveats are in [jb2bench](https://github.com/cmdcolin/jb2bench), including the
rows that were thrown out for having been measured on a loaded machine.
...
Under the covers, CRAM moved to `@gmod/cram` v13 with three read-feature-walk
bugs fixed against samtools, mismatches now driven off the library's own walk,
and `BamAdapter` applies every read filter. A long string of layout and hot-path
fixes landed with them — reference comparison two bases per byte, insert sizes
sorted as a typed array, targeted SA/MM/ML tag reads instead of a full-record
tag parse, and no refetch at all when you change the color scheme.
...
with JBrowse Desktop; JBrowse Web does not bundle it yet, and shipping it there
is planned. Hits come back as a track plus a results panel listing every hit,
and the view does not move on its own — which hit matters is yours to decide.
...
The "region too large" gate was reworked to be honest about which limit it hit
and to release itself when zooming can release it, instead of sitting there
until you force-load. A `forceLoad` config slot declares that bypass up front,
so a track that is always going to be over the limit can just say so.
...
Track hubs are a first-class way in. A `&hubURL=` link opens the hub instead of
explaining that it cannot, a session spec can carry connections, so one link can
open a hub as several views, and connections load lazily behind expandable
track-selector categories rather than all at once — which is what makes a hub
with thousands of tracks openable at all.
...
`jbrowse validate` is new. Point it at a config and it reports the shapes
JBrowse itself accepts silently and then fails to draw. It found real problems
in our own configs, and in the config blocks in these docs, which are now
checked with it.
...
suits it, with the real assembly names filled in. The add-track format guesser
was realigned with the browser's, so the CLI and the app now agree on what a
file is.
...
On the security side, a shell injection in `make-pif` and a zip-slip in the
archive extractor were both closed.
...
The GPU pipeline is the headline, but a large share of the release's performance
work is underneath and beside it, and shows up on machines with no usable GPU
just as much.

**Startup.** bgzf and pako, the generated shader source, `@mui/x-data-grid`,
`@floating-ui`, uFuzzy, the dockview stylesheet and react-dropzone all came off
the first-paint path, several RPC method bodies are deferred until something
calls them, and startup analytics no longer sit on the critical load path. A
handful of eagerly-evaluated modules that were pinning React components in the
startup chunk were split apart.

**Parsing.** GFF3 reads its attributes on demand rather than at parse time, GTF
scans the attribute column instead of splitting and re-trimming it, flat files
decode in chunks rather than one `TextDecoder` call per line, and the alignments
tag path reads the one tag it wants instead of decoding all of them. Hi-C
decodes contacts straight into typed arrays and keeps them typed all the way to
the GPU; MAF sends a columnar wire out of the worker and rehydrates it at
placement.

**Memory.** Parsed-chunk retention is bounded per worker rather than per open
track, in-flight chunk fetches are shared rather than duplicated, and the cache
cap is a real bound instead of an advisory one — which is what keeps a long
session with many tracks open from growing without limit.

**Redraw discipline.** A great deal of the work was finding things that
recomputed per frame, per event or per feature and shouldn't have: hit-test
indexes rebuilt on every mousemove, colors re-parsed hundreds of thousands of
times, pileups repacked for syncs that changed no bytes, regions refetched when
only the color scheme changed. Those are invisible individually and are most of
why a sustained gesture is smooth.

For contributors, `tsc` project references made the whole-repo build several
times faster.
...
Comparing the v4.3.0 tag against the merged branch: 8,778 files changed,
+820,063 / −219,053 lines. `plugins/` and `website/` dominate, which matches
...
Storybook was dropped in favor of lighter Astro example sites, and config and
state models were flattened so one model holds many slots instead of each slot
being its own MST instance. `refactor` commits outnumber `feat` commits on the
branch by more than half again, and `perf` is a category of its own rather than
a note under something else.
