...
Opening a track cold moves least, between 1.3× and 2.2×, because most of that
time is spent waiting on the network in both versions.

These come from one machine at one locus on simulated reads, so treat them as
the shape of the difference rather than a promise about your data. The harness,
the corpus and the full set of caveats are in
...
a compact numeric form rather than being rebuilt as objects along the way.
...
The last stretch of the release was net simplifying rather than additive. More
of the work went into refactoring and performance than into new features, which
is not what the file count above suggests and is the more accurate picture of
where the year went.
