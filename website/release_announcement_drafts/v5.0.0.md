...
A new
[consensus track](https://jbrowse.org/jb2/docs/user_guides/consensus_sequence)
...
Point it at a config and it reports the mistakes JBrowse itself accepts silently
and then fails to draw — the class of problem that otherwise shows up as an
empty track and no explanation. It found real ones in our own configs and in the
...
cover `arcs`, `samplot`, `linkedReads` and `sashimi`. And `--hub` pulls hosted
configs straight from genomes.jbrowse.org, `list` enumerates the available hubs
and tracks, and `--loc` navigates by gene name through the hub's text-search
index — so a figure of "SOX2 in hg38" no longer needs a hand-assembled config.
...
MST model. Drawer-widget support was added to embedded views, and
`DisplayChrome` was split into a toolkit-free base plus a swappable overlay set,
so an embedder can supply their own chrome.
...
Node access. A preload shim keeps `const { ipcRenderer } = require('electron')`
working, which is the shape nearly every crossing to the main process already
uses, so most desktop plugins need no change. One that reached for Node
built-ins directly in the renderer does.
...
Fifty-three names left the `@jbrowse/core/*` re-export ABI — the modules an
external plugin resolves through `jbrequire`. A removed name is `undefined`
...
- desktop file handles, now behind the preload bridge (`getFileHandleCache` and
  its seven siblings, on both `util` and `util/tracks`)
