<!--
Release announcement draft, consumed by `pnpm release` (scripts/release.ts):
at release time it is dropped into scripts/blog_template.txt, which supplies
the frontmatter, the Downloads block, and the generated changelog. A prerelease
skips that step, so this draft stays here until v5.0.0 proper is cut.

Image paths are written relative (`../static/img/...`) so the figures render in
the GitHub PR. `website/blog/` sits at the same depth, so they stay valid in the
repo after the move — but the built site serves those files at `/img/`, so flip
them when the post is published:

  sed -i 's|\.\./static/img/|/img/|g' website/blog/*-v5.0.0-release.md

Worth doing at the same time: most of these figures have screenshot specs, so
converting `![caption](src)` to `<Figure caption="..." src="..." />` gets each
one a lightbox and an auto-generated "Open this view in JBrowse" link.
-->
...
![LD at the human lactase locus computed live from phased 1000 Genomes genotypes, with ClinVar marking rs4988235 and the recombination track above.](../static/img/ld/lct_lactase.png)
...
![BXD strains ordered by genotype similarity across the whole of chr4 rather than at the QTL peak, so strains sharing long stretches become neighbors.](../static/img/qtl/bxd_painting_clustered.png)

![Dog10K chr1 painted by FLARE against gray wolf and breed-dog panels, two haplotype rows per animal. Wolf blocks tile the four Saarloos Wolfdogs and, in fewer and longer runs, the four Czechoslovakian Wolfdogs.](../static/img/dog10k-wolfdog-ancestry.png)
...
![CATlas single-cell ATAC over the albumin gene, one accessibility row per cell type on a shared scale, loaded straight from the atlas's hosted BigWigs.](../static/img/scatac/alb_hepatocyte.png)
...
![Structural alleles across the HPRC2 haplotypes, one row each, clustered by genotype and drawn under the HLA class II genes they fall in.](../static/img/hprc2/mhc_clustered.png)
