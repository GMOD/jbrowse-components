...
backend if you need to compare. On a machine whose WebGL is a CPU rasterizer — a
VM, a locked-down laptop, a remote desktop — the ladder steps over that rung and
lands on Canvas2D, which is several times quicker there than the GPU path that
only looks like one. If a GPU context is lost mid-session, every view now offers
the switch to Canvas2D on screen, dotplot and synteny included.
...
dendrogram beside it draws at tens of thousands of tips, where a deep
single-linkage tree used to give up partway through.
...
track survives an untick and retick. A track with features hidden past its own
bottom edge inks that edge, and says which way — scroll to the end and the
bottom fade goes and a top one appears — so a half-read pileup announces itself
instead of relying on you noticing a thin scrollbar.
...
closes it, Escape cancels a drag in progress, and the four whole-workspace
tilings — tabs, horizontal, vertical, grid — are one menu item each. The chrome
follows the theme instead of being dark either way.

All of it runs on the session's own state tree now, with no window-manager
library holding a second copy of the layout to reconcile against. A session
saved by v4 opens and keeps every view; only its arrangement is lost.

The tab strip and the splitters also take the keyboard now — arrows to move
between tabs and to resize a pane — and tabbing into a view registers the focus
straight away, so the first ctrl+arrow after arriving pans the view you arrived
at instead of the one you left.
...
The dialogs that do long work of their own report it too. Get sequence, Save
track data, Group by tag and the synteny launch dialog say what they are doing
and how far along they are, and closing one stops the worker instead of leaving
it grinding on an answer nobody is waiting for.
...
consistent text in figures, and a track whose display cannot draw itself as SVG
is skipped instead of taking the whole export down with it.
...
shape a notebook or an in-house build actually has. `createViewState` takes the
same declarative `init` the managed components take, so holding the engine
yourself no longer costs you a hand-written `defaultSession`. A host can also
pin the rendering backend, which matters when the page has its own WebGL on it
and JBrowse is not the only thing spending the browser's context budget.
...
per-user and updater-aware, and double-clicking a `.jbrowse` session file opens
it on macOS and Linux now, not only on Windows.
