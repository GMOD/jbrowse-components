...
Tracks upload their data to the GPU once and redraw from it as you move. What
the worker sends is absolute genomic coordinates, so panning re-projects data
that is already loaded rather than fetching and re-rendering from scratch — on
every track type, including the whole-genome views that used to be slow enough
that you avoided them.

Zoom is the one gesture that can still need new data, and only where the zoom
level decides the _content_ rather than where it goes. Alignments and dotplots
re-project and never go back. Hi-C, MAF and synteny fetch again on crossing into
another tier — a coarser matrix resolution, a summary row, a lower level of
detail — and not otherwise. Wiggle and the multi-sample variant matrix fetch on
any zoom change at all, because a BigWig's zoom levels and a matrix's
per-feature columns are both chosen by how far out you are.

The difference is starkest zooming into an alignments track. The old renderer
tied its output to one zoom level, so every zoom threw the drawing away and
fetched it again — on a deep long-read track that was a fifteen-second wait. The
new one re-projects reads it already holds, and shows no loading indicator at
all. `current` below is how long an indicator was on screen, and no indicator
appeared:
...
The harness and the corpus behind that are in
[jb2bench](https://github.com/cmdcolin/jb2bench), and what the table does and
does not claim is in
[RENDERER_BENCHMARKS.md](https://github.com/GMOD/jbrowse-components/blob/main/agent-docs/reference/RENDERER_BENCHMARKS.md).
Zoom in is the architecture's best case — it is the gesture where the work is
skipped rather than made faster — so do not read it as the speedup.
...
names the one connection under the cursor — an end coordinate on each
chromosome, and how many molecules agree with it.
...
menu was thinned out to toggles alone: the row cap, the one action sitting among
the checkboxes, moved into Read height. A new `SamAdapter` reads plain SAM
files, and any feature carrying a CIGAR string now draws its indels, not only
BAM and CRAM reads.

Under the covers, CRAM moved to `@gmod/cram` v13. Its test suite decodes every
indexed CRAM in its corpus and compares each read field by field against
`samtools view`, which is what turned up the places JBrowse and samtools
disagreed about what a read contained — a B read feature's base, and reads
overlapping a query's first base. `BamAdapter` now applies every read filter
rather than most of them. Changing the color scheme no longer refetches the
region.
