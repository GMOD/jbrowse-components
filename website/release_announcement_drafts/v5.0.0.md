...
region under the limit, instead of waiting for you to force-load. A `forceLoad`
...
config now shows a real error rather than crashing or silently doing nothing.
...
per-user and updater-aware, and it now associates `.jbrowse` files with the app.
