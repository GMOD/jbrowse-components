...
// A compose spec's parts are never written as `/img/<part>.png` in a doc — the
// doc references the stacked parent, and reaches each part only through
// `<Figure links=...>`. Scanning for image names therefore finds no usage for a
// part and the UI flags it "not referenced in any doc", which reads as an
// orphan figure to delete when it is in fact half of a live before/after figure
// (and the mechanism the website CLAUDE.md prefers over `stages`). Lend each
// part its parent's usages so it reports where it actually shows up.
function composePartUsages(
  specs: ScreenshotSpec[],
  index: Map<string, DocUsage[]>,
): Map<string, DocUsage[]> {
  const out = new Map<string, DocUsage[]>()
  for (const spec of specs) {
    if (spec.mode === 'compose') {
      const parentUsages = index.get(spec.name) ?? []
      for (const part of spec.parts) {
        const lent = parentUsages.map(usage => ({
          ...usage,
          caption: `(stacked into ${spec.name}) ${usage.caption}`,
        }))
...
export function collectScreenshots(specs: ScreenshotSpec[]): Screenshot[] {
  ...
  const partUsages = composePartUsages(specs, index)
  ...
  return [...names]
    ...
    .map(name => ({
      name,
      usages: [...(index.get(name) ?? []), ...(partUsages.get(name) ?? [])],
