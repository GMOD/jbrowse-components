...
export interface ScreenshotPart {
  ...
  // git blob sha of the origin/main copy, absent when there isn't one. It is
  // what the reviewed-against picture IS, so it doubles as the cache key the
  // /img-main/ URL carries — see mainPngBlob.
  mainHash?: string
...
// Cache of name -> blob sha for every PNG on origin/main. The sha comes free
// with the listing (`<mode> blob <sha>\t<path>`) and is the exact identity of
// those bytes, which is what lets the /img-main/ URL be content-addressed the
// way the working-tree one already is.
let mainBlobs: Map<string, string> | undefined

function getMainBlobs(): Map<string, string> {
  if (!mainBlobs) {
    mainBlobs = new Map()
    try {
      const out = execFileSync(
        'git',
        ['ls-tree', '-r', 'origin/main', imgGitPrefix],
        { cwd: repoRoot, encoding: 'utf8', maxBuffer: gitMaxBuffer },
      )
      for (const line of out.trim().split('\n')) {
        const m = /^\S+ blob (\S+)\t(.*)$/.exec(line)
        if (m) {
          mainBlobs.set(gitPathToName(m[2]!), m[1]!)
        }
      }
    } catch {
      // no origin/main to compare against; every figure reads as new
    }
  }
  return mainBlobs
}

// undefined when origin/main has no copy of this figure — which is what
// "existsOnMain: false" (a figure added on this branch) is derived from.
export function mainPngBlob(name: string): string | undefined {
  return getMainBlobs().get(name)
}
...
export function collectScreenshots(specs: ScreenshotSpec[]): Screenshot[] {
  ...
  const toPart = (name: string): ScreenshotPart => {
    const exists = pngExists(name)
    const mainHash = mainPngBlob(name)
    return {
      name,
      usages: index.get(name) ?? [],
      autogenerated: specNames.has(name) || desktopAutogenNames.has(name),
      exists,
      existsOnMain: mainHash !== undefined,
      mainHash,
      changed: exists && mainPngChanged(name),
    }
  }
