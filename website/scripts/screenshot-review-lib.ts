...
// `autogenerated` means hasSpec && !curated — the generator will actually
// re-create it on the next run; curated specs exist but pin a human screenshot.
...
export function collectScreenshots(
  ...
  return [...names]
    ...
    .map(name => {
      ...
      const isDesktopAutogen = desktopAutogenNames.has(name)
      return {
        name,
        usages: index.get(name) ?? [],
        hasSpec: specMap.has(name) || isDesktopAutogen,
        autogenerated:
          isDesktopAutogen || (spec !== undefined && !spec.curated),
