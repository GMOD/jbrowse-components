...
// Every generated block says so, in one line, emitted here so no doc has to
// hand-write the sentence and then drift from it. Readers arrive mid-file from
// a search hit or a deep link, where the marker comments are invisible in the
// rendered page and the table looks hand-maintained — which is how a table gets
// edited and silently overwritten on the next run.
// `agent-docs/reference/GENERATED_DOC_BLOCKS.md` names the generator behind each
// marker. An empty block gets no notice: there is nothing to mistake.
