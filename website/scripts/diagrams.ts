// Renders website/diagrams/*.dot and gates them against drift.
//
//   pnpm diagrams          render every .dot, then record what was rendered
//   pnpm diagrams:check    CI gate: no .dot is ahead of its figure
//
...
// diagram makes that sharper rather than softer: edit the .dot, forget to
...
// perfectly consistent with each other the whole time — it is the .dot that is
// ahead of both.
//
// So diagrams.lock pairs each source with the figure it produced:
//
//   <source>.dot <sha256 of the .dot> <sha256 of the .png it rendered to>
//
...
const figureFor = (source: string) =>
  `website/static/img/${source.replace(/\.dot$/, '.png')}`
...
# them to the .dot that produced them, so a source edited without a re-render
...
function render() {
  ...
  if (!sources.length) {
    console.error(`no .dot sources in ${diagramsDir}`)
    process.exit(1)
  }
  const pairs: Pair[] = []
  for (const source of sources) {
    ...
    execFileSync(
      'dot',
      ['-Tpng', `-Gdpi=${DPI}`, source, '-o', join(repoRoot, figure)],
      { cwd: diagramsDir },
    )
