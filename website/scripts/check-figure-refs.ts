...
import { figureName, isDerivedFigure } from './figure-store.ts'
...
for (const file of docFiles(docsDir)) {
  ...
  while ((match = figureRe.exec(text)) !== null) {
    ...
    // a derived figure is regenerated by every build and stored by none
    if (
      known.has(path) ||
      isDerivedFigure(src.slice(IMG_PREFIX.length), 'website/static/img')
    ) {
      continue
    }
