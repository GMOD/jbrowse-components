...
export const svVideos: VideoSpec[] = [
  ...
  // correspondence two stills can only assert. The chain ends inverted at the
  // chr3 junction it starts from, so its last segment shares the first panel.
  //
  ...
  // film that as a list arriving from nowhere.
  {
    ...
    description:
      "COLO829's three-junction chain built by the app rather than typed in: the tumor track menu, Reconstruct derivative allele..., the four-segment route most reads support, and Draw as Breakpoint split view replacing the window with one panel per locus on the route",
    url: breakpointPanel,
    // SIZED TO THE END STATE, which is the three-panel split view and not the
    // dialog: cancer_sv/derivative_autogenerated holds the same dialog over the
    // same pileup in a 560px frame, while the panels are 70 of gene lane and 160
    // of pileup each, three times over (MULTIHOP_CHAIN_HEIGHT). The page under
    // the launching pileup at the start is the room those panels need.
    viewportHeight: 1130,
    ...
    steps: [
      ...
      // Three panels, each a 10 kb window centred on the junction its segment
      // carries, and each carrying the launching view's whole track list — so
      // this is three pileup fetches. A film of that is a film of spinners.
