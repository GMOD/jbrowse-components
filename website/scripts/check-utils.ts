...
// Every generated block says so, in one line. Defined here and imported by
// api-docs/util.ts because the tree writes generated blocks through TWO marker
// spellings and two writers — `<!-- NAME START -->` via `replaceMarkerRegions`,
// `<!-- BEGIN GENERATED NAME -->` via `spliceGeneratedBlock` below — and a
// notice spelled once per writer is the drift it exists to warn about. The
// spellings differ only in which generator wrote the block, so a reader must
// not be able to tell them apart by whether the block admits to being derived.
//
// Readers arrive mid-file from a search hit or a deep link, where the marker
// comments are invisible in the rendered page and the table looks
// hand-maintained — which is how a generated table gets edited and silently
...
export function spliceGeneratedBlock({
  ...
  return (
    ...
    [
      begin,
      '',
      ...(body.length ? [GENERATED_NOTICE, '', ...body, ''] : []),
    ].join('\n') +
    existing.slice(to)
  )
}
