...
// imported by api-docs/util.ts because the tree writes generated blocks
// through TWO marker spellings and two writers — `<!-- NAME START -->` via
// `replaceMarkerRegions`, `<!-- BEGIN GENERATED NAME -->` via
// `spliceGeneratedBlock` below — and a notice spelled once per writer is the
// drift it exists to warn about. The spellings differ only in which generator
// wrote the block, so a reader must not be able to tell them apart by whether
// the block admits to being derived.
//
// A reader landing mid-file from a search hit or a deep link, where the
// marker comments are invisible, sees what looks like a hand-maintained
// table — which is how a generated table gets edited and silently
// overwritten on the next run. `isWebsiteDoc` below keeps the notice out of
// the rendered site itself: a jbrowse.org visitor has no file to edit.
export const GENERATED_NOTICE =
  '_Generated by `pnpm autogen` — edit the source, not this block._'

// True for a doc under website/docs — the pages Astro renders to
// jbrowse.org — as opposed to agent-docs, which is read as source. `file` may
// be absolute or relative to `repoRoot`, since the two writers pass paths
// built either way.
export function isWebsiteDoc(file: string): boolean {
  return resolve(repoRoot, file).startsWith(`${docsDir}/`)
}
...
export function spliceGeneratedBlock({
  ...
  return (
    ...
    [
      ...
      '',
      ...(body.length
        ? [...(isWebsiteDoc(path) ? [] : [GENERATED_NOTICE, '']), ...body, '']
        : []),
    ].join('\n') +
    existing.slice(to)
  )
