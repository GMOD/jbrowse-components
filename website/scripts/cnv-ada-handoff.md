...
copy-number segments, and phased (already-folded) BAF. If `~/cgiab` already has a
Wakhan run or a driver script, reuse it. Inputs it needs, locate them in
`~/cgiab`:
...
A hand-rolled `mosdepth`/`bcftools` fallback (exact awk formulas for log2 ratio +
folded BAF) is in `cnv-data-recipe.md` if Wakhan isn't usable.
...
- **`HG008-T_baf_folded.bw`** — folded/phased BAF: one value per het bin where
  0 = balanced (het ≈ 0.5) and 0.5 = full skew (LOH). This is the key new file —
...
- **`HG008-T_coverage_perbase.bw`** (or a fine ~100–500bp-bin bigWig) covering at
  least `chr9:21,900,000–22,020,000` (CDKN2A) — fine enough to show the
...
size, the score domain (min/max), and — for the folded BAF — confirm 0 = balanced
/ 0.5 = LOH. The jbrowse-side agent will then swap the indexcov subadapters for
`HG008_log2ratio.bw`, point the BAF tracks at `HG008-T_baf_folded.bw` with
`summaryScoreMode:'avg'`, point `driver_cdkn2a_deletion` at the fine-coverage
bigWig, regenerate the four screenshots, and flip their review entries to good.
