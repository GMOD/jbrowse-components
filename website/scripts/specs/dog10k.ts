...
export const dog10kSpecs: ScreenshotSpec[] = [
  ...
  // Built by scripts/build_dog10k_nhej1_sv.sh.
  {
    ...
    // ONE track. The VCF is still declared twice in
    // test_data/dog10k/config.json (`dog10k_denr_svs` and
    // `dog10k_denr_svs_records`) because that used to be the only way to get two
    // displays of one callset into one view; the second id is now unused here.
      ...
      views: [
        {
          ...
          // at both edges, so the transcript rows ran off both sides. Zooming out
          // now costs the records some width — they are drawn at their real span —
          // but 220 bp over an 18 kb window is still ~17 px, and being able to see
          // the whole gene is what the review asked for.
          ...
          tracks: [
            ...
            },
            {
              ...
              // NOT the matrix display, and this is the figure that says why.
              // A matrix lays its columns out by feature INDEX at equal widths,
              // so these two records filled the panel as two half-width blocks
              // and read as multi-kb deletions — they are 222 bp and 219 bp.
              // That cost a pill saying so, a connector band to tie each column
              // back to a position, and a second 90px track of the same records
              // at their real coordinates. All three are gone: a structural
              // variant's SPAN is the thing being shown, and this display draws
              // each record at its own coordinate and width.
              type: 'LinearMultiSampleVariantDisplay',
              ...
              height: 730,
              // and the lane above the rows is what the separate record track
              // used to be: each SINE deletion once, at its span, labelled with
              // its id and size.
              showVariantLane: true,
              layout: DENR_LAYOUT,
    ...
    readyTimeout: 90000,
    settleMs: 6000,
    // gene track plus the variant lane, all 56 sample rows and the genotype
    // legend
    viewportHeight: 1076,
    // NO PILL. There used to be one reading "one column per variant, not per
    // span: both are ~220 bp", which existed only to correct the impression the
    // matrix layout gave — the reviewer's "are there 'giant' SV overshadowing
    // the ones that are intending to be shown?" was a question about the
    // display, not about the data. Drawn at their real spans the records answer
    // it themselves, and a caption that apologises for a rendering choice is
    // the sign the choice was wrong.
  },
