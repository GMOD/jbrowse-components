...
export const pangenomeSpecs: ScreenshotSpec[] = [
  ...
  // so nothing here can drift from what the file says.
  {
    ...
    // unconvinced. complementary visualizations are useful"). The largest of
    // the five reverse runs is boxed here and on the dotplot, at its own
    // coordinates on both axes -- 594 kb of K12 at 1,635,838-2,229,302, which
    // is IAI39 1,574,975-906,630 read backwards. The two are one figure,
    // pangenome/pggb_untangle_inversion, so the boxes need no callout saying
    // they match (review: "if this is a multipart figure, combine them").
    //
  ...
  {
    mode: 'compose',
    name: 'pangenome/pggb_untangle_inversion',
    parts: ['pangenome/pggb_untangle_rows', 'pangenome/pggb_untangle_dotplot'],
  },
