...
// The 8p23.1 inversion, which is the largest inverted chain in that file by an
// order of magnitude (the next is 747 kb on chr7). Read off the chain rather
// than off a paper:
//
...
// only legible as one because the sequence around it did not move.
const INVERSION_WINDOW_MAT = 'chr8_MATERNAL:7,300,000-12,500,000'
const INVERSION_WINDOW_PAT = 'chr8_PATERNAL:7,300,000-12,500,000'
...
// where the ribbon is a straight band and the het sites underneath it are the
// only thing separating the haplotypes. Structural agreement and sequence
// identity are different claims, and this is the frame that shows the second
// one failing where the first one holds.
//
    ...
    views: [
      {
        ...
        // strand is the whole point here: the inverted block is the one
        // off-color sweep in a view that is otherwise a single color
        colorBy: 'strand',
        drawCurves: true,
