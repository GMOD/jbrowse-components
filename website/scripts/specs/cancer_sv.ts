...
export const cancerSvSpecs: ScreenshotSpec[] = [
  ...
  // one frame, and here the pileup only has to read as a wall of clipping.
  {
    ...
    stages: [
      ...
      {
        // the gene track rides along onto the reference panel (the dialog
        // carries the launching view's non-alignments tracks), so the frame
        // says which genes the path runs through rather than drawing ribbons
        // between two bare axes
