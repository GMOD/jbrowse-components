...
export const cancerSvSpecs: ScreenshotSpec[] = [
  ...
  // builds from the reads on screen, then the synteny view picking one draws.
  // Both drive the real UI; the der3_* tracks further down are sv_multihop.py's
  // output and must not stand in for stage 2.
  //
  // The window ends just past the chr3 breakpoints so they sit right of the
  // dialog, where the clipping wall and the validated calls over it stay
  // visible. The pills say what the list cannot: the top row's three junctions
  // are all truth-set calls (truthset_8, _43, _7), and the two reads of the
  // second row place their chr10 piece at MAPQ 10 and 34 and never align the
  // 183 bp chr12 piece, so that row is an alignment miss rather than a second
  // allele.
  {
    mode: 'url',
    ...
    viewportWidth: 1300,
    url: sessionSpec(CONFIG, {
      sessionTracks: [TRUTH_SET_TRACK],
      views: [
        {
          type: 'LinearGenomeView',
          assembly: 'hg38',
          loc: 'chr3:25,355,300-25,359,900',
          tracks: [
    ...
    stages: [
      {
        ...
        annotations: [
          ...
          {
            type: 'text',
            text: 'all three junctions are validated calls',
            fontSize: 17,
            maxWidth: 260,
            leader: true,
            dx: -60,
          ...
          {
            type: 'text',
            text: 'the chr12 piece did not align in these two reads',
            fontSize: 17,
            maxWidth: 260,
            leader: true,
            dx: -60,
      ...
      {
        // `Replace current view` spends the whole frame on the result; the
        // gene and truth-set tracks ride along onto the reference panel
        actions: [
          {
            type: 'click',
            selector: `[data-testid="${CHAIN_ROUTE_TESTID}"]`,
  ...
  // The page's first reconstruction: a fold-back, chr9 running out at
  // 28,031,837 and resuming inverted from 28,059,142. All three rows are
  // truth-set junctions (the two-segment row is truthset_34; the three-segment
  // row is truthset_35 then _36 on one molecule), so the two nine-read rows are
  // two alleles through one breakage-fusion-bridge locus.
  {
    mode: 'url',
      ...
      views: [
        {
          type: 'LinearGenomeView',
          assembly: 'hg38',
          // Only the anchors: the reconstruction reads SA tags, and 200x ONT
          // over the whole 40 kb event exceeds the track's byte budget.
          loc: 'chr9:28,030,000-28,036,000',
          tracks: [
            { ...GENE_TRACK, height: 60 },
            // rides onto the synteny view's reference panel, where its
            // breakends land on the ribbon ends
            { trackId: TRUTH_SET, displayMode: 'collapsed', height: 30 },
            {
              trackId: TUMOUR,
              ...DEEP_ONT,
    ...
    stages: [
      ...
      },
      {
        ...
        annotations: [
          {
            type: 'text',
            text: 'the junction, on a validated call',
            fontSize: 17,
            leader: true,
            dx: -90,
            anchor: {
