  ...
  // one frame, and here the pileup only has to read as a wall of clipping.
  {
    ...
    stages: [
      ...
      },
      {
        ...
        // the synteny view alone, measured off the run's own clipped-below-the-
        // fold report: two panels, the ribbons between them, and under the
        // derivative both the provenance track and the reads placed on the
        // allele -- the last of which is the whole reason the frame grew
        viewportHeight: 850,
      },
    ],
  },
  ...
  // answer.
  {
    ...
    stages: [
      ...
      },
      {
        ...
        // the pileup it was launched from, and the frame has to hold the reads
        // placed on the allele as well as the provenance track. Shorter than
        // that figure's because a fold-back's derivative carries two segments
        // rather than four, so the provenance track is one row
        actions: [{ type: 'click', text: 'Replace current view' }],
        viewportHeight: 800,
      },
    ],
