...
- **Retiring a figure: drop its lock line in the same commit, and delete the PNG
  LAST.** The command is a BARE `push --allow-deletions` — `--filter` cannot
  name a retirement, because the selection is computed from the files on disk
  and the file is the thing you just deleted, so the run exits 1 with "no
  figures on disk match --filter". A bare one is safe here precisely because the
  removal is all that is left to express: dry-run it first and the diff should
  be the removals and nothing else. Every worktree's `static/img` is the same
  directory though, and `pull` installs anything the lock still lists — so
  another agent's `dev`, `build` or `screenshots` landing between your `rm` and
  your push puts the file back, the push hashes it, and the removal you asked
  for reads as "0 manifest change(s)". Do the two back to back, then check
  `figures status`: a restored file shows up under `new (not in the manifest)`
  and is an ORPHAN to delete rather than something to push.
