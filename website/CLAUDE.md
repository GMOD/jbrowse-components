...
- **Retiring a figure: drop its lock line in the same commit, and delete the PNG
  ...
  `static/img` is the same directory, and `pull` installs anything the lock
  still lists. So another agent's `dev`, `build` or `screenshots` between your
  `rm` and your `push` puts the file back, the push hashes it, and the removal
  you asked for reads as "0 manifest change(s)" — which looks like the flag not
  working. Order the deletion after the push, or check `figures status`, where a
  restored file shows up under `new (not in the manifest)` and is an ORPHAN to
  delete rather than something to push.
