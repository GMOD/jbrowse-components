...
- **A figure cropped from another figure is never stored** — cards, homepage
  images and gallery thumbs are computed by `pnpm gen:derived-figures`, so
...
- **Retiring a figure: drop its lock line in the same commit, and delete the PNG
  LAST** (`push --allow-deletions`; `--filter` selects from disk and cannot
  express it). Every worktree shares `static/img`, so another agent's `pull`
  between your `rm` and your push puts the file back and the removal reads as "0
  manifest change(s)". Check `figures status` after.
...
- Size a figure from the run's `CONTENT CLIPPED BELOW THE FOLD` /
  ...
  content overflows its own fixed height; on a pileup, `heightMode: 'grow'`
  turns that into page height they can see.
...
Run both probes after touching the card:
...
Neither can pass over an empty list.
...
absolute URL of their own keep the link they had, and every tooltip names the
host it reaches.
...
- **Few numbers in prose, and none that assert a result.** A number that names
  ...
  however real. Measurements belong in a table, the script, or a spec comment.
- **Keep specific values out of captions and callouts** — a sourced number is
  still a claim a reader cannot check against the picture.
...
- **Something in the frame has to come out negative.** This is the commonest
  ...
  "dense" or "high" without seeing what sparse looks like in the same picture,
  and the window is nearly always the lever: an exon becomes an exon EDGE, a
  promoter becomes a promoter and the gene body under it.
- **Say the thing; don't set it against the thing it isn't.** "X rather than Y",
  "not a Z, a W" — the contrastive frame is the strongest tell of text written
  here and turns a label into an argument with a reader who never held the other
  position. It fires hardest where there is least room: titles, node labels,
  callouts. Where the contrast is a real instruction it stays; the test is
  whether the sentence tells someone what to do or defends a choice.
- **A diagram's labels are names** — what each node and edge is CALLED, nothing
  else. The page under it has room and the box does not.
