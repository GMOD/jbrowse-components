...
**Live links.** `remark-figure.ts` auto-appends an "Open this view in
JBrowse ↗" link to any `<Figure src="/img/<name>.png">` whose `<name>` matches
a spec in `screenshotLiveUrls` (built from `screenshot-specs.ts`); an explicit
`link=` attribute on `<Figure>` overrides this. Figures with no matching spec
(hand-curated screenshots/diagrams that predate the spec system, or
non-app images like desktop screenshots) get no link unless `link=` is set.
Run `pnpm audit-figures` to regenerate `scripts/figure-manifest.json`, which
