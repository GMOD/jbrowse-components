...
stacked comparison — that image silently drifts from the specs. Use a `stages`
spec instead: each stage's frame is captured and the generator stacks them
vertically (`convert -append`) into one content-stable, spec-tracked PNG. To
contrast a _setting_, leave stage 1 as the loaded view and have stage 2 toggle it
from the menu (e.g. synteny "CIGAR display mode" is on the comparative view's
`[aria-label="View options"]` header button, not `view_menu_icon`), then `delay`
for any refetch/repaint; set `viewportHeight` to just the content so the stacked
frames aren't mostly whitespace. To open _both_ states from one combined figure,
`<Figure>` takes `links="Label=spec,Label=spec"` — each spec name resolves to its
live session (so the links can't drift). See `synteny_human_chimp_cigar_modes`.
