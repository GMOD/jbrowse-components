...
// Autogenerated config/model docs are CamelCase files (BaseTrack.md) but their
// cross-links — and hand-written links to them — use the lowercase Docusaurus
// `id:` slug (../basetrack), so everything else lowercases its filename id.
const docsLoader = glob({
  base: docsBase,
  pattern: ['**/*.md', '!**/CLAUDE.md*'],
  generateId: ({ entry, data }) =>
    data.slug === '/' ? 'index' : entry.replace(/\.md$/, '').toLowerCase(),
