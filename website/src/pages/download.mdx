...
export const DownloadCard = ({ os, icon, link }) => {
  const downloadLink = `https://github.com/GMOD/jbrowse-components/releases/download/tag/${config.customFields.currentVersion}/jbrowse-desktop-${config.customFields.currentVersion}-${link}`
  return (
