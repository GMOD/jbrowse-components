...
export const Link = ({ extra, children }) => {
  return (
    <a target="_blank" href={`https://jbrowse.org/code/jb2/latest/${extra}`}>
      {children}
    </a>
  )
}
...
link above will always direct to the most recent demo that has been released.
...
- <Link extra="?config=test_data%2Fconfig_demo.json&session=share-ofjI26CNas&password=ohqlR">
